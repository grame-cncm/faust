...
public class mydsp extends dsp {
	...
	public void buildUserInterface(UI ui_interface) {
		...
		ui_interface.addHorizontalSlider("attenuation", new FaustVarAccess() {
				public String getId() { return "fHslider1"; }
				public void set(float val) { fHslider1 = val; }
				public float get() { return (float)fHslider1; }
			}
			, 0.0f, -96.0f, 10.0f, 0.100000001f);
	...
	public void compute(int count, float[][] inputs, float[][] outputs) {
		...
		float fSlow1 = (float)java.lang.Math.pow(10.0f, (0.0f - (0.0500000007f * fHslider1)));
		float fSlow2 = (float)java.lang.Math.sqrt((2.0f * fSlow1));
		...
		for (int i = 0; (i < count); i = (i + 1)) {
			fRec0[0] = (input0[i] - (fSlow4 * ((fSlow7 * fRec0[2]) + (fSlow8 * fRec0[1]))));
			output0[i] = (fSlow4 * (((fSlow6 * fRec0[1]) + (fSlow9 * fRec0[0])) + (fSlow10 * fRec0[2])));
			fRec0[2] = fRec0[1];
			fRec0[1] = fRec0[0];
