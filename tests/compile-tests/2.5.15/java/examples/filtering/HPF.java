...
public class mydsp extends dsp {
	...
	public void compute(int count, float[][] inputs, float[][] outputs) {
		...
		float fSlow3 = (float)java.lang.Math.cos(fSlow0);
		float fSlow4 = (0.5f * (fSlow3 + 1.0f));
		float fSlow5 = (0.0f - (2.0f * fSlow3));
		float fSlow6 = (1.0f - fSlow1);
		float fSlow7 = (-1.0f - fSlow3);
		for (int i = 0; (i < count); i = (i + 1)) {
			fRec0[0] = (input0[i] - (fSlow2 * ((fSlow5 * fRec0[1]) + (fSlow6 * fRec0[2]))));
			output0[i] = (fSlow2 * (((fSlow4 * fRec0[0]) + (fSlow7 * fRec0[1])) + (fSlow4 * fRec0[2])));
			fRec0[2] = fRec0[1];
			fRec0[1] = fRec0[0];
