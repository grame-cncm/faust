...
void metadatamydsp(MetaGlue* m) { 
	...
	m->declare(m->metaInterface, "name", "highShelf");
}
...
void buildUserInterfacemydsp(mydsp* dsp, UIGlue* ui_interface) {
	ui_interface->openVerticalBox(ui_interface->uiInterface, "highShelf");
...
void computemydsp(mydsp* dsp, int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
	...
	float fSlow2 = ((sqrtf(fSlow0) * sinf(fSlow1)) / max(0.00100000005f, (float)dsp->fHslider2));
	...
	/* C99 loop */
	{
		...
		for (i = 0; (i < count); i = (i + 1)) {
			dsp->fRec0[0] = ((float)input0[i] - (fSlow5 * ((fSlow8 * dsp->fRec0[2]) + (fSlow10 * dsp->fRec0[1]))));
			output0[i] = (FAUSTFLOAT)(fSlow5 * (((fSlow7 * dsp->fRec0[0]) + (fSlow11 * dsp->fRec0[1])) + (fSlow12 * dsp->fRec0[2])));
