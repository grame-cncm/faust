...
void metadatamydsp(MetaGlue* m) { 
	...
	m->declare(m->metaInterface, "name", "bandFilter");
...
void instanceResetUserInterfacemydsp(mydsp* dsp) {
	dsp->fEntry0 = (FAUSTFLOAT)1000.0f;
...
void buildUserInterfacemydsp(mydsp* dsp, UIGlue* ui_interface) {
	ui_interface->openVerticalBox(ui_interface->uiInterface, "Bandfilter");
	...
	ui_interface->addNumEntry(ui_interface->uiInterface, "freq", &dsp->fEntry0, 1000.0f, 20.0f, 20000.0f, 1.0f);
...
void computemydsp(mydsp* dsp, int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
	...
	/* C99 loop */
	{
		...
		for (i = 0; (i < count); i = (i + 1)) {
			float fTemp0 = (fSlow4 * dsp->fRec0[1]);
			dsp->fRec0[0] = ((float)input0[i] - (fSlow3 * ((fSlow5 * dsp->fRec0[2]) + fTemp0)));
			output0[i] = (FAUSTFLOAT)(fSlow3 * ((fTemp0 + (fSlow7 * dsp->fRec0[0])) + (fSlow8 * dsp->fRec0[2])));
