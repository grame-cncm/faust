...
void metadatamydsp(MetaGlue* m) { 
	...
	m->declare(m->metaInterface, "description", "Simple acoustic guitar model with nylon strings.");
	...
	m->declare(m->metaInterface, "name", "NylonGuitarMidi");
...
void buildUserInterfacemydsp(mydsp* dsp, UIGlue* ui_interface) {
	ui_interface->openVerticalBox(ui_interface->uiInterface, "nylonGuitar");
...
void computemydsp(mydsp* dsp, int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
	...
	/* C99 loop */
	{
		...
		for (i = 0; (i < count); i = (i + 1)) {
			...
			dsp->fRec32[(dsp->IOTA & 2047)] = ((0.400000006f * dsp->fRec32[((dsp->IOTA - 1) & 2047)]) + (0.600000024f * dsp->fVec3[1]));
