...
void metadatamydsp(MetaGlue* m) { 
	...
	m->declare(m->metaInterface, "name", "quadEcho");
...
void computemydsp(mydsp* dsp, int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
	...
	/* C99 loop */
	{
		...
		for (i = 0; (i < count); i = (i + 1)) {
			...
			dsp->fRec2[(dsp->IOTA & 131071)] = ((float)input2[i] + (fSlow0 * dsp->fRec2[((dsp->IOTA - iSlow1) & 131071)]));
			output2[i] = (FAUSTFLOAT)dsp->fRec2[((dsp->IOTA - 0) & 131071)];
			dsp->fRec3[(dsp->IOTA & 131071)] = ((float)input3[i] + (fSlow0 * dsp->fRec3[((dsp->IOTA - iSlow1) & 131071)]));
			output3[i] = (FAUSTFLOAT)dsp->fRec3[((dsp->IOTA - 0) & 131071)];
			dsp->IOTA = (dsp->IOTA + 1);
