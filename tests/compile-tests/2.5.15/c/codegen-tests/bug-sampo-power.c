...
static int mydspSIG0_faustpower2_i(int value) {
	return (value * value);
...
static void fillmydspSIG0(mydspSIG0* dsp, int count, int* output) {
	/* C99 loop */
	{
		int i;
		for (i = 0; (i < count); i = (i + 1)) {
			dsp->iRec0[0] = (dsp->iRec0[1] + 1);
			output[i] = mydspSIG0_faustpower2_i(dsp->iRec0[0]);
...
void metadatamydsp(MetaGlue* m) { 
	m->declare(m->metaInterface, "name", "bug-sampo-power");
}
...
void classInitmydsp(int samplingFreq) {
	...
	fillmydspSIG0(sig0, 128, itbl0mydspSIG0);
...
void buildUserInterfacemydsp(mydsp* dsp, UIGlue* ui_interface) {
	ui_interface->openVerticalBox(ui_interface->uiInterface, "bug-sampo-power");
...
void computemydsp(mydsp* dsp, int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
	...
	/* C99 loop */
	{
		int i;
		for (i = 0; (i < count); i = (i + 1)) {
			output0[i] = (FAUSTFLOAT)mydsp_faustpower2_i(itbl0mydspSIG0[(int)(127.0f * fabsf((float)input0[i]))]);
