...
void metadatamydsp(MetaGlue* m) { 
	...
	m->declare(m->metaInterface, "name", "math");
}
...
void buildUserInterfacemydsp(mydsp* dsp, UIGlue* ui_interface) {
	ui_interface->openVerticalBox(ui_interface->uiInterface, "math");
...
void computemydsp(mydsp* dsp, int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
	...
	/* C99 loop */
	{
		...
		for (i = 0; (i < count); i = (i + 1)) {
			...
			int iTemp0 = (dsp->iRec0[0] + -1);
			output0[i] = (FAUSTFLOAT)(sqrtf((float)iTemp0) + (tanf((float)iTemp0) + (rintf((float)iTemp0) + (sinf((float)iTemp0) + (remainderf((float)iTemp0, (float)input5[i]) + (powf((float)input4[i], (float)iTemp0) + (min((float)iTemp0, (float)input3[i]) + (logf((float)iTemp0) + (log10f((float)iTemp0) + ((((((((((float)abs(iTemp0) + asinf((float)iTemp0)) + atan2f((float)iTemp0, (float)input0[i])) + atanf((float)iTemp0)) + ceilf((float)iTemp0)) + cosf((float)iTemp0)) + expf((float)iTemp0)) + floorf((float)iTemp0)) + fmodf((float)iTemp0, (float)input1[i])) + max((float)iTemp0, (float)input2[i])))))))))));
