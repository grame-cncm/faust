...
void metadatamydsp(MetaGlue* m) { 
	...
	m->declare(m->metaInterface, "name", "BUG080127-twgosc");
}
...
void buildUserInterfacemydsp(mydsp* dsp, UIGlue* ui_interface) {
	ui_interface->openVerticalBox(ui_interface->uiInterface, "BUG080127-twgosc");
	ui_interface->addVerticalSlider(ui_interface->uiInterface, "_freq", &dsp->fVslider0, 440.0f, 10.0f, 20000.0f, 1.0f);
...
void computemydsp(mydsp* dsp, int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
	...
	float fSlow0 = cosf((dsp->fConst0 * (float)dsp->fVslider0));
	float fSlow1 = sqrtf(((1.0f - fSlow0) / (fSlow0 + 1.0f)));
	/* C99 loop */
	{
		int i;
		for (i = 0; (i < count); i = (i + 1)) {
			...
			output1[i] = (FAUSTFLOAT)((fSlow1 - dsp->fVec0[1]) * (float)input1[i]);
