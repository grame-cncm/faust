...
class mydsp : public dsp {
 ...
 public:
	...
	virtual void instanceConstants(int samplingFreq) {
		...
		iConst13 = min(1024, max(0, (int((0.0126077095f * fConst0)) + -1)));
		iConst14 = min(1024, max(0, (int((0.00999999978f * fConst0)) + -1)));
		iConst15 = min(1024, max(0, (int((0.00773242628f * fConst0)) + -1)));
		iConst16 = min(1024, max(0, (int((0.00510204071f * fConst0)) + -1)));
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addHorizontalSlider("key", &fHslider2, 0.0f, 0.0f, 2.0f, 1.0f);
		ui_interface->addHorizontalSlider("keyboard", &fHslider3, 0.0f, 0.0f, 2.0f, 1.0f);
		ui_interface->declare(&fHslider0, "acc", "1 0 -10 0 10");
		ui_interface->addHorizontalSlider("res", &fHslider0, 0.5f, 0.0f, 1.0f, 0.00999999978f);
		ui_interface->declare(&fHslider1, "acc", "0 0 -10 0 10");
		ui_interface->addHorizontalSlider("wet", &fHslider1, 0.0f, 0.0f, 1.0f, 0.00999999978f);
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		float fSlow2 = tanf((fConst1 * powf(2.0f, (0.0833333358f * ((float(fHslider2) + (8.0f * float(fHslider3))) + -19.0f)))));
		...
		for (int i = 0; (i < count); i = (i + 1)) {
			...
			fVec1[0] = fSlow6;
			fRec12[0] = ((((fSlow6 - fVec1[1]) > 0.0f) > 0)?0.0f:min(fConst2, (fRec12[1] + (1.0f - (0.00200000009f * (fVec0[1] - fConst0))))));
			int iTemp0 = (fRec12[0] < fConst3);
			fRec10[0] = ((4.65661287e-10f * (float(iRec11[0]) * (iTemp0?((fRec12[0] < 0.0f)?0.0f:(iTemp0?(fConst4 * fRec12[0]):1.0f)):((fRec12[0] < fConst2)?((fConst4 * (0.0f - (fRec12[0] - fConst3))) + 1.0f):0.0f)))) - (fSlow7 * ((fSlow8 * fRec10[2]) + (fSlow9 * fRec10[1]))));
