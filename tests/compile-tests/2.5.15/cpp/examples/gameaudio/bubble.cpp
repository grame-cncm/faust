...
class mydsp : public dsp {
 ...
 public:
	...
	virtual void instanceConstants(int samplingFreq) {
		...
		iConst15 = min(1024, max(0, (iConst14 + -1)));
		iConst16 = int((0.00999999978f * fConst0));
		iConst17 = min(1024, max(0, (iConst16 + -1)));
		iConst18 = int((0.00773242628f * fConst0));
		iConst19 = min(1024, max(0, (iConst18 + -1)));
		iConst20 = int((0.00510204071f * fConst0));
		iConst21 = min(1024, max(0, (iConst20 + -1)));
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->closeBox();
		ui_interface->openVerticalBox("bubble");
		ui_interface->addHorizontalSlider("freq", &fHslider0, 600.0f, 150.0f, 2000.0f, 1.0f);
		ui_interface->closeBox();
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		int iSlow12 = int((fConst22 * float(fVslider3)));
		...
		int iSlow22 = min(1024, max(0, (iConst14 + iSlow21)));
		int iSlow23 = min(1024, max(0, (iConst16 + iSlow21)));
		int iSlow24 = min(1024, max(0, (iConst18 + iSlow21)));
		int iSlow25 = min(1024, max(0, (iConst20 + iSlow21)));
		for (int i = 0; (i < count); i = (i + 1)) {
			...
			fRec13[0] = ((0.99000001f * fRec13[1]) + (0.00999999978f * expf((fSlow10 * float(iRec12[0])))));
			...
			output0[i] = FAUSTFLOAT(((fRec1 + fRec0[1]) + fTemp9));
			...
			output1[i] = FAUSTFLOAT((fRec28[1] + (fRec29 + fTemp9)));
			iVec0[1] = iVec0[0];
			fRec9[1] = fRec9[0];
			fVec1[1] = fVec1[0];
			iRec12[1] = iRec12[0];
