...
function mydsp() {
	...
	this.compute = function(count, inputs, outputs) {
		...
		var fSlow3 = (0 - (2 * Math.cos(fSlow0)));
		var fSlow4 = (1 - fSlow1);
		for (var i = 0; (i < count); i = (i + 1)) {
			var fTemp0 = (fSlow3 * this.fRec0[1]);
			this.fRec0[0] = (input0[i] - (fSlow2 * (fTemp0 + (fSlow4 * this.fRec0[2]))));
			output0[i] = (this.fRec0[2] + (fSlow2 * (fTemp0 + (fSlow4 * this.fRec0[0]))));
