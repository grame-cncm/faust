...
<<includeclass>>
	...
	def process_block(
		...
		# Unnormalize parameters and merge into carry
		params = self.unnormalize(i)
		for k, v in params.items():
			carry[k] = v

		def scan_body(carry, input_sample):
			new_carry, y = self.tick(carry, input_sample)
			return new_carry, y
		...
		# inputs has shape (num_inputs, length), need (length, num_inputs) for scan
		new_carry, outputs = jax.lax.scan(scan_body, carry, jnp.moveaxis(inputs, -1, 0), length=length, unroll=unroll)
		# outputs has shape (length, num_outputs), need (num_outputs, length)
	...
	def __call__(
		...
		# x has shape (num_inputs, length), need (length, num_inputs) for scan
		new_carry, outputs = jax.lax.scan(scan_body, carry, jnp.moveaxis(x, -1, 0), length=length, unroll=unroll)
		# outputs has shape (length, num_outputs), need (num_outputs, length)
		outputs = jnp.moveaxis(outputs, 0, -1)
