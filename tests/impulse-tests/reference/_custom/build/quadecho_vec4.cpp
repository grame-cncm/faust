...
class mydsp : public dsp {
 ...
 public:
	...
	virtual void instanceResetUserInterface() {
		...
		fHslider1 = static_cast<FAUSTFLOAT>(108.7);
	}
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addHorizontalSlider("feedback", &fHslider0, FAUSTFLOAT(1e+02), FAUSTFLOAT(0.0), FAUSTFLOAT(1e+02), FAUSTFLOAT(0.1));
		ui_interface->addHorizontalSlider("millisecond", &fHslider1, FAUSTFLOAT(108.7), FAUSTFLOAT(0.0), FAUSTFLOAT(1e+03), FAUSTFLOAT(0.1));
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (vindex = 0; vindex <= (faust_wrap_sub(count, 4)); vindex = faust_wrap_add(vindex, 4)) {
			...
			/* Recursive loop 4 */
			/* Pre code */
			fRec2_idx = (faust_wrap_add(fRec2_idx, fRec2_idx_save)) & 131071;
			/* Compute code */
			for (int i = 0; i < vsize; i = faust_wrap_add(i, 1)) {
				fRec2[(faust_wrap_add(i, fRec2_idx)) & 131071] = static_cast<double>(input2[i]) + fSlow0 * fRec2[(faust_wrap_sub(faust_wrap_add(i, fRec2_idx), iSlow1)) & 131071];
			}
			/* Post code */
			...
			/* Compute code */
			for (int i = 0; i < vsize; i = faust_wrap_add(i, 1)) {
				output2[i] = static_cast<FAUSTFLOAT>(fRec2[(faust_wrap_add(i, fRec2_idx)) & 131071]);
			}
			/* Recursive loop 6 */
			/* Pre code */
			fRec3_idx = (faust_wrap_add(fRec3_idx, fRec3_idx_save)) & 131071;
			/* Compute code */
			for (int i = 0; i < vsize; i = faust_wrap_add(i, 1)) {
				fRec3[(faust_wrap_add(i, fRec3_idx)) & 131071] = static_cast<double>(input3[i]) + fSlow0 * fRec3[(faust_wrap_sub(faust_wrap_add(i, fRec3_idx), iSlow1)) & 131071];
			}
			/* Post code */
			...
			/* Compute code */
			for (int i = 0; i < vsize; i = faust_wrap_add(i, 1)) {
				output3[i] = static_cast<FAUSTFLOAT>(fRec3[(faust_wrap_add(i, fRec3_idx)) & 131071]);
			}
		}
		/* Remaining frames */
		if (vindex < count) {
