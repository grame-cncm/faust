...
class mydsp : public dsp {
 ...
 public:
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addHorizontalBargraph("hbargraph0", &fHbargraph1, FAUSTFLOAT(-7e+01), FAUSTFLOAT(5.0));
		ui_interface->declare(&fHbargraph0, "2", "");
		ui_interface->declare(&fHbargraph0, "unit", "dB");
		ui_interface->addHorizontalBargraph("hbargraph1", &fHbargraph0, FAUSTFLOAT(-7e+01), FAUSTFLOAT(5.0));
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (vindex = 0; vindex <= (faust_wrap_sub(count, 4)); vindex = faust_wrap_add(vindex, 4)) {
			...
			for (int i = 0; i < vsize; i = faust_wrap_add(i, 1)) {
				fRec0[i] = std::max<double>(fRec0[faust_wrap_sub(i, 1)] - fConst0, std::fabs(static_cast<double>(input0[i])));
			}
			/* Post code */
			...
			/* Compute code */
			for (int i = 0; i < vsize; i = faust_wrap_add(i, 1)) {
				fHbargraph0 = static_cast<FAUSTFLOAT>(2e+01 * std::log10(std::max<double>(0.00031622776601683794, fRec0[i])));
			...
			/* Compute code */
			for (int i = 0; i < vsize; i = faust_wrap_add(i, 1)) {
				fRec1[i] = std::max<double>(fRec1[faust_wrap_sub(i, 1)] - fConst0, std::fabs(static_cast<double>(input1[i])));
			}
			/* Post code */
			...
			/* Compute code */
			for (int i = 0; i < vsize; i = faust_wrap_add(i, 1)) {
				fHbargraph1 = static_cast<FAUSTFLOAT>(2e+01 * std::log10(std::max<double>(0.00031622776601683794, fRec1[i])));
				output1[i] = static_cast<FAUSTFLOAT>(static_cast<double>(input1[i]));
			}
		}
		/* Remaining frames */
		if (vindex < count) {
