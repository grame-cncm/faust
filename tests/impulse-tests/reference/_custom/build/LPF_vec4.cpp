...
class mydsp : public dsp {
 ...
 public:
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		double fSlow5 = std::cos(fSlow0);
		double fSlow6 = 2.0 * fSlow5;
		double fRec0_tmp[8];
		double* fRec0 = &fRec0_tmp[4];
		double fSlow7 = (1.0 - fSlow5) / fSlow2;
		int vindex = 0;
		/* Main loop */
		for (vindex = 0; vindex <= (faust_wrap_sub(count, 4)); vindex = faust_wrap_add(vindex, 4)) {
			...
			for (int i = 0; i < vsize; i = faust_wrap_add(i, 1)) {
				fRec0[i] = static_cast<double>(input0[i]) - fSlow3 * (fSlow4 * fRec0[faust_wrap_sub(i, 2)] - fSlow6 * fRec0[faust_wrap_sub(i, 1)]);
			}
			/* Post code */
			...
			/* Compute code */
			for (int i = 0; i < vsize; i = faust_wrap_add(i, 1)) {
				output0[i] = static_cast<FAUSTFLOAT>(fSlow7 * (fRec0[faust_wrap_sub(i, 1)] + 0.5 * fRec0[i] + 0.5 * fRec0[faust_wrap_sub(i, 2)]));
			}
		}
		/* Remaining frames */
		if (vindex < count) {
