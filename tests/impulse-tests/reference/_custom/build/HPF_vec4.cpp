	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (vindex = 0; vindex <= (faust_wrap_sub(count, 4)); vindex = faust_wrap_add(vindex, 4)) {
			...
			for (int i = 0; i < vsize; i = faust_wrap_add(i, 1)) {
				fRec0[i] = static_cast<double>(input0[i]) - fSlow2 * (fSlow3 * fRec0[faust_wrap_sub(i, 2)] - fSlow5 * fRec0[faust_wrap_sub(i, 1)]);
			}
			/* Post code */
			...
			/* Compute code */
			for (int i = 0; i < vsize; i = faust_wrap_add(i, 1)) {
				output0[i] = static_cast<FAUSTFLOAT>(fSlow2 * (fSlow6 * fRec0[faust_wrap_sub(i, 1)] + fSlow7 * fRec0[i] + fSlow7 * fRec0[faust_wrap_sub(i, 2)]));
			}
		}
		/* Remaining frames */
		if (vindex < count) {
