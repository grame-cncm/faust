	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (vindex = 0; vindex <= (faust_wrap_sub(count, 4)); vindex = faust_wrap_add(vindex, 4)) {
			...
			for (int i = 0; i < vsize; i = faust_wrap_add(i, 1)) {
				fZec0[i] = fSlow5 * fRec0[faust_wrap_sub(i, 1)];
				fRec0[i] = static_cast<double>(input0[i]) - fSlow3 * (fSlow4 * fRec0[faust_wrap_sub(i, 2)] + fZec0[i]);
			}
			/* Post code */
			...
			/* Compute code */
			for (int i = 0; i < vsize; i = faust_wrap_add(i, 1)) {
				output0[i] = static_cast<FAUSTFLOAT>(fSlow3 * (fZec0[i] + fSlow7 * fRec0[i] + fSlow8 * fRec0[faust_wrap_sub(i, 2)]));
			}
		}
		/* Remaining frames */
		if (vindex < count) {
