...
class mydsp : public dsp {
 ...
 public:
	...
	virtual void instanceResetUserInterface() {
		fVslider0 = static_cast<FAUSTFLOAT>(0.0697);
	}
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addVerticalSlider("Volume", &fVslider0, FAUSTFLOAT(0.0697), FAUSTFLOAT(0.0), FAUSTFLOAT(1.0), FAUSTFLOAT(0.1));
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		double fSlow0 = 4.656612875245797e-10 * static_cast<double>(fVslider0);
		int vindex = 0;
		/* Main loop */
		for (vindex = 0; vindex <= (faust_wrap_sub(count, 4)); vindex = faust_wrap_add(vindex, 4)) {
			...
			for (int i = 0; i < vsize; i = faust_wrap_add(i, 1)) {
				iRec0[i] = faust_wrap_add(faust_wrap_mul(1103515245, iRec0[faust_wrap_sub(i, 1)]), 12345);
			}
			/* Post code */
			...
			/* Compute code */
			for (int i = 0; i < vsize; i = faust_wrap_add(i, 1)) {
				output0[i] = static_cast<FAUSTFLOAT>(fSlow0 * static_cast<double>(iRec0[i]));
			}
		}
		/* Remaining frames */
		if (vindex < count) {
