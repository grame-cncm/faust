  ...
  public:
	...
	void fillmydspSIG0(int count, int* table) {
		for (int i1 = 0; i1 < count; i1 = faust_wrap_add(i1, 1)) {
			...
			imydspSIG0Wave0_idx = (faust_wrap_add(1, imydspSIG0Wave0_idx)) % 6;
		}
	}
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (vindex = 0; vindex <= (faust_wrap_sub(count, 4)); vindex = faust_wrap_add(vindex, 4)) {
			...
			for (int i = 0; i < vsize; i = faust_wrap_add(i, 1)) {
				output0[i] = static_cast<FAUSTFLOAT>(static_cast<double>(input1[i]) * std::sin(static_cast<double>(itbl0mydspSIG0[static_cast<int>(4.0 * std::fabs(static_cast<double>(input0[i])))])));
			}
		}
		/* Remaining frames */
		if (vindex < count) {
