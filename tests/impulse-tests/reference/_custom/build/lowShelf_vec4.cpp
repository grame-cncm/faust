...
class mydsp : public dsp {
 ...
 public:
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		double fSlow5 = fSlow0 + fSlow4;
		double fSlow6 = fSlow2 + fSlow5 + 1.0;
		double fSlow7 = 1.0 / fSlow6;
		double fSlow8 = fSlow5 + (1.0 - fSlow2);
		double fSlow9 = (fSlow0 + 1.0) * fSlow3;
		double fSlow10 = 2.0 * (fSlow0 + fSlow9 + -1.0);
		double fRec0_tmp[8];
		double* fRec0 = &fRec0_tmp[4];
		double fSlow11 = fSlow0 / fSlow6;
		double fSlow12 = fSlow0 + fSlow2 + (1.0 - fSlow4);
		double fSlow13 = 2.0 * (fSlow0 + (-1.0 - fSlow9));
		double fSlow14 = fSlow0 + (1.0 - (fSlow4 + fSlow2));
		int vindex = 0;
		/* Main loop */
		for (vindex = 0; vindex <= (faust_wrap_sub(count, 4)); vindex = faust_wrap_add(vindex, 4)) {
			...
			for (int i = 0; i < vsize; i = faust_wrap_add(i, 1)) {
				fRec0[i] = static_cast<double>(input0[i]) - fSlow7 * (fSlow8 * fRec0[faust_wrap_sub(i, 2)] - fSlow10 * fRec0[faust_wrap_sub(i, 1)]);
			}
			/* Post code */
			...
			/* Compute code */
			for (int i = 0; i < vsize; i = faust_wrap_add(i, 1)) {
				output0[i] = static_cast<FAUSTFLOAT>(fSlow11 * (fSlow12 * fRec0[i] + fSlow13 * fRec0[faust_wrap_sub(i, 1)] + fSlow14 * fRec0[faust_wrap_sub(i, 2)]));
			}
		}
		/* Remaining frames */
		if (vindex < count) {
