...
class mydsp : public dsp {
 ...
 public:
	...
	virtual void instanceResetUserInterface() {
		...
		fHslider0 = static_cast<FAUSTFLOAT>(327.023);
		fHslider1 = static_cast<FAUSTFLOAT>(0.0);
		fHslider2 = static_cast<FAUSTFLOAT>(0.5);
		fHslider3 = static_cast<FAUSTFLOAT>(271.994);
	}
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addHorizontalSlider("excitation", &fHslider0, FAUSTFLOAT(327.023), FAUSTFLOAT(2.0), FAUSTFLOAT(512.0), FAUSTFLOAT(1.0));
		...
		ui_interface->addHorizontalSlider("duration", &fHslider3, FAUSTFLOAT(271.994), FAUSTFLOAT(2.0), FAUSTFLOAT(512.0), FAUSTFLOAT(1.0));
		ui_interface->closeBox();
		ui_interface->closeBox();
	}
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (vindex = 0; vindex <= (faust_wrap_sub(count, 4)); vindex = faust_wrap_add(vindex, 4)) {
			...
			/* Compute code */
			for (int i = 0; i < vsize; i = faust_wrap_add(i, 1)) {
				fYec0[(faust_wrap_add(i, fYec0_idx)) & 1023] = fSlow2 * (fRec0[faust_wrap_sub(i, 1)] + fRec0[faust_wrap_sub(i, 2)]) + fSlow3 * static_cast<double>(iRec1[i]) * static_cast<double>(fRec2[i] > 0.0);
				fRec0[i] = fYec0[(faust_wrap_sub(faust_wrap_add(i, fYec0_idx), iSlow4)) & 1023];
			}
			/* Post code */
