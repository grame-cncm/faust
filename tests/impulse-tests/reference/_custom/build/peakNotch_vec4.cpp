...
class mydsp : public dsp {
 ...
 public:
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addHorizontalSlider("Gain", &fHslider2, FAUSTFLOAT(1.0), FAUSTFLOAT(0.0), FAUSTFLOAT(8.0), FAUSTFLOAT(0.01));
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		double fSlow2 = std::max<double>(0.001, static_cast<double>(fHslider1));
		double fSlow3 = std::sqrt(std::max<double>(1e-05, static_cast<double>(fHslider2)));
		double fSlow4 = 0.5 * (fSlow1 / (fSlow2 * fSlow3));
		double fSlow5 = 1.0 / (fSlow4 + 1.0);
		double fSlow6 = 1.0 - fSlow4;
		double fSlow7 = 2.0 * std::cos(fSlow0);
		double fZec0[4];
		double fRec0_tmp[8];
		double* fRec0 = &fRec0_tmp[4];
		double fSlow8 = 0.5 * (fSlow3 * fSlow1 / fSlow2);
		double fSlow9 = fSlow8 + 1.0;
		...
		for (vindex = 0; vindex <= (faust_wrap_sub(count, 4)); vindex = faust_wrap_add(vindex, 4)) {
			...
			for (int i = 0; i < vsize; i = faust_wrap_add(i, 1)) {
				fZec0[i] = fSlow7 * fRec0[faust_wrap_sub(i, 1)];
				fRec0[i] = static_cast<double>(input0[i]) - fSlow5 * (fSlow6 * fRec0[faust_wrap_sub(i, 2)] - fZec0[i]);
			}
			/* Post code */
			...
			/* Compute code */
			for (int i = 0; i < vsize; i = faust_wrap_add(i, 1)) {
				output0[i] = static_cast<FAUSTFLOAT>(fSlow5 * (fSlow9 * fRec0[i] - fZec0[i] + fSlow10 * fRec0[faust_wrap_sub(i, 2)]));
			}
		}
		/* Remaining frames */
		if (vindex < count) {
