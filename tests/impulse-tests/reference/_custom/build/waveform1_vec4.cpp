...
const static double fmydspWave0[10] = {0.1,0.123,0.142567,0.152637391,0.568453,0.1,0.123,0.142567,0.152637391,0.568453};
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (vindex = 0; vindex <= (faust_wrap_sub(count, 4)); vindex = faust_wrap_add(vindex, 4)) {
			...
			for (int i = 0; i < vsize; i = faust_wrap_add(i, 1)) {
				output0[i] = static_cast<FAUSTFLOAT>(10);
			}
			/* Vectorizable loop 1 */
			/* Compute code */
			for (int i = 0; i < vsize; i = faust_wrap_add(i, 1)) {
				output1[i] = static_cast<FAUSTFLOAT>(fmydspWave0[(faust_wrap_add(fmydspWave0_idx, i)) % 10]);
			}
			/* Post code */
			fmydspWave0_idx = (faust_wrap_add(fmydspWave0_idx, vsize)) % 10;
		}
		/* Remaining frames */
		if (vindex < count) {
			FAUSTFLOAT* output0 = &output0_ptr[vindex];
