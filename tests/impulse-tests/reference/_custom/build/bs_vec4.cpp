...
class mydsp : public dsp {
 ...
 public:
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		double fSlow0 = 1e+01 / static_cast<double>(count);
		double fZec0[4];
		double fZec1[4];
		double fRec0_tmp[8];
		...
		for (vindex = 0; vindex <= (faust_wrap_sub(count, 4)); vindex = faust_wrap_add(vindex, 4)) {
			...
			for (int i = 0; i < vsize; i = faust_wrap_add(i, 1)) {
				fZec0[i] = fSlow0 + fRec0[faust_wrap_sub(i, 1)];
				fZec1[i] = fRec0[faust_wrap_sub(i, 1)] - fSlow0;
				fRec0[i] = ((fZec0[i] < 1e+01) ? fZec0[i] : ((fZec1[i] > 1e+01) ? fZec1[i] : 1e+01));
			}
			/* Post code */
