...
class mydsp : public dsp {
 ...
 public:
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		double fSlow0 = 0.005555555555555556 * (static_cast<double>(fEntry0) + -9e+01);
		double fSlow1 = std::sqrt(-fSlow0);
		double fSlow2 = std::sqrt(fSlow0 + 1.0);
		int vindex = 0;
		/* Main loop */
		for (vindex = 0; vindex <= (faust_wrap_sub(count, 4)); vindex = faust_wrap_add(vindex, 4)) {
			...
			for (int i = 0; i < vsize; i = faust_wrap_add(i, 1)) {
				output0[i] = static_cast<FAUSTFLOAT>(fSlow1 * static_cast<double>(input0[i]));
			}
			/* Vectorizable loop 1 */
			/* Compute code */
			for (int i = 0; i < vsize; i = faust_wrap_add(i, 1)) {
				output1[i] = static_cast<FAUSTFLOAT>(fSlow2 * static_cast<double>(input0[i]));
			}
		}
		/* Remaining frames */
		if (vindex < count) {
