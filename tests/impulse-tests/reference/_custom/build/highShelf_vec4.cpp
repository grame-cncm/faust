...
class mydsp : public dsp {
 ...
 public:
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addHorizontalSlider("Freq", &fHslider1, FAUSTFLOAT(1e+03), FAUSTFLOAT(1e+02), FAUSTFLOAT(1e+04), FAUSTFLOAT(1.0));
		ui_interface->declare(&fHslider0, "unit", "dB");
		ui_interface->addHorizontalSlider("Gain", &fHslider0, FAUSTFLOAT(0.0), FAUSTFLOAT(-1e+01), FAUSTFLOAT(1e+01), FAUSTFLOAT(0.1));
		ui_interface->addHorizontalSlider("Q", &fHslider2, FAUSTFLOAT(1.0), FAUSTFLOAT(0.01), FAUSTFLOAT(1e+02), FAUSTFLOAT(0.01));
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		double fSlow0 = std::pow(1e+01, 0.025 * static_cast<double>(fHslider0));
		double fSlow1 = fConst0 * std::max<double>(0.0, static_cast<double>(fHslider1));
		double fSlow2 = std::sqrt(fSlow0) * std::sin(fSlow1) / std::max<double>(0.001, static_cast<double>(fHslider2));
		double fSlow3 = std::cos(fSlow1);
		double fSlow4 = (fSlow0 + -1.0) * fSlow3;
		double fSlow5 = fSlow0 + fSlow2 + (1.0 - fSlow4);
		double fSlow6 = 1.0 / fSlow5;
		double fSlow7 = fSlow0 + (1.0 - (fSlow4 + fSlow2));
		double fSlow8 = (fSlow0 + 1.0) * fSlow3;
		double fSlow9 = 2.0 * (fSlow0 + (-1.0 - fSlow8));
		double fRec0_tmp[8];
		double* fRec0 = &fRec0_tmp[4];
		double fSlow10 = fSlow0 / fSlow5;
		double fSlow11 = fSlow0 + fSlow4;
		double fSlow12 = fSlow11 + fSlow2 + 1.0;
		double fSlow13 = 2.0 * (fSlow0 + fSlow8 + -1.0);
		double fSlow14 = fSlow11 + (1.0 - fSlow2);
		int vindex = 0;
		/* Main loop */
		for (vindex = 0; vindex <= (faust_wrap_sub(count, 4)); vindex = faust_wrap_add(vindex, 4)) {
			...
			for (int i = 0; i < vsize; i = faust_wrap_add(i, 1)) {
				fRec0[i] = static_cast<double>(input0[i]) - fSlow6 * (fSlow7 * fRec0[faust_wrap_sub(i, 2)] + fSlow9 * fRec0[faust_wrap_sub(i, 1)]);
			}
			/* Post code */
			...
			/* Compute code */
			for (int i = 0; i < vsize; i = faust_wrap_add(i, 1)) {
				output0[i] = static_cast<FAUSTFLOAT>(fSlow10 * (fSlow12 * fRec0[i] - fSlow13 * fRec0[faust_wrap_sub(i, 1)] + fSlow14 * fRec0[faust_wrap_sub(i, 2)]));
			}
		}
		/* Remaining frames */
		if (vindex < count) {
