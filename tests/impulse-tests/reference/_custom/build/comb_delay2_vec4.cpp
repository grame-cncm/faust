 ...
 public:
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (vindex = 0; vindex <= (faust_wrap_sub(count, 4)); vindex = faust_wrap_add(vindex, 4)) {
			...
			/* Vectorizable loop 2 */
			/* Pre code */
			fYec0_idx = (faust_wrap_add(fYec0_idx, fYec0_idx_save)) & 127;
			/* Compute code */
			for (int i = 0; i < vsize; i = faust_wrap_add(i, 1)) {
				fYec0[(faust_wrap_add(i, fYec0_idx)) & 127] = ftbl0mydspSIG0[std::max<int>(0, std::min<int>(static_cast<int>(65536.0 * fRec0[i]), 65535))];
			}
			/* Post code */
			...
			/* Compute code */
			for (int i = 0; i < vsize; i = faust_wrap_add(i, 1)) {
				output0[i] = static_cast<FAUSTFLOAT>(fYec0[(faust_wrap_sub(faust_wrap_add(i, fYec0_idx), faust_wrap_add(static_cast<int>(std::max<double>(0.0, std::min<double>(1e+01, 5.0 * (ftbl0mydspSIG0[std::max<int>(0, std::min<int>(static_cast<int>(65536.0 * fRec1[i]), 65535))] + 1.0)))), 100))) & 127]);
			}
		}
		/* Remaining frames */
		if (vindex < count) {
