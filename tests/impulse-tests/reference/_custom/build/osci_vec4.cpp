...
class mydsp : public dsp {
 ...
 public:
	...
	virtual void instanceResetUserInterface() {
		...
		fHslider1 = static_cast<FAUSTFLOAT>(564.0);
	}
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addHorizontalSlider("freq", &fHslider1, FAUSTFLOAT(564.0), FAUSTFLOAT(2e+01), FAUSTFLOAT(2.4e+04), FAUSTFLOAT(1.0));
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (vindex = 0; vindex <= (faust_wrap_sub(count, 4)); vindex = faust_wrap_add(vindex, 4)) {
			...
			/* Compute code */
			for (int i = 0; i < vsize; i = faust_wrap_add(i, 1)) {
				output0[i] = static_cast<FAUSTFLOAT>(fRec0[i] * (fZec2[i] + (fZec0[i] - std::floor(fZec0[i])) * (ftbl0mydspSIG0[std::max<int>(0, std::min<int>(faust_wrap_add(iZec1[i], 1), 65536))] - fZec2[i])));
			}
		}
		/* Remaining frames */
		if (vindex < count) {
