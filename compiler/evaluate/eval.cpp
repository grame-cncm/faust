...
static Tree realeval(Tree exp, Tree visited, Tree localValEnv)
{
    ...
    if (xt || isBoxInt(exp) || isBoxReal(exp) || isBoxWire(exp) || isBoxCut(exp) || isBoxPrim0(exp) ||
        ...
        const char* l1 = tree2str(label);
        const char* l2 = evalLabel(l1, visited, localValEnv);
        return (boxVSlider(tree(l2), tree(eval2double(cur, visited, localValEnv)),
        ...
        const char* l1 = tree2str(label);
        const char* l2 = evalLabel(l1, visited, localValEnv);
        return (boxHSlider(tree(l2), tree(eval2double(cur, visited, localValEnv)),
                           tree(eval2double(lo, visited, localValEnv)), tree(eval2double(hi, visited, localValEnv)),
                           tree(eval2double(step, visited, localValEnv))));
        ...
        const char* l1 = tree2str(label);
        const char* l2 = evalLabel(l1, visited, localValEnv);
        return (boxNumEntry(tree(l2), tree(eval2double(cur, visited, localValEnv)),
                            tree(eval2double(lo, visited, localValEnv)), tree(eval2double(hi, visited, localValEnv)),
                            tree(eval2double(step, visited, localValEnv))));
        ...
        const char* l1 = tree2str(label);
        const char* l2 = evalLabel(l1, visited, localValEnv);
        return boxHBargraph(tree(l2), tree(eval2double(lo, visited, localValEnv)),
                            tree(eval2double(hi, visited, localValEnv)));

    } else if (isBoxVBargraph(exp, label, lo, hi)) {
        const char* l1 = tree2str(label);
        const char* l2 = evalLabel(l1, visited, localValEnv);
        return boxVBargraph(tree(l2), tree(eval2double(lo, visited, localValEnv)),
                            tree(eval2double(hi, visited, localValEnv)));
...
static void writeIdentValue(std::string& dst, const std::string& format, const std::string& ident, Tree visited,
                            Tree localValEnv)
{
...
static Tree applyList(Tree fun, Tree larg)
{
    ...
    if (!isClosure(fun, abstr, globalDefEnv, visited, localValEnv)) {
        ...
        if (outs > ins) {
            ...
            cerr << "when applying : " << boxpp(fun) << endl << "           to : " << boxpp(larg) << endl;
            assert(false);
        }

        if ((outs == 1) && ((isBoxPrim2(fun, &p2) && (p2 != sigPrefix) && (p2 != sigVectorize)) ||
