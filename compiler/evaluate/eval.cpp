...
static Tree realeval(Tree exp, Tree visited, Tree localValEnv)
{
    ...
    if (xt || isBoxInt(exp) || isBoxReal(exp) || isBoxWire(exp) || isBoxCut(exp) || isBoxMem(exp) || isBoxDelay(exp) || isBoxPrim0(exp) ||
        ...
        if (isNumericalTuple(a1, lsig) && (xxt || isBoxWire(a2) || isBoxPrim1(a2) || isBoxPrim2(a2))) {
            ...
            Tree lres = boxPropagateSig(a2, lsig);
        ...
        if ((i1 == 0) & (o1 == 1) & (i2 == 0) & (o2 == 1) & (i3 == 0) & (o3 > 1) & ((o3 % 2) == 0)) {
            ...
            Tree ls1 = boxPropagateSig(v1, makeSigInputList(0));
            Tree ls2 = boxPropagateSig(v2, makeSigInputList(0));
            Tree lsr = boxPropagateSig(vr, makeSigInputList(0));
...
static bool isBoxNumeric(Tree in, Tree& out)
{
    ...
    if (isBoxInt(in, &i) || isBoxReal(in, &x)) {
        ...
        if (getBoxType(v, &numInputs, &numOutputs) && (numInputs == 0) && (numOutputs == 1)) {
            ...
            Tree lsignals = boxPropagateSig(v, makeSigInputList(numInputs));
...
static double eval2double(Tree exp, Tree visited, Tree localValEnv)
{
    ...
    if ((numInputs > 0) || (numOutputs != 1)) {
        ...
        Tree lsignals = boxPropagateSig(diagram, makeSigInputList(numInputs));
...
Tree numericBoxSimplification(Tree box)
{
    ...
    if (ins == 0 && outs == 1) {
        ...
        if (isBoxInt(box, &i) || isBoxReal(box, &x)) {
            ...
            int    ii;
            double xx;
            Tree   lsignals = boxPropagateSig(box, makeSigInputList(0));
