...
static Tree realeval(Tree exp, Tree visited, Tree localValEnv)
{
    ...
    if (xt || isBoxInt(exp) || isBoxReal(exp) || isBoxWire(exp) || isBoxCut(exp) || isBoxPrim0(exp) || isBoxPrim1(exp) || isBoxPrim2(exp) ||
        isBoxPrim3(exp) || isBoxPrim4(exp) || isBoxPrim5(exp) || isBoxFFun(exp) || isBoxFConst(exp) || isBoxFVar(exp) ||
        isBoxWaveform(exp)) {
        return exp;
...
static Tree applyList(Tree fun, Tree larg)
{
    ...
    if (!isClosure(fun, abstr, globalDefEnv, visited, localValEnv)) {
        ...
        if ((outs == 1) &&
            ((isBoxPrim2(fun, &p2) && (p2 != sigPrefix)) || (getUserData(fun) && ((xtended*)getUserData(fun))->isSpecialInfix()))) {
            // special case : /(3) ==> _,3 : /
