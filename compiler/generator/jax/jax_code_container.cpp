...
void JAXCodeContainer::produceClass()
{
    ...
    *fOut << "def add_slider(self, label: str, init: float, a_min: float, a_max: float, scale_mode='linear'):";
    tab(n + 2, *fOut);
    *fOut << "init_unit = jnp.interp(init, [a_min, a_max], [-1.,1.])";
    tab(n + 2, *fOut);
    ...
    *fOut << "if scale_mode == 'linear':";
    tab(n + 3, *fOut);
    *fOut << "param = jnp.interp(param, [-1., 1.], [a_min, a_max])";
    tab(n + 2, *fOut);
    *fOut << "elif scale_mode == 'exp':";
    tab(n + 3, *fOut);
    *fOut << "param = jnp.interp(param, [-1., 1.], [0., 1.])";
    tab(n + 3, *fOut);
    *fOut << "param = jnp.interp(jnp.exp(param), [1., jnp.e], [a_min, a_max])";
    tab(n + 2, *fOut);
    *fOut << "elif scale_mode == 'log':";
    tab(n + 3, *fOut);
    *fOut << "param = jnp.interp(param, [-1., 1.], [10.^-4, 1.])";
    tab(n + 3, *fOut);
    *fOut << "param = jnp.interp(jnp.log10(param), [-4., 0.], [a_min, a_max])";
    tab(n + 2, *fOut);
    *fOut << "else:";
    tab(n + 3, *fOut);
