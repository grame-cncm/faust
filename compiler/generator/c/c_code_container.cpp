...
/*
 C backend and module description:
    ...
    1) in -os mode:
        ...
        - CScalarOneSampleCodeContainer1 (used in -os0) separates the DSP control state in
 iControl/fControl (possibly to be allocated elsewhere)
        - CScalarOneSampleCodeContainer2 (used in -os1) separates the DSP control state in
 iControl/fControl and the DSP state in iZone/fZone (possibly to be allocated elsewhere)
        - CScalarOneSampleCodeContainer3 (used in -os2) separates the DSP control state in
 ...
 memory are generated.
        - CScalarOneSampleCodeContainer4 (used in -os3) is similar to
                                                     ...
                                                     int sub_container_type)
{
    if (gGlobal->gOneSample == 0) {
        return new CScalarOneSampleCodeContainer1(name, numInputs, numOutputs, dst,
                                                  sub_container_type);
    } else if (gGlobal->gOneSample == 1) {
        return new CScalarOneSampleCodeContainer2(name, numInputs, numOutputs, dst,
                                                  sub_container_type);
    } else if (gGlobal->gOneSample == 2) {
        return new CScalarOneSampleCodeContainer3(name, numInputs, numOutputs, dst,
                                                  sub_container_type);
    } else if (gGlobal->gOneSample == 3) {
        return new CScalarOneSampleCodeContainer4(name, numInputs, numOutputs, dst,
                                                  sub_container_type);
    } else {
    ...
    *fOut << "} " << fKlassName << ";";
    ...
    if (!gGlobal->gLightMode) {
        ...
        *fOut << "static " << fKlassName << "* new" << fKlassName << "() {" << " return ("
              << fKlassName << "*)calloc(1, sizeof(" << fKlassName << ")); }";
...
void CScalarOneSampleCodeContainer1::produceClass()
{
    ...
    *fOut << "void control" << fKlassName << "(" << fKlassName << "* dsp, "
          << subst("int* RESTRICT iControl, $0* RESTRICT fControl) {", ifloat());
    tab(n + 1, *fOut);
    fCodeProducer->Tab(n + 1);
...
void CScalarOneSampleCodeContainer2::produceClass()
{
    ...
    *fOut << "}";
    ...
    tab(n, *fOut);
    *fOut << "void instanceConstants" << fKlassName << "(" << fKlassName
          << "* dsp, int sample_rate, " << subst("int* iZone, $0* fZone) {", ifloat());
    {
        tab(n + 1, *fOut);
        fCodeProducer->Tab(n + 1);
        // Rename 'sig' in 'dsp', remove 'dsp' allocation, inline subcontainers 'instanceInit' and
    ...
    *fOut << "void control" << fKlassName << "(" << fKlassName << "* dsp, "
          << subst(
                 "int* RESTRICT iControl, $0* RESTRICT fControl, int* RESTRICT iZone, $0* RESTRICT "
                 "fZone) {",
                 ifloat());
    tab(n + 1, *fOut);
    fCodeProducer->Tab(n + 1);
...
void CScalarOneSampleCodeContainer3::produceClass()
{
    ...
    fCodeProducer = new CInstVisitor2(fOut, fKlassName,
                                      std::max(0, heap_counter.fSizeBytes - gGlobal->gFPGAMemory),
                                      Address::kFunArgs);
            ...
            for (set<Tree>::iterator j = i.second.begin(); j != i.second.end(); j++) {
                if (j == i.second.begin()) {
                    ...
                    *fOut << "m->declare(m->metaInterface, \"" << *(i.first) << "\", " << **j
                    ...
                    tab(tabs + 1, *fOut);
                    *fOut << "m->declare(m->metaInterface, \"" << "contributor" << "\", " << **j
                          << ");";
                }
            }
        }
...
void CScalarOneSampleCodeContainer1::generateComputeAux(int n)
{
    ...
    if (gGlobal->gInPlace) {
        *fOut << "void compute" << fKlassName << "(" << fKlassName
              << subst(
                     "* dsp, $0* inputs, $0* outputs, int* RESTRICT iControl, $1* RESTRICT "
                     "fControl) {",
                     xfloat(), ifloat());
    } else {
        *fOut << "void compute" << fKlassName << "(" << fKlassName
...
void CScalarOneSampleCodeContainer2::generateComputeAux(int n)
{
    ...
    if (gGlobal->gInPlace) {
        ...
        *fOut << "void compute" << fKlassName << "(" << fKlassName
              << subst(
                     "* dsp, $0* RESTRICT inputs, $0* RESTRICT outputs, int* RESTRICT iControl, "
                     "$1* RESTRICT fControl, int* RESTRICT iZone, $1* RESTRICT fZone) {",
                     xfloat(), ifloat());
    }
    tab(n + 1, *fOut);
    fCodeProducer->Tab(n + 1);
