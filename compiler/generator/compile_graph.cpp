...
set<Tree> GraphCompiler::ExpressionsListToInstructionsSet(Tree L3) const
{
    ...
    if (gGlobal->gDebugDiagram) signalGraph(fileprefix + "-phase4b-afterTimeTransform.dot", INSTR4);
    ...
    if (gGlobal->gDebugDiagram) signalVectorGraph(fileprefix + "-phase7-vectorgraph.dot", INSTR6);
...
Scheduling GraphCompiler::schedule(const set<Tree>& I) const
{
    ...
    if (gGlobal->gCodeMode == 0) {
        ...
        // b) for the sample level graph we have (probably) cycles
        digraph<digraph<Tree, multidep>, multidep> DG = graph2dag(E);
        digraph<digraph<Tree, multidep>, multidep> RG = reverse(DG);
        ...
        vector<digraph<Tree, multidep>> SCHED;
...
string GraphCompiler::generateCode(Tree sig)
{
    ...
    /* we should not have any control at this stage*/
        ...
        stringstream error;
        error << __FILE__ << ":" << __LINE__ << " ERROR when compiling, unrecognized signal : " << ppsig(sig) << endl;
