...
bool llvm_dsp_factory_aux::initJIT(string& error_msg)
{
    ...
    if (fObjectCache) {
    ...
    #if (defined(LLVM_34) || defined(LLVM_35)) && !defined(_MSC_VER)
        builder.setUseMCJIT(true);
    #elif !defined(LLVM_36) && !defined(LLVM_37) && !defined(LLVM_38) && !defined(LLVM_39)
        ...
        */
        ...
        #if defined(LLVM_37) || defined(LLVM_38) || defined(LLVM_39) // Code taken from opt.cpp
                                                    ...
                                                    string& error_msg, int opt_level)
{
    ...
    if (getFactory(sha_key, it)) {
        ...
        llvm_dsp_factory_aux::gLLVMFactoryTable[factory] = list<llvm_dsp_aux*>();
        return factory;
    } else {
        return NULL;
    }*/
...
{
    if (dsp) {
        reinterpret_cast<llvm_dsp_aux*>(dsp)->instanceConstants(samplingRate);
    }
}
...
{
    if (dsp) {
        reinterpret_cast<llvm_dsp_aux*>(dsp)->instanceClear();
    }
}
