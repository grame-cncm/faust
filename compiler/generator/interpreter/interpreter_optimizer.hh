...
struct FIRInstructionMoveOptimizer : public FIRInstructionOptimizer<T> {
    ...
    FIRBasicInstruction<T>* rewrite(InstructionIT cur, InstructionIT& end)
    {
        ...
        if (inst1->fOpcode == FIRInstruction::kLoadReal && inst2->fOpcode == FIRInstruction::kStoreReal) {
            ...
            return new FIRBasicInstruction<T>(FIRInstruction::kMoveReal, 0, 0, inst2->fOffset1,
                                              inst1->fOffset1);  // reverse order
        } else if (inst1->fOpcode == FIRInstruction::kLoadInt && inst2->fOpcode == FIRInstruction::kStoreInt) {
            end = cur + 2;
            return new FIRBasicInstruction<T>(FIRInstruction::kMoveInt, 0, 0, inst2->fOffset1,
                                              inst1->fOffset1);  // reverse order
            ...
            return new FIRBasicInstruction<T>(FIRInstruction::kStoreRealValue, 0, inst1->fRealValue, inst2->fOffset1,
                                              0);
...
struct FIRInstructionPairMoveOptimizer : public FIRInstructionOptimizer<T> {
    ...
    FIRBasicInstruction<T>* rewrite(InstructionIT cur, InstructionIT& end)
    {
        ...
        if (inst1->fOpcode == FIRInstruction::kMoveReal && inst2->fOpcode == FIRInstruction::kMoveReal &&
        ...
        } else if (inst1->fOpcode == FIRInstruction::kMoveInt && inst2->fOpcode == FIRInstruction::kMoveInt &&
                   (inst1->fOffset1 == (inst1->fOffset2 + 1)) && (inst2->fOffset1 == (inst2->fOffset2 + 1)) &&
                   (inst2->fOffset1 == inst1->fOffset2)) {
            end = cur + 2;
            // std::cout << "FIRInstructionPairMoveOptimizer" << inst1->fOffset1 << " " << inst2->fOffset1 << std::endl;
...
struct FIRInstructionMathOptimizer : public FIRInstructionOptimizer<T> {
    ...
    FIRInstructionMathOptimizer()
    {
        ...
        for (int i = FIRInstruction::kAddReal; i <= FIRInstruction::kXORInt; i++) {
            ...
            // std::cout << gFIRInstructionTable[i + (FIRInstruction::kAddRealStack - FIRInstruction::kAddReal)] <<
            // std::endl;
        }
        ...
        for (int i = FIRInstruction::kAddReal; i <= FIRInstruction::kXORInt; i++) {
            ...
            // std::cout << gFIRInstructionTable[i + (FIRInstruction::kAddRealStackValue - FIRInstruction::kAddReal)] <<
            // std::endl;
        }
        ...
        for (int i = FIRInstruction::kAddReal; i <= FIRInstruction::kXORInt; i++) {
            ...
            // std::cout << gFIRInstructionTable[i + (FIRInstruction::kAddRealValue - FIRInstruction::kAddReal)] <<
            // std::endl;
        }
    ...
    FIRBasicInstruction<T>* rewrite(InstructionIT cur, InstructionIT& end)
    {
        ...
        if (inst1->fOpcode == FIRInstruction::kLoadReal && inst2->fOpcode == FIRInstruction::kLoadReal &&
        ...
        } else if (inst1->fOpcode == FIRInstruction::kLoadInt && inst2->fOpcode == FIRInstruction::kLoadInt &&
                   FIRInstruction::isMath(inst3->fOpcode)) {
        ...
        } else if (inst1->fOpcode == FIRInstruction::kLoadReal && inst2->fOpcode == FIRInstruction::kLoadReal &&
        ...
        } else if (inst1->fOpcode == FIRInstruction::kLoadReal && inst2->fOpcode == FIRInstruction::kRealValue &&
                   FIRInstruction::isMath(inst3->fOpcode)) {
        ...
        } else if (inst1->fOpcode == FIRInstruction::kRealValue && inst2->fOpcode == FIRInstruction::kLoadReal &&
                   FIRInstruction::isMath(inst3->fOpcode)) {
        ...
        } else if (inst1->fOpcode == FIRInstruction::kLoadInt && inst2->fOpcode == FIRInstruction::kInt32Value &&
                   FIRInstruction::isMath(inst3->fOpcode)) {
        ...
        } else if (inst1->fOpcode == FIRInstruction::kInt32Value && inst2->fOpcode == FIRInstruction::kLoadInt &&
                   FIRInstruction::isMath(inst3->fOpcode)) {
        ...
        } else if (((inst1->fOpcode == FIRInstruction::kLoadReal) || (inst1->fOpcode == FIRInstruction::kLoadInt)) &&
                   FIRInstruction::isMath(inst2->fOpcode)) {
            ...
            // kInt32Value binary OP ==> Stack/Value version
                    ...
                    (inst1->fOpcode == FIRInstruction::kInt32Value)) &&
                   FIRInstruction::isMath(inst2->fOpcode)) {
            // std::cout << "kRealValue binary op ==> Stack/Value version " << gFIRInstructionTable[inst2->fOpcode] <<
            // endl;
            end = cur + 2;
            ...
            // kLoadReal/kLoadInt binary EXTENDED-OP ==> Stack version
                   ...
                   FIRInstruction::isExtendedBinaryMath(inst2->fOpcode)) {
            // std::cout << "kLoadReal/kLoadInt binary op ==> Stack version " << gFIRInstructionTable[inst2->fOpcode] <<
            // endl;
            end = cur + 2;
...
struct FIRInstructionConstantValueHeap2Map : public FIRInstructionOptimizer<T> {
    ...
    virtual FIRBasicInstruction<T>* rewrite(InstructionIT cur, InstructionIT& end)
    {
        ...
        if (inst1->fOpcode == FIRInstruction::kRealValue && inst2->fOpcode == FIRInstruction::kStoreReal) {
            ...
            // std::cout << "kNop FIRInstruction::kRealValue && FIRInstruction::kStoreReal " << inst1->fRealValue <<
            // std::endl;
            ...
            // std::cout << "kNop FIRInstruction::kInt32Value && FIRInstruction::kStoreInt " << inst1->fIntValue <<
            // std::endl;
...
struct FIRInstructionMathSpecializer : public FIRInstructionOptimizer<T> {
    ...
    FIRBasicInstruction<T>* rewriteBinaryRealMath(FIRBasicInstruction<T>* inst1, FIRBasicInstruction<T>* inst2,
                                                  FIRBasicInstruction<T>* inst3)
    {
        switch (inst3->fOpcode) {
            ...
            case FIRInstruction::kRemReal:
                return new FIRBasicInstruction<T>(FIRInstruction::kRealValue, 0,
                                                  std::remainder(inst2->fRealValue, inst1->fRealValue));

            case FIRInstruction::kGTReal:
                return new FIRBasicInstruction<T>(FIRInstruction::kInt32Value, inst2->fRealValue > inst1->fRealValue,
                                                  0);
    ...
    FIRBasicInstruction<T>* rewriteBinaryRealMath2(FIRBasicInstruction<T>* inst1, FIRBasicInstruction<T>* inst2,
                                                   FIRBasicInstruction<T>* inst3)
    {
    ...
    FIRBasicInstruction<T>* rewriteBinaryRealMath3(FIRBasicInstruction<T>* inst1, FIRBasicInstruction<T>* inst2,
                                                   FIRBasicInstruction<T>* inst3)
    {
        switch (inst3->fOpcode) {
            case FIRInstruction::kAddReal:
    ...
    FIRBasicInstruction<T>* rewriteBinaryIntMath(FIRBasicInstruction<T>* inst1, FIRBasicInstruction<T>* inst2,
                                                 FIRBasicInstruction<T>* inst3)
    {
        switch (inst3->fOpcode) {
    ...
    FIRBasicInstruction<T>* rewriteBinaryIntMath2(FIRBasicInstruction<T>* inst1, FIRBasicInstruction<T>* inst2,
                                                  FIRBasicInstruction<T>* inst3)
    {
        switch (inst3->fOpcode) {
            case FIRInstruction::kAddInt:
    ...
    FIRBasicInstruction<T>* rewriteBinaryIntMath3(FIRBasicInstruction<T>* inst1, FIRBasicInstruction<T>* inst2,
                                                  FIRBasicInstruction<T>* inst3)
    {
        switch (inst3->fOpcode) {
            case FIRInstruction::kAddInt:
    ...
    FIRBasicInstruction<T>* rewriteExtendedBinaryRealMath(FIRBasicInstruction<T>* inst1, FIRBasicInstruction<T>* inst2,
                                                          FIRBasicInstruction<T>* inst3)
    {
        switch (inst3->fOpcode) {
    ...
    FIRBasicInstruction<T>* rewriteExtendedBinaryIntMath(FIRBasicInstruction<T>* inst1, FIRBasicInstruction<T>* inst2,
                                                         FIRBasicInstruction<T>* inst3)
    {
        switch (inst3->fOpcode) {
            case FIRInstruction::kMax:
                return new FIRBasicInstruction<T>(FIRInstruction::kInt32Value,
    ...
    FIRBasicInstruction<T>* rewriteUnaryRealMath(FIRBasicInstruction<T>* inst1, FIRBasicInstruction<T>* inst2)
    {
    ...
    FIRBasicInstruction<T>* rewriteUnaryIntMath(FIRBasicInstruction<T>* inst1, FIRBasicInstruction<T>* inst2)
    {
        switch (inst2->fOpcode) {
    ...
    virtual FIRBasicInstruction<T>* rewrite(InstructionIT cur, InstructionIT& end)
    {
        ...
        // Real
        if (inst1->fOpcode == FIRInstruction::kRealValue && inst2->fOpcode == FIRInstruction::kRealValue &&
            FIRInstruction::isMath(inst3->fOpcode)) {
            end = cur + 3;
            ...
            // Int
        } else if (inst1->fOpcode == FIRInstruction::kInt32Value && inst2->fOpcode == FIRInstruction::kInt32Value &&
                   FIRInstruction::isMath(inst3->fOpcode)) {
            end = cur + 3;
        ...
        } else if (inst1->fOpcode == FIRInstruction::kRealValue && inst2->fOpcode == FIRInstruction::kRealValue &&
                   FIRInstruction::isExtendedBinaryMath(inst3->fOpcode)) {
            end = cur + 3;
...
struct FIRInstructionOptimizer {
    ...
    virtual FIRBasicInstruction<T>* rewrite(InstructionIT cur, InstructionIT& end) { return nullptr; }

    // Return an optimized block by traversing it (including sub-blocks) with an 'optimizer'
    static FIRBlockInstruction<T>* optimize_aux(FIRBlockInstruction<T>*     cur_block,
                                                FIRInstructionOptimizer<T>& optimizer)
    {
        ...
        do {
            ...
            if (inst1->fOpcode == FIRInstruction::kInt32Value && FIRInstruction::isChoice(inst2->fOpcode)) {
                ...
                new_block->push(new FIRBasicInstruction<T>(
    ...
    static FIRBlockInstruction<T>* specialize2Heap(FIRBlockInstruction<T>* cur_block, std::map<int, int>& int_map,
    ...
    // Specialize a block
    static FIRBlockInstruction<T>* specialize(FIRBlockInstruction<T>* cur_block, std::map<int, int>& int_map,
                                              std::map<int, T>& real_map)
    {
