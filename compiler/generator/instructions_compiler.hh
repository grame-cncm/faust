...
class InstructionsCompiler : public virtual Garbageable {
    ...
    */
    ...
    virtual StatementInst* generateInitArray(const std::string& vname, Typed::VarType ctype,
                                             int delay);
    virtual StatementInst* generateCopyArray(const std::string& vname, int index_from,
                                             int index_to);
    ...
    ValueInst* generateSliderAux(Tree sig, Tree path, Tree cur, Tree min, Tree max, Tree step,
                                 const std::string& name);
    ValueInst* generateBargraphAux(Tree sig, Tree path, Tree min, Tree max, ValueInst* exp,
                                   const std::string& name);
    ...
    {
        return fContainer->pushUserInterfaceMethod(inst);
    }
    ...
    {
        return fContainer->pushExtGlobalDeclare(inst);
    }

    StatementInst* pushPreComputeDSPMethod(StatementInst* inst)
    {
        return fContainer->pushPreComputeDSPMethod(inst);
    }
    StatementInst* pushComputeDSPMethod(StatementInst* inst)
    {
        return fContainer->pushComputeDSPMethod(inst);
    }
    StatementInst* pushPostComputeDSPMethod(StatementInst* inst)
    {
        return fContainer->pushPostComputeDSPMethod(inst);
    }
   ...
   public:
    ...
    virtual ValueInst* generateVSlider(Tree sig, Tree label, Tree cur, Tree min, Tree max,
                                       Tree step);
    virtual ValueInst* generateHSlider(Tree sig, Tree label, Tree cur, Tree min, Tree max,
                                       Tree step);
    virtual ValueInst* generateNumEntry(Tree sig, Tree label, Tree cur, Tree min, Tree max,
                                        Tree step);
    ...
    virtual ValueInst* generateFConst(Tree sig, Tree type, const std::string& file,
                                      const std::string& name);
    virtual ValueInst* generateFVar(Tree sig, Tree type, const std::string& file,
                                    const std::string& name);

    virtual ValueInst* generateDelayVec(Tree sig, ValueInst* exp, Typed::VarType ctype,
                                        const std::string& vname, int mxd);
    virtual ValueInst* generateDelayLine(ValueInst* exp, Typed::VarType ctype,
                                         const std::string& vname, int mxd,
