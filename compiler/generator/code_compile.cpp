...
CodeCompiler::CodeCompiler(const string& name, const string& super, int numInputs, int numOutputs, bool vec)
		: fCodegen(new CodeGen(name, super, numInputs, numOutputs, vec)),
		  fNeedToDeleteClass(true), 
		  fUIRoot(uiFolder(cons(tree(0), tree(subst("$0", gMasterName))))),
		  fDescription(0)
{}

CodeCompiler::CodeCompiler(CodeGen* k)
		: fCodegen(k),
...
{ 
	if (fNeedToDeleteClass) delete fCodegen;
}
 ...
 */
void CodeCompiler::addUIWidget(Tree path, Tree widget)
{
 ...
 */
Tree CodeCompiler::prepareUserInterfaceTree(Tree t)
{
	Tree root, elems;
 ...
 */
void CodeCompiler::generateUserInterfaceTree(Tree t)
{
 ...
 */
void CodeCompiler::generateUserInterfaceElements(Tree elements)
{
 ...
 */
void CodeCompiler::generateWidgetCode(Tree fulllabel, Tree varname, Tree sig)
{
	Tree path, c, x, y, z;
    string label;
    ...
    for (map<string, set<string> >::iterator i = metadata.begin(); i != metadata.end(); i++) {
        ...
        for (set<string>::const_iterator j = values.begin(); j != values.end(); j++) {
            fCodegen->addMetaDeclare(tree2str(varname), wdel(key), wdel(*j));
        }
    }

	if ( isSigButton(sig, path) ) 					{
        fCodegen->addButton(label, tree2str(varname));

	} else if ( isSigCheckbox(sig, path) ) 			{
    	fCodegen->addCheckButton(label, tree2str(varname));

	} else if ( isSigVSlider(sig, path,c,x,y,z) )	{
    	fCodegen->addVerticalSlider(label, tree2str(varname), tree2float(c), tree2float(x), tree2float(y), tree2float(z));

	} else if ( isSigHSlider(sig, path,c,x,y,z) )	{
        fCodegen->addHorizontalSlider(label, tree2str(varname), tree2float(c), tree2float(x), tree2float(y), tree2float(z));

	} else if ( isSigNumEntry(sig, path,c,x,y,z) )	{
        fCodegen->addNumEntry(label, tree2str(varname), tree2float(c), tree2float(x), tree2float(y), tree2float(z));

	} else if ( isSigVBargraph(sig, path,x,y,z) )	{
        fCodegen->addHorizontalBargraph(label, tree2str(varname), tree2float(x), tree2float(y));

	} else if ( isSigHBargraph(sig, path,x,y,z) )	{
 		fCodegen->addVerticalBargraph(label, tree2str(varname), tree2float(x), tree2float(y));
 ...
 * to user interface element t
 */
void CodeCompiler::generateMacroInterfaceTree(const string& pathname, Tree t)
{
	Tree 	label, elements, varname, sig;
 ...
 */
void CodeCompiler::generateMacroInterfaceElements(const string& pathname, Tree elements)
{
	while (!isNil(elements)) {
 ...
 */
void CodeCompiler::generateWidgetMacro(const string& pathname, Tree fulllabel, Tree varname, Tree sig)
{
	Tree path, c, x, y, z;
    string label;
    map<string, set<string> >   metadata;
	...
	if ( isSigButton(sig, path) ) 					{
		fCodegen->addUIMacro(subst("FAUST_ADDBUTTON(\"$0\", $1);", pathlabel, tree2str(varname)));

	} else if ( isSigCheckbox(sig, path) ) 			{
		fCodegen->addUIMacro(subst("FAUST_ADDCHECKBOX(\"$0\", $1);", pathlabel, tree2str(varname)));

	} else if ( isSigVSlider(sig, path,c,x,y,z) )	{
		fCodegen->addUIMacro(subst("FAUST_ADDVERTICALSLIDER(\"$0\", $1, $2, $3, $4, $5);",
				pathlabel,
				tree2str(varname),
				T(tree2float(c)),
				T(tree2float(x)),
		...
		fCodegen->addUIMacro(subst("FAUST_ADDHORIZONTALSLIDER(\"$0\", $1, $2, $3, $4, $5);",
				pathlabel,
				tree2str(varname),
				T(tree2float(c)),
				T(tree2float(x)),
		...
		fCodegen->addUIMacro(subst("FAUST_ADDNUMENTRY(\"$0\", $1, $2, $3, $4, $5);",
				pathlabel,
				tree2str(varname),
				T(tree2float(c)),
				T(tree2float(x)),
		...
		fCodegen->addUIMacro(subst("FAUST_ADDVERTICALBARGRAPH(\"$0\", $1, $2, $3);",
				pathlabel,
				tree2str(varname),
				T(tree2float(x)),
				T(tree2float(y))));
		...
		fCodegen->addUIMacro(subst("FAUST_ADDHORIZONTALBARGRAPH(\"$0\", $1, $2, $3);",
				pathlabel,
				tree2str(varname),
				T(tree2float(x)),
				T(tree2float(y))));
