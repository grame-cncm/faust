...
- `.clangd`: configuration used to silence some Clangd LSP errors.
  - It defines the `MOJO_BUILD=1` preprocessor macro for Clangd.

### Configuration files

These are the files prefixed with `__`. They contain private code outside the classes directly used by the
other compiler components (`faust/compiler` and `faust/compiler/generator`).

- `__mojo_hal.hh`: abstraction layer for the underlying hardware (`Hardware Abstraction Layer`).
- `__mojo_macro.hh`: macros used to improve the ergonomics and semantic clarity of the public code.
- `__mojo_utils.hh`: helper functions for the main public classes.
...
- `mojo_instructions.hh` - Declares `MojoInstVisitor` and its derived classes `MojoVecInstVisitor`,
  `MojoGpuInstVisitor` and `MojoInitFieldsVisitor`.
- `mojo_code_container.hh` - Declares `MojoCodeContainer` and its derived classes
  `MojoScalarCodeContainer`, `MojoVecCodeContainer` and `MojoGpuCodeContainer`.

- `mojo_instructions.cpp` - Implements `MojoInstVisitor` and `MojoInitFieldsVisitor`.
- `mojo_instructions_vec.cpp` - Implements `MojoVecInstVisitor`.
- `mojo_instructions_gpu.cpp` - Implements `MojoGpuInstVisitor`.
- `mojo_code_container.cpp` - Implements `MojoCodeContainer` and its scalar, vector and GPU variants.
...
- a comment banner with the compilation options;
...
`MojoCodeContainer` is an abstract class from which `MojoScalarCodeContainer`, `MojoVecCodeContainer` and
`MojoGpuCodeContainer` derive. The `createContainer` factory selects the GPU container for `-gpu`, the
vector container for `-vec` and the scalar container otherwise. All three share the general structure of
the DSP `struct` and specialize computation through `writeCompute`.
...
- `gCurGraph` identifies a bargraph whose values are temporarily stored in an array.
...
The `-gpu` option selects the Mojo GPU container. FAUST prepares a dependency graph of DSP subloops; the
GPU backend turns its ordered levels into device tasks. GPU generation shares the class layout, lifecycle,
metadata and user interface writers with the scalar backend. The architecture supplies the Mojo imports
and composes the generated DSP with its GPU device, audio driver and GUI.

#### GPU Code Container

`MojoGpuCodeContainer` inherits `produceClass` and `createContainer`. Its `writeSupport` sorts the loop DAG,
expands the loop controls, prepares task stages and emits a `mydspWork` structure for device scratch state.
It delegates field initialization and GPU method generation to `MojoGpuInstVisitor` through
`writeFieldInitializers` and `writeCompute`. The generated DSP implements `FaustDspGpu`; the architecture
connects it to the ordinary audio interface through `AdapterDsp` and `GpuDevice`.

#### GPU Instructions Visitor

`MojoGpuInstVisitor` generates `gpu_controls` for block controls, `gpu_stage_0` and subsequent kernels for
ordered task stages, and `gpu_post` when there is post-compute work. It groups tasks that do not conflict
over shared inputs, outputs, DSP fields or work state. Tasks within a stage can run on separate GPU threads;
each task still processes its samples in order, including recursive state updates. No separate compiler
option currently selects parallelism by voice, oscillator or frequency band.

Temporary fields shared by tasks reside in `mydspWork`. DSP fields must have device-copyable scalar types
or fixed-length inline arrays. Pointer views into those arrays are rebuilt inside each kernel. The visitor
reports unsupported device functions and state layouts during generation.

#### Precision and execution

GPU generation requires `-single` (`f32` internal computation). `FaustFloat` remains the type of UI zones
and audio samples, as defined by the architecture's `DFAUST` compile-time setting. The current GPU
architecture also requires 32-bit external samples. Thus `-single` and `DFAUST=f32` describe two separate
precision choices; the GPU path needs both to be 32 bit. The `-vec` SIMD path still requires `-double`.

The generated `gpu_work_size` returns the size of `mydspWork`; `gpu_compute` enqueues the controls once,
then launches the ordered stages in chunks of at most `-vs` frames, and finally enqueues the optional
post-compute kernel. The architecture owns device buffers, UI-zone transfers and the audio callback.
`-gpu` is available only with `-lang mojo`; this generator does not accept GPU mode together with OpenCL,
CUDA, OpenMP, the scheduler or unsupported execution options.

For example, from `architecture/mojo`:

```
    $FAUST_BIN -single -lang mojo -gpu \
        -a portaudio-terminal-gpu.mojo \
        -o $NAME.mojo src/$NAME.dsp
```

The architecture [README](../../../architecture/mojo/README.md) documents the separate Mojo build and
PortAudio/terminal GUI linking steps.
