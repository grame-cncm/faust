...
ValueInst* DAGInstructionsCompiler::CS(Tree sig)
{
    ...
    if (!getCompiledExpression(sig, code)) {
        ...
        } else if (isSigDelay(sig, x, d) && isProj(x, &i, r) &&
                   fContainer->getLoopProperty(r, ls)) {
...
ValueInst* DAGInstructionsCompiler::generateDelayLine(ValueInst* exp, Typed::VarType ctype,
                                                      const string& vname, int mxd,
                                                      Address::AccessType& access,
                                                      ValueInst*           unused)
{
...
void DAGInstructionsCompiler::generateVectorLoop(Typed::VarType ctype, const string& vname,
                                                 ValueInst* exp, Address::AccessType& access)
{
    // "$0 $1[$2];"
...
void DAGInstructionsCompiler::generateDlineLoop(Typed::VarType ctype, const string& vname,
                                                int delay, ValueInst* exp,
                                                Address::AccessType& access)
{
    ...
    if (delay < gGlobal->gMaxCopyDelay) {
        ...
        // Implementation of a ring-buffer delayline, the size should be large enough and aligned on
        // a power of two
        ...
        pushPostComputeDSPMethod(
            InstBuilder::genStoreStructVar(idx_save, InstBuilder::genLoadLoopVar("vsize")));
...
StatementInst* DAGInstructionsCompiler::generateCopyBackArray(const string& vname_to,
...
ValueInst* DAGInstructionsCompiler::generateWaveform(Tree sig)
{
    ...
    string   idx = subst("$0_idx", vname);
    FIRIndex index1 =
        (FIRIndex(InstBuilder::genLoadStructVar(idx)) + InstBuilder::genLoadLoopVar("vsize")) %
        size;
