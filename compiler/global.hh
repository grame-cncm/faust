...
struct global {
    ...
    bool        gDrawHierarchy;   // -hg option : the loop-split blocks as a hierarchy, in dot format
    ...
    int  gFamilyAlign;  // -fam-align : alignment in bytes of the family's arrays (default 0 = none ; a contract checked by the generated code)
    ...
    int  gLSSchedRegisters; // -ls-sched-R option: register budget of the EMISSION scheduler alone
                            // (the fusion oracle keeps -ls-R) ; -1 = same as -ls-R
    ...
    std::map<Tree, std::pair<int, std::string>, treeorder> gBoxTable;
    int                                         gBoxCounter;
    ...
    Tree                        gInverter[6];
    std::map<Tree, std::string, treeorder> gBackLink;  // link to enclosing file for sub schema
