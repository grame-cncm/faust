...
static int compareRecProj(Tree x, Tree y)
{
    ...
    TLIB_ASSERT(isProj(x, &ix, rx) && isProj(y, &iy, ry));
...
Tree proj2SigIIR(Tree rt, Tree sig)
{
    ...
    TLIB_ASSERT(isProj(rt, &ti, tg));
    TLIB_ASSERT(isProj(sig, &si, sg));
...
Tree delaySigIIR_real(Tree rt, Tree x, Tree y)
{
    ...
    if (cy) {
        return ::nil();  // a signal delay by a concerned IIR can't be expressed has an IIR
    }
    if (cx) {
        if (int d; isSigNum(y, d)) {
            return delayIIRCoef(coef1, d);  // we can delay the IIR
        } else {
            return ::nil();  // can't be expressed as an IIR
        }
    }
 ...
 * @return    The transformed signal, or ::nil() if the transformation is not possible.
 */
Tree embeddedIIR(Tree rt, Tree fir)
{
    tvec cfir, ciir;
    TLIB_ASSERT(isSigFIR(fir, cfir) && isSigIIR(cfir[0], ciir) && (ciir[0] == rt));
