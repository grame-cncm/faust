...
static schema* generateInsideSchema(Tree t)
{
    ...
    if (xt) {
        ...
        return makeBlockSchema(1, 1, "mem", normalcolor, "");
    } else if (isBoxDelay(t)) {
        return makeBlockSchema(2, 1, "@", normalcolor, "");
    }
    ...
    // don't draw group rectangle when labels are empty (ie "")
        ...
        schema* rr = generateDiagramSchema(a);
        return makeDecorateSchema(rr, 10, s.str());
