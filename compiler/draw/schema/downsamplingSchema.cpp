...
const double downsamplingSchema::fTopMargin(
...
{
    return new downsamplingSchema(s);
}
 ...
 */
downsamplingSchema::downsamplingSchema(schema* s)
 ...
 */
void downsamplingSchema::place(double ox, double oy, int orientation)
{
 ...
 */
point downsamplingSchema::inputPoint(unsigned int i) const
{
    faustassert(placed());
 ...
 */
point downsamplingSchema::outputPoint(unsigned int i) const
{
    faustassert(placed());
 ...
 */
void downsamplingSchema::draw(device& dev)
{
    faustassert(placed());
    ...
    // draw the downsampling label
    // dev.label(tl, y0, fText.c_str());  //
    // draw text
    if (orientation() == kLeftRight) {
        dev.label(x0 + (width() - tw) / 2, y0 + 5, "downsampling");
    } else {
        dev.label(x0 + (width() - tw) / 2, y1 - 5, "downsampling");
    }
    ...
    if (orientation() == kLeftRight) {
        dev.label(p0.x + 2, p0.y, "rate");  //
    } else {
        dev.label(p0.x - 16 - dx, p0.y, "rate");  //
    }
}
 ...
 * be called after the block have been placed
 */
void downsamplingSchema::collectTraits(collector& c)
{
    faustassert(placed());
