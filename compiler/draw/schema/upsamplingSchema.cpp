...
const double upsamplingSchema::fTopMargin(
 ...
 */
upsamplingSchema::upsamplingSchema(schema* s)
 ...
 */
void upsamplingSchema::place(double ox, double oy, int orientation)
{
 ...
 */
point upsamplingSchema::inputPoint(unsigned int i) const
{
    faustassert(placed());
 ...
 */
point upsamplingSchema::outputPoint(unsigned int i) const
{
    faustassert(placed());
 ...
 */
void upsamplingSchema::draw(device& dev)
{
    faustassert(placed());
    ...
    // draw the upsampling label
    // dev.label(tl, y0, fText.c_str());  //
    // draw text
    if (orientation() == kLeftRight) {
        dev.label(x0 + (width() - tw) / 2, y0 + 5, "upsampling");
    } else {
        dev.label(x0 + (width() - tw) / 2, y1 - 5, "upsampling");
    }
 ...
 * be called after the block have been placed
 */
void upsamplingSchema::collectTraits(collector& c)
{
    faustassert(placed());
