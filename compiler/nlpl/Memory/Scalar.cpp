...
class ScalarMemory : public MemoryZone
{
   ...
   public:
    explicit ScalarMemory(DeclareMode decl, std::string typ, std::string name, unsigned int phase)
        : fDecl(decl), fType(std::move(typ)), fName(std::move(name)), fPhase(phase)
    {
    }
