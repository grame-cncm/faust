...
class WriteMemInstr : public Instruction {
   ...
   public:
    explicit WriteMemInstr(Mem mem, int mode, Expr expr) : fMem(mem), fMode(mode), fExpr(expr) {}
    ...
    void print(std::ostream& os, int indent) override { tab(os, indent) << fMem << ' ' << WriteModeOp(fMode) << ' ' << fExpr << ';'; }
...
{
    static std::unordered_map<std::tuple<Mem, int, Expr>, Instr> gWriteMemHash;
    std::tuple<Mem, int, Expr>                                   key(mem, mode, expr);
