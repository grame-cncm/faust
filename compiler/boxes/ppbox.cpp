...
const char* prim2name(Tree (*ptr)(Tree, Tree))
{
    ...
    if (ptr == sigARightShift) {
        return ">>";
    }
    if (ptr == sigLRightShift) {
        return ">>>";
    }
...
static void streambinop(ostream& fout, Tree t1, const char* op, Tree t2, int curPriority,
                        int upPriority)
{
...
static void streambinopShared(ostream& fout, Tree t1, const char* op, Tree t2, int curPriority,
                              int upPriority)
{
    if (upPriority > curPriority) {
        fout << '(';
...
ostream& boxpp::print(ostream& fout) const
{
    ...
    Tree t1, t2, t3, ff, label, cur, min, max, step, type, name, file, arg, body, fun, args, abstr,
        genv, vis, lenv, ldef, slot, ident, rules, chan, ins, outs, lroutes;
    ...
    // Foreign elements
        ...
        fout << "fconstant(" << type2str(tree2int(type)) << ' ' << tree2str(name) << ", "
             << tree2str(file) << ')';
    } else if (isBoxFVar(fBox, type, name, file)) {
        fout << "fvariable(" << type2str(tree2int(type)) << ' ' << tree2str(name) << ", "
             << tree2str(file) << ')';
    }
    ...
    // User interface
        ...
        fout << "vslider(" << tree2quotedstr(label) << ", " << boxpp(cur) << ", " << boxpp(min)
             << ", " << boxpp(max) << ", " << boxpp(step) << ')';
    } else if (isBoxHSlider(fBox, label, cur, min, max, step)) {
        fout << "hslider(" << tree2quotedstr(label) << ", " << boxpp(cur) << ", " << boxpp(min)
             << ", " << boxpp(max) << ", " << boxpp(step) << ')';
        ...
        fout << "nentry(" << tree2quotedstr(label) << ", " << boxpp(cur) << ", " << boxpp(min)
             << ", " << boxpp(max) << ", " << boxpp(step) << ')';
        ...
        */
        ...
        fout << "closure[" << boxpp(abstr) << ", genv = " << envpp(genv)
             << ", lenv = " << envpp(lenv) << "]";
...
#define BOX_INSERT_ID(exp)                                                                  \
    if (gGlobal->gBoxTable.find(fBox) == gGlobal->gBoxTable.end()) {                        \
        ...
        gGlobal->gBoxTrace.push_back("ID_" + std::to_string(gGlobal->gBoxCounter) + " = " + \
                                     s.str() + ";\n");                                      \
        gGlobal->gBoxCounter++;                                                             \
    }                                                                                       \
...
ostream& boxppShared::print(ostream& fout) const
{
    ...
    if (xt) {
        ...
        // BoxAbstr cannot be safely expanded with 'boxppShared' since 'arg' used in 'body' will be
        // somewhat free
        fout << "\\" << boxpp(arg) << ".(" << boxpp(body) << ")";
    } else if (isBoxAppl(fBox, fun, args)) {
        ...
        BOX_INSERT_ID(s << "fconstant(" << type2str(tree2int(type)) << ' ' << tree2str(name) << ", "
                        << tree2str(file) << ')');
    } else if (isBoxFVar(fBox, type, name, file)) {
        BOX_INSERT_ID(s << "fvariable(" << type2str(tree2int(type)) << ' ' << tree2str(name) << ", "
                        << tree2str(file) << ')');
        // Block diagram binary operator
    } else if (isBoxSeq(fBox, t1, t2)) {
        ...
        BOX_INSERT_ID(s << "par(" << boxppShared(t1) << ", " << boxppShared(t2) << ") {"
                        << boxppShared(t3) << "}");
    } else if (isBoxISeq(fBox, t1, t2, t3)) {
        BOX_INSERT_ID(s << "seq(" << boxppShared(t1) << ", " << boxppShared(t2) << ") {"
                        << boxppShared(t3) << "}");
    } else if (isBoxISum(fBox, t1, t2, t3)) {
        BOX_INSERT_ID(s << "sum(" << boxppShared(t1) << ", " << boxppShared(t2) << ") {"
                        << boxppShared(t3) << "}");
    } else if (isBoxIProd(fBox, t1, t2, t3)) {
        ...
        BOX_INSERT_ID(s << "vslider(" << tree2quotedstr(label) << ", " << boxppShared(cur) << ", "
                        << boxppShared(min) << ", " << boxppShared(max) << ", " << boxppShared(step)
                        << ')');
    } else if (isBoxHSlider(fBox, label, cur, min, max, step)) {
        BOX_INSERT_ID(s << "hslider(" << tree2quotedstr(label) << ", " << boxppShared(cur) << ", "
                        << boxppShared(min) << ", " << boxppShared(max) << ", " << boxppShared(step)
                        << ')');
    } else if (isBoxVGroup(fBox, label, t1)) {
        ...
        BOX_INSERT_ID(s << "hbargraph(" << tree2quotedstr(label) << ", " << boxppShared(min) << ", "
                        << boxppShared(max) << ')');
    } else if (isBoxMetadata(fBox, t1, t2)) {
        BOX_INSERT_ID(s << boxppShared(t1, 0) << "/* md */");
    } else if (isBoxVBargraph(fBox, label, min, max)) {
        BOX_INSERT_ID(s << "vbargraph(" << tree2quotedstr(label) << ", " << boxppShared(min) << ", "
                        << boxppShared(max) << ')');
    } else if (isBoxNumEntry(fBox, label, cur, min, max, step)) {
        BOX_INSERT_ID(s << "nentry(" << tree2quotedstr(label) << ", " << boxppShared(cur) << ", "
                        << boxppShared(min) << ", " << boxppShared(max) << ", " << boxppShared(step)
                        << ')');
    } else if (isBoxSoundfile(fBox, label, chan)) {
        ...
        BOX_INSERT_ID(s << "closure[" << boxppShared(abstr) << ", genv = " << envpp(genv)
                        << ", lenv = " << envpp(lenv) << "]");
    } else if (isBoxComponent(fBox, label)) {
        fout << "component(" << tree2quotedstr(label) << ')';
    } else if (isBoxAccess(fBox, t1, t2)) {
        ...
        // BoxSymbolic cannot be safely expanded with 'boxppShared' since 'slot' used in 'body' will
        // be somewhat free
        fout << "\\(" << boxpp(slot) << ").(" << boxpp(body) << ")";
    }
    ...
    // Beautify messages involving lhs patterns
        ...
        BOX_INSERT_ID(s << "route(" << boxppShared(ins) << "," << boxppShared(outs) << ","
                        << boxppShared(lroutes) << ")");
    }
