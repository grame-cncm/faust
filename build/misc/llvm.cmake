...
function (scan_backends TARGET FLAG)
    ...
    string (FIND "${NNX_BACKEND}" ${FLAG} POS)
    if (${POS} GREATER -1)
        backend (NNX nnx ${TARGET})
    endif()
    string (FIND "${LINEN_BACKEND}" ${FLAG} POS)
    if (${POS} GREATER -1)
        backend (LINEN linen ${TARGET})
    endif()
    string (FIND "${JULIA_BACKEND}" ${FLAG} POS)
    if (${POS} GREATER -1)
        backend (JULIA julia ${TARGET})
    endif()
    string (FIND "${MOJO_BACKEND}" ${FLAG} POS)
    if (${POS} GREATER -1)
        backend (MOJO mojo ${TARGET})
    endif()
