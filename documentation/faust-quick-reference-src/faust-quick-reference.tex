...
Every user interface widget has a label (a string) that identifies it and informs the user of its purpose. There are three important meccanisms associated with labels : \textit{variable parts}, \textit{pathnames} and \textit{metadata}.
...
\paragraph{Pathnames.}
Thanks to horizontal, vertical and tabs groups, user interfaces have a hierarchical structure analog to a hierarchical file system. Each widget has an associated \textit{pathname} obtained by concatenating the labels of all its surounding groups with its own label.
...
In order to give more flexibility to the design of user interfaces, it is possible to explicitely specify the absolute or relative pathname of a widget directly in its label. 
...
\paragraph{Metadata}
Widget labels can contain metadata enclosed in square brakets. These metadata associate a key with a value and are used to provide additional information to the architecture file.  They are tipically used to improve the look and feel of the user interface. 
The Faust code :
\begin{lstlisting}
...
\texttt{-mdlang l}			& \texttt{--mathdoc-lang l} 		& choose the language of the mathematical description (l = en, fr, ...) \\
...
Modern C++ compilers are able to do autovectorization, that is to use SIMD instructions to speedup the code. These instructions can typically operate in parallel on short vectors of 4 simple precision floating point numbers thus leading to a theoretical speedup of x4. 
...
\item Variables external to the parallel region are shared by default. The \lstinline!firstprivate(fRec0,fRec1)! clause indicates that each thread should have its private copy of fRec0 and fRec1. The reason is that accessing shared variables requires an indirection and is quite inefficient compared to private copies.
...
Many persons are contributing to the Faust project by providing code for the compiler, architecture files, libraries, examples, documentation, scripts, bug reports, ideas, etc. I would like in particular to thanks :
...
\begin{itemize}
\item[-]the French Ministery of Culture
