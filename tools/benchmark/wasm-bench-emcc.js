...
// Standard Faust DSP
faust.mydsp = function (instance, memory, buffer_size, sample_rate) {
    ...
    // mydsp is placed first with index 0. Audio buffer start at the end of mydsp.
    ...
    // Start of mydsp memory : mydsp is placed first with index 0
    var dsp = 0;
    ...
    function update_outputs ()
    {
        if (outputs_items.length > 0 && output_handler && outputs_timer-- === 0) {
            ...
            for (var i = 0; i < outputs_items.length; i++) {
                output_handler(outputs_items[i], factory._getParamValuemydsp(dsp, pathTable[outputs_items[i]]));
            }
        }
    }
    ...
    function computeAux (inputs, outputs)
    {
        ...
        // Compute
        factory._computemydsp(dsp, buffer_size, ins, outs);
    ...
    function init ()
    {
        ...
        // Init mydsp
        factory._initmydsp(dsp, sample_rate);
    }
    ...
    return {
        ...
        {
            return factory._getSampleRatemydsp(dsp);
        },
        ...
        {
            factory._instanceInitmydsp(dsp, sample_rate);
        },
        
        instanceConstants : function (sample_rate)
        {
            factory._instanceConstantsmydsp(dsp, sample_rate);
        },
        
        instanceResetUserInterface : function ()
        {
            factory._instanceResetUserInterfacemydsp(dsp);
        },
        
        instanceClear : function ()
        {
            factory._instanceClearmydsp(dsp);
        },
        ...
        {
            factory._setParamValuemydsp(dsp, pathTable[path], val);
        },
        
        getParamValue : function (path)
        {
            return factory._getParamValuemydsp(dsp, pathTable[path]);
        },
        ...
        checkDefaults : function ()
        {
            for (var i = 0; i < default_values.length; i++) {
                if (default_values[i] !== factory._getParamValuemydsp(dsp, pathTable[inputs_items[i]])) return false;
            }
            return true;
        },
        ...
        {
            for (var i = 0; i < default_values.length; i++) {
                factory._setParamValuemydsp(dsp, pathTable[inputs_items[i]], 0.123456789);
            }
        }
    };
};
...
{
    // Creates DSP
    var DSP = faust.mydsp(instance, memory, buffer_size, sample_rate);
