//-----------------------------------------------------
// name: "expander -- expander/noise gate unit"
// author: "Albert Graef"
// version: "1.0"
//
...
class mydsp : public dsp {
  ...
  public:
	static void metadata(Meta* m) 	{ 
		m->declare("name", "expander -- expander/noise gate unit");
	...
	virtual void buildUserInterface(UI* interface) {
		...
		interface->openHorizontalBox("1-expander");
		interface->addNumEntry("knee", &fentry1, 3.0f, 0.0f, 2e+01f, 0.1f);
		interface->addNumEntry("ratio", &fentry2, 2.0f, 1.0f, 2e+01f, 0.1f);
		interface->addNumEntry("threshold", &fentry0, -4e+01f, -96.0f, 1e+01f, 0.1f);
		interface->closeBox();
		interface->openVerticalBox("2-envelop");
		interface->addHorizontalSlider("attack", &fslider1, 0.001f, 0.0f, 1.0f, 0.001f);
		interface->addHorizontalSlider("release", &fslider0, 0.1f, 0.0f, 1e+01f, 0.01f);
		interface->closeBox();
	...
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		float 	fSlow4 = (1.0f / (0.001f + fSlow2));
		...
		for (int i=0; i<count; i++) {
			...
			fRec1[0] = ((fConst1 * fRec1[1]) + (fConst2 * fabsf(fTemp0)));
			float fTemp1 = (float)input1[i];
			fRec2[0] = ((fConst1 * fRec2[1]) + (fConst2 * fabsf(fTemp1)));
			float fTemp2 = max(fRec1[0], fRec2[0]);
			float fTemp3 = ((fSlow1 * (fRec0[1] < fTemp2)) + (fSlow0 * (fRec0[1] >= fTemp2)));
			fRec0[0] = ((fRec0[1] * fTemp3) + (fTemp2 * (0 - (fTemp3 - 1))));
			float fTemp4 = max((float)0, (fSlow3 - (20 * log10f(fRec0[0]))));
			float fTemp5 = (fTemp4 * (0 - (fSlow5 * min((float)1, max((float)0, (fSlow4 * fTemp4))))));
			fbargraph0 = fTemp5;
			float fTemp6 = powf(10,(0.05f * fTemp5));
			output0[i] = (FAUSTFLOAT)(fTemp0 * fTemp6);
			output1[i] = (FAUSTFLOAT)(fTemp1 * fTemp6);
			// post processing
			fRec0[1] = fRec0[0];
			fRec2[1] = fRec2[0];
			fRec1[1] = fRec1[0];
