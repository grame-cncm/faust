...
class mydsp : public dsp {
  ...
  public:
	...
	virtual void buildUserInterface(UI* interface) {
		interface->openVerticalBox("dist");
		interface->addHorizontalSlider("distortion", &fslider0, 12.0f, 0.0f, 1e+02f, 0.1f);
		interface->addHorizontalSlider("gain", &fslider1, 3.0f, -96.0f, 96.0f, 0.1f);
		interface->closeBox();
	}
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		float 	fSlow0 = float(fslider0);
		float 	fSlow1 = powf(10,(0.05f * fSlow0));
		float 	fSlow2 = (fSlow1 - 1);
		float 	fSlow3 = (powf(10,(0.05f * float(fslider1))) / sqrtf((1 + fSlow0)));
		...
		for (int i=0; i<count; i++) {
			...
			output0[i] = (FAUSTFLOAT)(fSlow3 * ((fTemp0 * (fTemp1 + fSlow1)) / (1 + (faustpower<2>(fTemp0) + (fSlow2 * fTemp1)))));
			float fTemp2 = (float)input1[i];
			float fTemp3 = fabsf(fTemp2);
			output1[i] = (FAUSTFLOAT)(fSlow3 * ((fTemp2 * (fTemp3 + fSlow1)) / (1 + (faustpower<2>(fTemp2) + (fSlow2 * fTemp3)))));
		}
	}
};
