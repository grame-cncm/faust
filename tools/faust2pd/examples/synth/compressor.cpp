...
class mydsp : public dsp {
  ...
  public:
	...
	virtual void buildUserInterface(UI* interface) {
		...
		interface->openHorizontalBox("1-compression");
		...
		interface->addHorizontalSlider("makeup gain", &fslider2, 0.0f, -96.0f, 96.0f, 0.1f);
		interface->closeBox();
		interface->closeBox();
	}
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		for (int i=0; i<count; i++) {
			...
			float fTemp3 = ((fSlow3 * (fRec0[1] < fTemp2)) + (fSlow2 * (fRec0[1] >= fTemp2)));
			fRec0[0] = ((fRec0[1] * fTemp3) + (fTemp2 * (0 - (fTemp3 - 1))));
			float fTemp4 = max((float)0, ((20 * log10f(fRec0[0])) + fSlow1));
			float fTemp5 = (fSlow5 * min((float)1, max((float)0, (fSlow4 * fTemp4))));
			float fTemp6 = ((fTemp4 * (0 - fTemp5)) / (1 + fTemp5));
			fbargraph0 = fTemp6;
			float fTemp7 = powf(10,(0.05f * (fSlow6 + fTemp6)));
			output0[i] = (FAUSTFLOAT)(fTemp0 * fTemp7);
			output1[i] = (FAUSTFLOAT)(fTemp1 * fTemp7);
			// post processing
			fRec0[1] = fRec0[0];
			fRec2[1] = fRec2[0];
			fRec1[1] = fRec1[0];
