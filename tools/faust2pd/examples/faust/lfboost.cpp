...
class mydsp : public dsp {
  ...
  public:
	...
	virtual void buildUserInterface(UI* interface) {
		...
		interface->addNumEntry("freq (Hz)", &fentry0, 1e+02f, 2e+01f, 1.5e+02f, 1.0f);
		interface->addVerticalSlider("gain (dB)", &fslider0, 0.0f, -2e+01f, 2e+01f, 0.1f);
		interface->closeBox();
	}
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		float 	fSlow9 = (1 + (fSlow0 * (fSlow6 + fSlow7)));
		float 	fSlow10 = (2 * ((fSlow5 * fSlow1) - 1));
		FAUSTFLOAT* input0 = input[0];
		FAUSTFLOAT* output0 = output[0];
		for (int i=0; i<count; i++) {
			fRec0[0] = ((float)input0[i] - (fSlow4 * ((fSlow3 * fRec0[2]) + (fSlow2 * fRec0[1]))));
			output0[i] = (FAUSTFLOAT)(fSlow4 * (((fSlow10 * fRec0[1]) + (fSlow9 * fRec0[0])) + (fSlow8 * fRec0[2])));
			// post processing
