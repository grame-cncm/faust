...
class mydsp : public dsp {
  ...
  public:
	...
	virtual void buildUserInterface(UI* interface) {
		...
		interface->addNumEntry("1-attack (ms)", &fentry3, 2e+01f, 1.0f, 1e+03f, 1.0f);
		interface->addNumEntry("2-decay (speed)", &fentry2, 2.0f, 1.0f, 1e+02f, 0.1f);
		interface->addNumEntry("3-sustain (pcent)", &fentry1, 1e+01f, 1.0f, 1e+02f, 0.1f);
		interface->addNumEntry("4-release (speed)", &fentry0, 1e+01f, 1.0f, 1e+02f, 0.1f);
	...
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		int 	iSlow1 = (fSlow0 > 0);
		int 	iSlow2 = (fSlow0 <= 0);
		float 	fSlow3 = (1e-05f * float(fentry0));
		float 	fSlow4 = (0.01f * float(fentry1));
		float 	fSlow5 = (1e-05f * float(fentry2));
		float 	fSlow6 = (fConst0 / float(fentry3));
		FAUSTFLOAT* output0 = output[0];
		for (int i=0; i<count; i++) {
			iRec0[0] = (12345 + (1103515245 * iRec0[1]));
			iRec1[0] = (iSlow1 & (iRec1[1] | (fRec2[1] >= 1)));
			fRec2[0] = ((fSlow6 * (((iRec1[1] == 0) & iSlow1) & (fRec2[1] < 1))) + (fRec2[1] * ((1 - (fSlow5 * (iRec1[1] & (fRec2[1] > fSlow4)))) - (fSlow3 * (iSlow2 & (fRec2[1] > 0))))));
			output0[i] = (FAUSTFLOAT)(4.656612875245797e-10f * (fRec2[0] * iRec0[0]));
			// post processing
			fRec2[1] = fRec2[0];
			iRec1[1] = iRec1[0];
			iRec0[1] = iRec0[0];
