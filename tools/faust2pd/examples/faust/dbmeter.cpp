...
class mydsp : public dsp {
  ...
  public:
	...
	virtual void instanceInit(int samplingFreq) {
		...
		fConst0 = (96.0f / float(min(192000, max(1, fSamplingFreq))));
	...
	virtual void buildUserInterface(UI* interface) {
		...
		interface->addVerticalBargraph("dB", &fbargraph0, -96.0f, 1e+01f);
		interface->closeBox();
	}
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		for (int i=0; i<count; i++) {
			...
			fRec0[0] = max((fRec0[1] - fConst0), min((float)10, (20 * log10f(max(1.584893192461114e-05f, fabsf(fTemp0))))));
			fbargraph0 = fRec0[0];
