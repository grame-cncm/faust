...
class mydsp : public dsp {
  ...
  public:
	static void metadata(Meta* m) 	{ 
		m->declare("name", "white -- white noise");
	...
	virtual void buildUserInterface(UI* interface) {
		...
		interface->addNumEntry("pan", &fentry0, 0.5f, 0.0f, 1.0f, 0.01f);
		interface->addNumEntry("vol", &fentry1, 0.3f, 0.0f, 1e+01f, 0.01f);
		interface->closeBox();
	}
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		float 	fSlow0 = float(fentry0);
		float 	fSlow1 = float(fentry1);
		float 	fSlow2 = (4.656612875245797e-10f * (fSlow1 * (1 - fSlow0)));
		float 	fSlow3 = (4.656612875245797e-10f * (fSlow0 * fSlow1));
		FAUSTFLOAT* output0 = output[0];
		FAUSTFLOAT* output1 = output[1];
		for (int i=0; i<count; i++) {
			iRec0[0] = (12345 + (1103515245 * iRec0[1]));
			output0[i] = (FAUSTFLOAT)(fSlow2 * iRec0[0]);
			output1[i] = (FAUSTFLOAT)(fSlow3 * iRec0[0]);
			// post processing
			iRec0[1] = iRec0[0];
		}
	}
