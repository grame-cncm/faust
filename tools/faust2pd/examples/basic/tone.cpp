//-----------------------------------------------------
// name: "tone -- sine oscillator with adjustable frequency"
// author: "Albert Graef"
// version: "1.0"
//
...
class mydsp : public dsp {
  ...
  public:
	static void metadata(Meta* m) 	{ 
		m->declare("name", "tone -- sine oscillator with adjustable frequency");
	...
	virtual void buildUserInterface(UI* interface) {
		...
		interface->addNumEntry("pan", &fentry1, 0.5f, 0.0f, 1.0f, 0.01f);
		interface->addNumEntry("pitch", &fentry0, 4.4e+02f, 2e+01f, 2e+04f, 0.01f);
		interface->addNumEntry("vol", &fentry2, 0.3f, 0.0f, 1e+01f, 0.01f);
		interface->closeBox();
	}
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		float 	fSlow3 = (fSlow2 * (1 - fSlow1));
		float 	fSlow4 = (fSlow1 * fSlow2);
		...
		for (int i=0; i<count; i++) {
			...
			float fTemp4 = (fTemp3 + ((fTemp1 - floorf(fTemp1)) * (ftbl0[(1 + iTemp2)] - fTemp3)));
			output0[i] = (FAUSTFLOAT)(fSlow3 * fTemp4);
			output1[i] = (FAUSTFLOAT)(fSlow4 * fTemp4);
			// post processing
			fRec1[1] = fRec1[0];
		}
	}
