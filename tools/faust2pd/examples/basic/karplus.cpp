...
class mydsp : public dsp {
  ...
  public:
	...
	virtual void buildUserInterface(UI* interface) {
		...
		interface->addHorizontalSlider("pan", &fslider2, 0.5f, 0.0f, 1.0f, 0.01f);
		interface->addHorizontalSlider("vol", &fslider3, 0.3f, 0.0f, 1e+01f, 0.01f);
		interface->closeBox();
		interface->closeBox();
	}
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		float 	fSlow13 = (fSlow12 * (1 - fSlow11));
		float 	fSlow14 = (fSlow12 * fSlow11);
		...
		for (int i=0; i<count; i++) {
			...
			float fTemp0 = ((fSlow4 * (fRec1[1] + fRec1[2])) + (fSlow2 * (iRec3[0] * ((fRec2[0] > 0.0f) + 1.52587890625e-05f))));
			fVec1[IOTA&4095] = fTemp0;
			fRec1[0] = ((fSlow10 * fVec1[(IOTA-iSlow9)&4095]) + (fSlow8 * fVec1[(IOTA-iSlow7)&4095]));
			fRec0[0] = ((fRec1[0] + (0.995f * fRec0[1])) - fRec1[1]);
			output0[i] = (FAUSTFLOAT)(fSlow13 * fRec0[0]);
			output1[i] = (FAUSTFLOAT)(fSlow14 * fRec0[0]);
			// post processing
			fRec0[1] = fRec0[0];
