//-----------------------------------------------------
// name: "chorus -- a simple chorus effect"
// author: "Albert Graef"
// version: "1.0"
//
...
class mydsp : public dsp {
  ...
  public:
	...
	virtual void buildUserInterface(UI* interface) {
		...
		interface->addHorizontalSlider("delay", &fslider2, 0.025f, 0.0f, 0.2f, 0.001f);
		interface->addHorizontalSlider("depth", &fslider1, 0.02f, 0.0f, 1.0f, 0.001f);
		interface->addHorizontalSlider("freq", &fslider0, 2.0f, 0.0f, 1e+01f, 0.01f);
		interface->addHorizontalSlider("level", &fslider3, 0.5f, 0.0f, 1.0f, 0.01f);
		interface->closeBox();
	}
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		for (int i=0; i<count; i++) {
			...
			int iTemp7 = (1 + iTemp6);
			int iTemp8 = int((int(iTemp7) & 65535));
			float fTemp9 = (fTemp5 - iTemp6);
			float fTemp10 = (iTemp7 - fTemp5);
			int iTemp11 = int((iTemp6 & 65535));
			output0[i] = (FAUSTFLOAT)(fVec0[IOTA&65535] + (fSlow3 * ((fVec0[(IOTA-iTemp11)&65535] * fTemp10) + (fTemp9 * fVec0[(IOTA-iTemp8)&65535]))));
			float fTemp12 = (float)input1[i];
			fVec1[IOTA&65535] = fTemp12;
			output1[i] = (FAUSTFLOAT)(fVec1[IOTA&65535] + (fSlow3 * ((fTemp10 * fVec1[(IOTA-iTemp11)&65535]) + (fTemp9 * fVec1[(IOTA-iTemp8)&65535]))));
			// post processing
			fRec1[1] = fRec1[0];
			IOTA = IOTA+1;
		}
