// CSSCHEDULE validation, stage 1 (spec: faust-migration/CSSCHEDULE.md §12).
// Exhaustive oracle on small DAGs: exact minimum cycles under (R,U) with
// hard R, computed by subset DP; current schedulers (dfschedule, the
// csschedule lab) replayed against it. Liveness follows the spec: global
// usage counts, net balance per op, sample after each op.
//
