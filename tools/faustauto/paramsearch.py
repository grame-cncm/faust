#!/usr/bin/env python3
"""paramsearch — la couche 3 : recherche de paramètres numériques du
compilateur, au même juge que les élections (six serrures).

Un BOUTON = des valeurs candidates autour du défaut, un jeu de TÉMOINS
(les programmes où la famille du bouton gagne ou frôle, d'après la
carte V8), et un mode de transport (variable d'environnement ou
drapeaux). Protocole : par témoin, un binaire par valeur, tours
alternés co-mesurés ; agrégat = geomean des courbes normalisées ;
un nouveau défaut n'est proclamé que s'il confirme son avance sur un
SECOND tirage espacé (la règle karplus). Usage :
...
KNOBS = {
    ...
    "atoms": {
        ...
        "default": 4,
        "values": [2, 3, 4, 5, 6],
        # dx7 est la FALAISE (8 -> croissance DNF, crash pile) : témoin
        # obligatoire, une valeur qui ne compile pas est disqualifiée
        "witnesses": [
...
def measure(knob, w, rounds_n):
    """Un témoin : binaire par valeur, tours alternés ; courbe normalisée."""
...
def main():
    ...
    # agregat : geomean des courbes par valeur (valeurs presentes partout)
    ...
    if winner == knob["default"]:
        ...
        return
    # regle des deux tirages : le pretendant doit confirmer, espace
