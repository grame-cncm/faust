...
tc = (0.00025+0.0018*(220/freq)^(0.6))*(1.35-0.6*gain):max(0.0002):min(0.0035);
...
hammer = pulse*gain^(1.5)*min(2.5, (freq/220)^(0.25)):fi.lowpass(1, 700+11000*gain*gain):fi.dcblocker;

//---------------------------------- string ----------------------------------
nAP = 4;
// dispersion allpass sections per string
...
string(f0) = fi.highpass(2, f0*0.25):(+:de.fdelay4(4096, dl):dispersion:lossFilter)~*(g0)
    with {
        ...
        w = 2*ma.PI*f0/ma.SR;
        budget = max(0.5, (ma.SR/f0-4)/nAP);
        // per-section delay budget
        ...
        apDelay = nAP*(atan2(sin(w), a+cos(w))-atan2(a*sin(w), 1+a*cos(w)))/w;
        ...
        lossFilter = *(1-s):+~*(s);
        lossDelay = atan2(s*sin(w), 1-s*cos(w))/w;
        // exact, at f0
...
soundboard(x) = loop~si.bus(4):mixdown
    with {
        ...
        line(i) = *(10^(-3.0*sbDelays(i)/(ma.SR*sbT60))):fi.lowpass(1, sbDamp):de.delay(4096, sbDelays(i));
