...
f0c = 27.5;
lr = log(freq/f0c)/log(4192.0/f0c);
// 0..1 across the keyboard
Lstr = 0.04+1.4/(1+exp(-3.4+1.4*log(freq/f0c)));
rstr = 0.002*(1+0.6*log(freq/f0c))^(-1.4);
// string radius (m)
rhoL = ma.PI*rstr*rstr*7850;
// linear mass density
Tstr = (2*Lstr*freq)^2*rhoL;
// tension
Zstr = sqrt(Tstr*rhoL);
// string impedance
Zb = 4000;
// bridge impedance
rcore = min(rstr, 0.0006);
// wound bass: steel core
Bstiff = ma.PI^3*200e9*rcore^4/(4*Lstr*Lstr*Tstr);
// inharmonicity
...
timer = (+(1):*(1-trig))~_:/(ma.SR);
// seconds since key-on
tc = (0.0002+0.0009*(220/freq)^(0.6))*(1.35-0.6*gain):max(0.0002):min(0.0035);
...
hammer = pulse*gain^(1.5)*min(2.5, (freq/220)^(0.25)):fi.lowpass(1, 1400+16000*gain*gain):strikeComb:fi.dcblocker:fi.highpass(2, freq*0.25);
...
stringBody(f0) = de.fdelay4(4096, dl):dispersion:lossFilter
    with {
        ...
        a1t = -2*(D-2)/(D+1);
        a2t = (D-1)*(D-2)/((D+1)*(D+2));
        sec(i) = fi.tf2(select2(active(i), 1, a2t),
            select2(active(i), 0, a1t),
            select2(active(i), 0, 1),
            select2(active(i), 0, a1t),
            select2(active(i), 0, a2t));
        dispersion = seq(i, 4, sec(i));
        // Exact phase delay of one active section at f0.
        pdSec = 0-(atan2(0-(a1t*sin(w)+sin(2*w)), a2t+a1t*cos(w)+cos(2*w))-atan2(0-(a1t*sin(w)+a2t*sin(2*w)), 1+a1t*cos(w)+a2t*cos(2*w)))/w:+(2*ma.PI/w*((a2t+a1t*cos(w)+cos(2*w))<0));
        // unwrap
...
bridge = (couple~si.bus(3)):sumS*100
    with {
        ...
        couple(o1, o2, o3) = stringBody(freq, hammer+o1-s), stringBody(freq*1.0003, hammer+o2-s), stringBody(freq*0.9996, hammer+o3-s)
            with {
                s = sumS(o1, o2, o3);
            };
    };
...
soundboard(x) = (fb~si.bus(8)):taps
    with {
        ...
        fb(o0, o1, o2, o3, o4, o5, o6, o7) = line(0, x+aC*s+o1), line(1, x+aC*s+o2), line(2, x+aC*s+o3), line(3, x+aC*s+o4), line(4, x+aC*s+o5), line(5, x+aC*s+o6), line(6, x+aC*s+o7), line(7, x+aC*s+o0)
            with {
                s = o0+o1+o2+o3+o4+o5+o6+o7;
            };
        taps(o0, o1, o2, o3, o4, o5, o6, o7) = 0.5/8*(o0-o1+o2-o3+o4-o5+o6-o7), 0.5/8*(o0+o1-o2-o3+o4+o5-o6-o7);
    };
