...
NLFM = nonLinearModulator((nonLinearity:si.smoo),
    1,
    freq,
    typeModulation,
    (frequencyMod:si.smoo),
    nlfOrder);
...
biquadBank = _<:sum(i, 4, oneFilter(i))
    with {
        ...
        oneFilter(j, y) = (loadPreset(presetNumber, 0, j:si.smoo):condition), loadPreset(presetNumber, 1, j:si.smoo)*(1-(gain*0.03*dampCondition)), y*(loadPreset(presetNumber, 2, j):si.smoo):bandPassH;
    };
...
process = excitation:sourceFilter:*(gain)<://resonance
(biquadBank<:-(*(directGain)))+(directGain*_)://vibrato for the vibraphone
