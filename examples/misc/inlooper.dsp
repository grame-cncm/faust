...
inlooper(length, x) = attach(rwtable(length, 0.0, rec_index, x, loop_index), pos)
    with {
        ...
        // User controls
        speed = hslider("h:Controls/[3]Speed", 1, 0.2, 5, 0.01);
        // Slider to control playback speed
        record = button("h:Controls/[4]Record");
        // Button to start/stop recording
        play = checkbox("h:Controls/[5]Play/Stop");
        // Checkbox to start/stop playback
        reverse = checkbox("h:Controls/[6]Reverse");
        // Checkbox to play in reverse
...
process = inlooper(MAX_LENGTH)
    with {
        MAX_LENGTH = 5*44100;
        // 5 seconds at 44100 Hz, has to be a constant
    };
