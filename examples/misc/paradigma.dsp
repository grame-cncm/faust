...
qsincurve(x) = 1 - ( (x*x)<: *(1.2253517*16),(_<:*:* (-3.60562732*16)):>_ );// cosine like half circle
qsin(x) = x+(0.5): wrap <: (abs:-(0.25):qsincurve),_:ma.copysign;           // full sine wave, input -0.5...0.5
...
xornx(step) = (*(constnx): + ( nxseed + step ) : int <: _,(_<<13) :xor<:_,(_>>17):xor<:_,(_<<5):xor:_ & 536870911 
            <:_,(float:_*(1/2147483648):wrap) ) ~ _: !,_*( 1 / ( constnx + 1));
...
sncnx(fw, off) = mphasor(fw):_+off:wrap           
            <:(_
                <:_+0.5 <:_,_+fw
                <:_/fw,_/fw <:(_<:_,_),_:_,floor,floor <:_,(_<:_, _),_:
                (_-_,xornx,xornx):(_<:_, _),_,_:route( 4,4, 1,1, 3,2, 2,3, 4,4):
                (1-_)*_,_*_:>_*5),
                sncwin:_,ba.if( constnx, 1, _):
            *;
...
widbrighten = octavesfromtop -4<:_,0:max:1/(2^_);   // curve to brighten ( == reduce triangle width) low notes
...
minwid = 4 / (ma.SR/f);  // minimum triangle edge time
maxwid = 0.5;            // maximum triangle edge time

// scale with envelope
// 1. rising edge
wid2 =  wid * (fiadapt^(dwo * (1-envg ))):
        min( _,  maxwid): max( _, minwid);
// 2. falling edge relative to rising edge
wid2e = (1/512 + wid2 * widsym ): min( _, maxwid): max( _, minwid); 
fiw = w/wid2;       //  t increment for rising edge
fiwtail = w/wid2e;  //  t increment for falling edge

fiwcompens = 0.25* wid2e / (2/512) ; // gain compensation
...
pickup(x) =  x,
            ...
            ( x,(x^2:_+ 0.25:_^(3/2)):/ ),
            // ILO anti aliasing:
            ( 0.25^(3/2) / ( sqrt(x*x + 1)):ma.neg:_+ 0.25^(3/2) ):
            // select
...
synthvox(fw, ph2, ph3, g1, g2, g3) = (oscxy(fw, 0):_*g1), (oscxy(fw, ph2):_*g2),(oscxy(fw, ph3):_*g3):>_:
            _*lpa:fi.pole(lpp):
            fi.pole(igpole);

// Sound 
// =====
process = synthvox(wplus, wid2, wid2e, apg0, apg1, apg2): fi.svf.peak( resf, fiq):
        fi.svf.hp( hpoffs*fplus/(wid2+wid2e):min(_, ma.SR*0.166), 0.707 ):
        _*pickupdrive: pickup * (min( 1/pickupdrive, 1 ))<:_,_;
