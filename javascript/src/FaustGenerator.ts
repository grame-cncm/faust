...
class InstanceAPIImpl implements InstanceAPI {
    ...
    compute(dsp: DSP, count: number, input: FaustAudioBuffer, output: FaustAudioBuffer) { this.fExports.compute(dsp, count, input, output); }
...
class FaustGeneratorImp implements FaustGenerator {
    ...
    async loadDSPMixer(mixer_path: string): Promise<WebAssembly.Module | null> {
        try {
            ...
            if (WFS) {
                mixer_buffer = WFS.readFile(mixer_path, { encoding: 'binary' });
            } else {
...
function createGenerator() { return new FaustGeneratorImp(); }
function createInstanceAPI(exports: InstanceAPI): InstanceAPI { return new InstanceAPIImpl(exports); }

if (typeof module === 'object' && module.exports) {
    module.exports = { createGenerator, createInstanceAPI };
}
