...
function createFaustJSON(json: string): TFaustJSON { return JSON.parse(json); }
function createCompiler(libfaust: LibFaust) { return new CompilerImp(libfaust); }


declare var sha256: any;

if (typeof module === 'object' && module.exports) {
    module.exports = { createFaustJSON, createCompiler };
}
...
class CompilerImp implements Compiler {
    ...
    private async createDSPFactoryImp(name: string, dsp_code: string, args: string, poly: boolean): Promise<Factory | null> {
        ...
        if (typeof sha256 !== 'undefined') {
            sha_key = sha256(name + dsp_code + args + ((poly) ? "poly" : "mono"));
            cached = CompilerImp.gFactories.has( sha256);
        }
        if (cached)
