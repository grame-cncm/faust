...
interface InstanceAPI {
     ...
     * @param {FaustAudioBuffer} inputs - the input audio buffer as in index in wasm memory
     * @param {FaustAudioBuffer} output - the output audio buffer as in index in wasm memory
     */
    compute(dsp: DSP, count: number, inputs: FaustAudioBuffer, output: FaustAudioBuffer): void;
 ...
 */
interface MixerAPI {
    clearOutput(bufferSize: number, chans: number, outputs: FaustAudioBuffer): void;
    mixCheckVoice(bufferSize: number, chans: number, inputs: FaustAudioBuffer, outputs: FaustAudioBuffer): number;
    fadeOut(bufferSize: number, chans: number, outputs: FaustAudioBuffer): void;
}

/**
 * The generator constructor.
 */
interface FaustGenerator {
