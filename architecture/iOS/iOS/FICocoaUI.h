...
class uiCocoaItem : public uiItem {
   ...
   public:
    ...
    uiCocoaItem(GUI* ui, float* zone, FIMainViewController* controller, const char* name) : uiItem(ui, zone)
    {
    ...
    void setAbstractFrame(float x, float y, float w, float h)
    {
        fAbstractX = x;
        fAbstractY = y;
        fAbstractW = w;
    ...
    void setInitAssignationType(int assignationType)
    {
        fInitAssignationType = assignationType;
        setAssignationType(assignationType);
    }
    ...
    void setInitAssignationCurve(int assignationCurve)
    {
        fInitAssignationCurve = assignationCurve;
        setAssignationCurve(assignationCurve);
    }
    ...
    void setHideOnGUI(BOOL hideOnGUI)
    {
        fHideOnGUI = hideOnGUI;
        if (fLabel) fLabel.hidden = hideOnGUI;
    }
...
class uiBox : public uiCocoaItem {
   public:
    ...
    BOOL isHExpandable() { return TRUE; }

    BOOL isVExpandable() { return FALSE; }
    ...
    void setFrame(float x, float y, float w, float h)
    {
        ...
        // For vertical and horizontal boxes
            ...
            for (i = fWidgetList.begin(); i != fWidgetList.end(); i++) {
                if (fBoxType == kVerticalLayout && (*i)->getW() < w - 2 * kSpaceSize && (*i)->getParent() == this) {
                    (*i)->setFrame((*i)->getX(), (*i)->getY(), w - 2 * kSpaceSize, (*i)->getH());
                } else if (fBoxType == kHorizontalLayout && (*i)->getH() < h - 2 * kSpaceSize - labelYOffset &&
                           (*i)->getParent() == this) {
    ...
    void close(int index) { fClosed = TRUE; }
...
class uiKnob : public uiCocoaItem {
   public:
    ...
    uiKnob(GUI* ui, FIMainViewController* controller, const char* name, float* zone, float init, float min, float max,
           float step, BOOL horizontal)
        : uiCocoaItem(ui, zone, controller, name)
    {
        ...
        fLongPressGesture          = [[UILongPressGestureRecognizer alloc] initWithTarget:controller
                                                                          action:@selector(showWidgetPreferencesView:)];
    ...
    void resetInitialValue() { fKnob.value = fInit; }

    BOOL isHExpandable() { return FALSE; }
...
class uiSlider : public uiCocoaItem {
   public:
    ...
    uiSlider(GUI* ui, FIMainViewController* controller, const char* name, float* zone, float init, float min, float max,
             float step, BOOL horizontal, const char* menu = NULL)
        : uiCocoaItem(ui, zone, controller, name)
    {
        fLabel      = [[[UILabel alloc] init] autorelease];
        fLabel.font = [UIFont boldSystemFontOfSize:12];
    ...
    void resetInitialValue() { fSlider.value = fInit; }
    ...
    void setFrame(float x, float y, float w, float h)
    {
        ...
        if (fHorizontal) {
            fLabel.frame =
                CGRectMake(pt.x, pt.y + (h - kStdSliderLabelHeight) / 2.f, kStdSliderLabelWidth, kStdSliderLabelHeight);

            fSlider.frame =
                CGRectMake(pt.x + kStdSliderLabelWidth + kSpaceSize, pt.y + (h - kStdHorizontalSliderHeight) / 2.f,
                           w - kStdSliderLabelWidth - kSpaceSize, kStdHorizontalSliderHeight);
        } else {
            fSlider.frame = CGRectMake(pt.x + (w - kStdVerticalSliderWidth) / 2.f, pt.y, kStdVerticalSliderWidth,
                                       h - kSpaceSize - kStdSliderLabelHeight);

            fLabel.frame =
                CGRectMake(pt.x + (w - kStdSliderLabelWidth) / 2.f, pt.y + h - kSpaceSize - kStdSliderLabelHeight,
                           kStdSliderLabelWidth, kStdSliderLabelHeight);
        }
    }
...
class uiButton : public uiCocoaItem {
   public:
        ...
        : uiCocoaItem(ui, zone, controller, name)
    {
        ...
        // fLongPressGesture = [[UILongPressGestureRecognizer alloc] initWithTarget:controller
        // action:@selector(showWidgetPreferencesView:)];  fLongPressGesture.delegate = controller; [fButton
        //addGestureRecognizer:fLongPressGesture];
    }
    ...
    void setFrame(float x, float y, float w, float h)
    {
        ...
        fButton.frame = CGRectMake(pt.x + (w - kStdButtonWidth) / 2.f, pt.y + (h - kStdButtonHeight) / 2.f,
...
class uiNumEntry : public uiCocoaItem {
   public:
    ...
    uiNumEntry(GUI* ui, FIMainViewController* controller, const char* label, float* zone, float init, float min,
               float max, float step, const char* menu = NULL)
    ...
    ~uiNumEntry() { [fTextField release]; }

    void resetInitialValue() { fTextField.value = fInit; }
    ...
    void setFrame(float x, float y, float w, float h)
    {
        ...
        fTextField.frame = CGRectMake(pt.x + (w - kStdNumEntryWidth) / 2.f,
                                      ...
                                      kStdNumEntryWidth, kStdNumEntryHeight);

        fLabel.frame =
            CGRectMake(pt.x + (w - kStdNumEntryLabelWidth) / 2.f,
                       pt.y + (h + kStdNumEntryHeight - kSpaceSize - kStdNumEntryLabelHeight) / 2.f + kSpaceSize,
                       kStdNumEntryLabelWidth, kStdNumEntryLabelHeight);
    }
...
class uiBargraph : public uiCocoaItem {
   public:
    ...
    uiBargraph(GUI* ui, FIMainViewController* controller, const char* name, float* zone, float min, float max,
               BOOL horizontal)
        : uiCocoaItem(ui, zone, controller, name)
    {
        fLed        = false;
    ...
    void setFrame(float x, float y, float w, float h)
    {
        ...
        if (fLed) {
            ...
            fLabel.frame = CGRectMake(      pt.x + (w - kStdBargraphLabelWidth) / 2.f,
                                            pt.y + (h + kStdBargraphLabelHeight - kSpaceSize - kStdBargraphLabelHeight)
            / 2.f + kSpaceSize, kStdBargraphLabelWidth, kStdBargraphLabelHeight);*/

            if (fHorizontal) {
                fBargraph.frame = CGRectMake(pt.x + kStdLedWidth + kSpaceSize, pt.y + (h - kStdLedHeight) / 2.f,
                                             w - kStdBargraphLabelWidth - kSpaceSize, kStdLedHeight);

                fLabel.frame = CGRectMake(pt.x, pt.y + (h - kStdBargraphLabelHeight) / 2.f, kStdBargraphLabelWidth,
                                          kStdBargraphLabelHeight);
            } else {
                fBargraph.frame = CGRectMake(pt.x + (w - kStdLedWidth) / 2.f, pt.y, kStdLedWidth,
                                             h - kSpaceSize - kStdBargraphLabelHeight);

                fLabel.frame = CGRectMake(pt.x + (w - kStdBargraphLabelWidth) / 2.f,
                                          pt.y + h - kSpaceSize - kStdBargraphLabelHeight, kStdBargraphLabelWidth,
                                          kStdBargraphLabelHeight);
            }
        } else if (fHorizontal) {
            fBargraph.frame =
                CGRectMake(pt.x + kStdBargraphLabelWidth + kSpaceSize, pt.y + (h - kStdHorizontalBargraphHeight) / 2.f,
                           w - kStdBargraphLabelWidth - kSpaceSize, kStdHorizontalBargraphHeight);

            fLabel.frame = CGRectMake(pt.x, pt.y + (h - kStdBargraphLabelHeight) / 2.f, kStdBargraphLabelWidth,
                                      kStdBargraphLabelHeight);
        } else {
            fBargraph.frame = CGRectMake(pt.x + (w - kStdVerticalBargraphWidth) / 2.f, pt.y, kStdVerticalBargraphWidth,
                                         h - kSpaceSize - kStdBargraphLabelHeight);

            fLabel.frame =
                CGRectMake(pt.x + (w - kStdBargraphLabelWidth) / 2.f, pt.y + h - kSpaceSize - kStdBargraphLabelHeight,
                           kStdBargraphLabelWidth, kStdBargraphLabelHeight);
        }
    }
...
class CocoaUI : public GUI {
   ...
   private:
        ...
        // Otherwise, computing (x, y) of the widget within its parent box
            ...
            switch (fCurrentLayoutType) {
                ...
                case kHorizontalLayout:
                    ...
                    if (parent->fLabel)
                        y = kSpaceSize + kStdBoxLabelHeight;
                    else
                        y = kSpaceSize;
                    break;
    ...
    void refreshLayout(uiCocoaItem* widget)
    {
        ...
        if (parent) {
            ...
            parent->setFrame(parent->getX(), parent->getY(), contentSize.width + kSpaceSize,
        ...
        if (fCurrentLayoutType == kTabLayout) {
            for (i = fWidgetList.rbegin(); i != fWidgetList.rend(); i++) {
                if ((box = dynamic_cast<uiBox*>(*i))) {
                    if (box->fBoxType == kTabLayout && box != widget && !box->fClosed) {
                        // Add FIButton in the fTabView
                        [box->fTabView
                            addButtonWithLabel:[NSString stringWithCString:label encoding:NSASCIIStringEncoding]];
            ...
            */
            ...
            for (i = fWidgetList.rbegin(); i != fWidgetList.rend(); i++) {
                if ((box = dynamic_cast<uiBox*>(*i))) {
                    if ((box->fBoxType == kHorizontalLayout || box->fBoxType == kVerticalLayout) && box != widget) {
                        // Add uiCocoaItem in the uiBox (*i)
                        box->fWidgetList.push_back(widget);
                    }
                }
   ...
   public:
    ...
    APIUI::Type getParamType(int index) { return fAPIUI.getParamType(index); }
    ...
    void expandBoxesContent()
    {
        ...
        for (i = fWidgetList.begin(); i != fWidgetList.end(); i++) {
            if ((box = dynamic_cast<uiBox*>(*i))) {
                ...
                if (box->fBoxType == kVerticalLayout && contentSize.height + kSpaceSize < box->getH()) {
                    // Init values
                    ...
                    if (extensibleElementsTotalSize > 0.) {
                        ...
                        rx = (box->getH() - fixedElementsTotalSize -
                              (box->getNumberOfDirectChildren() + 1) * kSpaceSize - labelHeight) /
                             extensibleElementsTotalSize;
                        ...
                        for (j = fWidgetList.begin(); j != fWidgetList.end(); j++) {
                            if ((*j)->getParent() == box) {
                                ...
                                (*j)->setFrame((*j)->getX(), cpt + kSpaceSize + labelHeight, (*j)->getW(), newVal);
                    ...
                    // There is no extensible element
                        ...
                        for (j = fWidgetList.begin(); j != fWidgetList.end(); j++) {
                            if ((*j)->getParent() == box) {
                                if (box->fLabel)
                                    labelHeight = kStdBoxLabelHeight;
                                else
                                    labelHeight = 0.f;
                                ...
                                if (contentSize.height == kSpaceSize) {
                                    ...
                                    if (divider != 0.0) {
                                        y = ((*j)->getY() - kSpaceSize - labelHeight) *
                                                ((box->getH() - 2.f * kSpaceSize - labelHeight) / divider) +
                                            kSpaceSize + labelHeight;
                ...
                // Expand objects if content width is < than box width (horizontal box)
                else if (box->fBoxType == kHorizontalLayout && contentSize.width + kSpaceSize < box->getW()) {
                    // Init values
                    extensibleElementsTotalSize = 0.f;
                    fixedElementsTotalSize      = 0.f;
                    rx                          = 1.f;
                    ...
                    if (extensibleElementsTotalSize > 0.) {
                        // Compute extension ratio
                        rx = (box->getW() - fixedElementsTotalSize -
                              (box->getNumberOfDirectChildren() + 1) * kSpaceSize) /
                             extensibleElementsTotalSize;
                        ...
                        for (j = fWidgetList.begin(); j != fWidgetList.end(); j++) {
                            if ((*j)->getParent() == box) {
                                ...
                                (*j)->setFrame(cpt + kSpaceSize, (*j)->getY(), newVal, (*j)->getH());
                        ...
                        for (j = fWidgetList.begin(); j != fWidgetList.end(); j++) {
                            if ((*j)->getParent() == box) {
                                ...
                                if (contentSize.width == kSpaceSize) {
                                    ...
                                    x = ((*j)->getX() - kSpaceSize) *
                                            ((box->getW() - 2.f * kSpaceSize) / (contentSize.width - kSpaceSize)) +
                                        kSpaceSize;
                                    w = (*j)->getW() *
                                        ((box->getW() - 2.f * kSpaceSize) / (contentSize.width - kSpaceSize));
                                }

                                (*j)->setFrame(x, (*j)->getY(), w, (*j)->getH());
                            }
                        }
                    }
                }
        ...
        if ((box = dynamic_cast<uiBox*>(*i))) {
            ...
            for (j = fWidgetList.begin(); j != fWidgetList.end(); j++) {
                if ((box = dynamic_cast<uiBox*>(*j))) {
                    if (!hExpandable && box->fBoxType == kHorizontalLayout && box->getNumberOfDirectChildren() > 1) {
                        hExpandable = TRUE;
                    }

                    if (!vExpandable && box->fBoxType == kVerticalLayout && box->getNumberOfDirectChildren() > 1) {
                        vExpandable = TRUE;
                    }
                }
            }
            ...
            (*i)->setFrame((*i)->getX(), (*i)->getY(), newWidth, newHeight);
            ...
            if (cpt == 1) {
                (*i)->setFrame((*i)->getX(), (*i)->getY(), width, height);
            }
        }
    ...
    uiBox* getBoxForPoint(CGPoint pt)
    {
        ...
        for (i = fWidgetList.rbegin(); i != fWidgetList.rend(); i++) {
            if ((box = dynamic_cast<uiBox*>(*i))) {
                if (!(*i)->isHidden() && pt.x >= absolutePosition(*i).x &&
                    pt.x <= absolutePosition(*i).x + (*i)->getW() && pt.y >= absolutePosition(*i).y &&
    ...
    uiBox* getMainBox() { return (fWidgetList.size() > 0) ? dynamic_cast<uiBox*>(*fWidgetList.begin()) : NULL; }

    bool isKnob(float* zone) { return fKnobSet.count(zone) > 0; }

    bool isRadio(float* zone) { return fRadioDescription.count(zone) > 0; }

    bool isMenu(float* zone) { return fMenuDescription.count(zone) > 0; }
    ...
    virtual void openHorizontalBox(const char* label = "")
    {
        ...
        if (getCurrentOpenedBox())
            ...
            item->setHideOnGUI(fNextBoxIsHideOnGUI);
    ...
    virtual void addVerticalKnob(const char* label, float* zone, float init, float min, float max, float step)
    {
        ...
        dynamic_cast<uiKnob*>(item)->fKnob.suffixe =
            [[NSString alloc] initWithCString:fUnit[zone].c_str() encoding:NSUTF8StringEncoding];
    ...
    virtual void addVerticalSlider(const char* label, float* zone, float init, float min, float max, float step)
    {
        ...
        if (isKnob(zone)) {
            ...
            if (dynamic_cast<uiSlider*>(item)->fSlider.suffixe)
                [dynamic_cast<uiSlider*>(item)->fSlider.suffixe release];
            dynamic_cast<uiSlider*>(item)->fSlider.suffixe =
                [[NSString alloc] initWithCString:fUnit[zone].c_str() encoding:NSUTF8StringEncoding];
    ...
    virtual void addVerticalRadioButtons(const char* label, float* zone, float init, float min, float max, float step,
                                         const char* mdescr)
    {
        if (!fBuildUI) {
            return;
    ...
    virtual void addHorizontalRadioButtons(const char* label, float* zone, float init, float min, float max, float step,
                                           const char* mdescr)
    {
        if (!fBuildUI) {
            return;
    ...
    virtual void addNumEntry(const char* label, float* zone, float init, float min, float max, float step)
    {
        ...
        if (isKnob(zone)) {
            ...
            uiCocoaItem* item =
                new uiNumEntry(this, fViewController, label, zone, init, min, max, step,
                               ((fMenuDescription.count(zone) ? fMenuDescription[zone].c_str() : NULL)));
            if (dynamic_cast<uiNumEntry*>(item)->fTextField.suffixe)
                [dynamic_cast<uiNumEntry*>(item)->fTextField.suffixe release];
            dynamic_cast<uiNumEntry*>(item)->fTextField.suffixe =
                [[NSString alloc] initWithCString:fUnit[zone].c_str() encoding:NSUTF8StringEncoding];
    ...
    virtual void addHorizontalBargraph(const char* label, float* zone, float min, float max)
    {
        ...
        if (fR[zone] && fG[zone] && fB[zone]) {
            item->setInitColor(fR[zone] - 1000., fG[zone] - 1000., fB[zone] - 1000.);
            dynamic_cast<uiBargraph*>(item)->fBargraph.ledMaxColor =
                [[UIColor colorWithRed:fR[zone] - 1000. green:fG[zone] - 1000. blue:fB[zone] - 1000. alpha:1.] retain];
        }
        if (getCurrentOpenedBox()) {
            if (fHideOnGUI[zone] || getCurrentOpenedBox()->getHideOnGUI()) item->setHideOnGUI(TRUE);
        } else {
        ...
        if (fLedR[zone] || fLedG[zone] || fLedB[zone]) {
            dynamic_cast<uiBargraph*>(item)->fBargraph.ledMinColor =
                [[UIColor colorWithRed:fLedR[zone] green:fLedG[zone] blue:fLedB[zone] alpha:1.] retain];
        }
    ...
    virtual void declare(float* zone, const char* key, const char* value)
    {
        if (zone == 0) {
            ...
            } else if (strcmp(key, "accx") == 0 || strcmp(key, "accy") == 0 || strcmp(key, "accz") == 0 ||
                       strcmp(key, "gyrox") == 0 || strcmp(key, "gyroy") == 0 || strcmp(key, "gyroz") == 0) {
                if (strcmp(key, "accx") == 0)
                    fAssignationType[zone] = kAssignationAccelX;
                else if (strcmp(key, "accy") == 0)
                    fAssignationType[zone] = kAssignationAccelY;
                else if (strcmp(key, "accz") == 0)
                    fAssignationType[zone] = kAssignationAccelZ;
                else if (strcmp(key, "gyrox") == 0)
                    fAssignationType[zone] = kAssignationGyroX;
                else if (strcmp(key, "gyroy") == 0)
                    fAssignationType[zone] = kAssignationGyroY;
                else if (strcmp(key, "gyroz") == 0)
                    fAssignationType[zone] = kAssignationGyroZ;
            }
        }
    }
};
