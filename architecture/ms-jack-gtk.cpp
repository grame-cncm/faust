...
// inline void *alloc(size_t nmemb, size_t size) { return (void*)((unsigned)(calloc((nmemb*size)+15,sizeof(char)))+15 &
// 0xfffffff0); }
...
class MidiShareUI : public UI {
   ...
   public:
    MidiShareUI() : fRefnum(-1)
    {
        for (int i = 0; i < 128; i++) fMidiCtrl[i] = 0;
    }
    ...
    {
        int n = atoi(value);
        if (strcmp(key, "midictrl") == 0 && fZoneCtrl[zone] != 0 && n >= 0 && n <= 127) fMidiCtrl[n] = fZoneCtrl[zone];
    }
