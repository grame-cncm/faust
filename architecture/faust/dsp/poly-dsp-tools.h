...
class dsp_poly_effect : public dsp_poly {
   ...
   public:
    dsp_poly_effect(dsp_poly* dsp1, dsp* dsp2) : dsp_poly(dsp2), fPolyDSP(dsp1) {}

    virtual ~dsp_poly_effect() {}

    MapUI* keyOn(int channel, int pitch, int velocity) { return fPolyDSP->keyOn(channel, pitch, velocity); }
    void   keyOff(int channel, int pitch, int velocity) { fPolyDSP->keyOff(channel, pitch, velocity); }
    void   keyPress(int channel, int pitch, int press) { fPolyDSP->keyPress(channel, pitch, press); }
    void   chanPress(int channel, int press) { fPolyDSP->chanPress(channel, press); }
    void   ctrlChange(int channel, int ctrl, int value) { fPolyDSP->ctrlChange(channel, ctrl, value); }
    void   ctrlChange14bits(int channel, int ctrl, int value) { fPolyDSP->ctrlChange14bits(channel, ctrl, value); }
...
struct dsp_poly_factory : public dsp_factory {
    ...
    dsp_poly_factory(llvm_dsp_factory* process_factory, llvm_dsp_factory* effect_factory)
        : fProcessFactory(process_factory), fEffectFactory(effect_factory)
    {
    }

    dsp_poly_factory(const std::string& name_app, const std::string& dsp_content, int argc, const char* argv[],
                     const std::string& target, std::string& error_msg, int opt_level = -1)
    {
    ...
    virtual ~dsp_poly_factory()
    {
        ...
        if (fEffectFactory) {
            deleteDSPFactory(fEffectFactory);
        }
    }
    ...
    {
        return fProcessFactory->getDSPFactoryIncludePathnames();
    }
 ...
 */
inline dsp_poly_factory* createPolyDSPFactoryFromString(const std::string& name_app, const std::string& dsp_content,
 ...
 * @return a Polyphonic DSP factory on success, otherwise a null pointer.
 */
inline dsp_poly_factory* createPolyDSPFactoryFromFile(const std::string& filename, int argc, const char* argv[],
                                                      const std::string& target, std::string& error_msg,
                                                      int opt_level = -1)
{
    return createPolyDSPFactoryFromString("FaustDSP", pathToContent(filename), argc, argv, target, error_msg,
                                          opt_level);
}
 ...
 */
inline dsp_poly_factory* readPolyDSPFactoryFromBitcodeFile(const std::string& bit_code_path, const std::string& target,
                                                           int opt_level = -1)
{
 ...
 * @return the Polyphonic DSP factory on success, otherwise a null pointer.
 */
inline dsp_poly_factory* readPolyDSPFactoryFromIRFile(const std::string& ir_code_path, const std::string& target,
                                                      int opt_level = -1)
{
 ...
 * @return the Polyphonic DSP factory on success, otherwise a null pointer.
 */
inline dsp_poly_factory* readPolyDSPFactoryFromMachineFile(const std::string& machine_code_path,
 ...
 *
 */
inline void writePolyDSPFactoryToMachineFile(dsp_poly_factory* factory, const std::string& machine_code_path,
                                             const std::string& target)
{
    std::string process_path = machine_code_path + "_machine_process";
