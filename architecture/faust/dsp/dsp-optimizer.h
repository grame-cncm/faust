...
class dsp_optimizer {
   private:
    ...
    double bench(int run)
    {
        ...
        if (fCount == -1) {
            ...
            for (int i = 0; i < run; i++) {
                mes.measure();
                std::cout << mes.getStats() << " "
                          << "(DSP CPU % : " << (mes.getCPULoad() * 100) << ")" << std::endl;
    ...
    void init()
    {
        ...
        if (typeid(SAMPLE_TYPE).name() == typeid(double).name()) {
            t0.push_back("-double");
        }
        t0.push_back("-scal");
    ...
    std::pair<double, std::vector<std::string> > findOptimizedParametersAux(
    ...
    void init(const std::string& filename, const std::string input, int argc, const char* argv[],
              const std::string& target, int buffer_size, int run, int opt_level_max)
    {
        fFilename   = filename;
        fInput      = input;
        fTarget     = target;
   ...
   public:
     ...
     */
    dsp_optimizer(const char* filename, int argc, const char* argv[], const std::string& target, int buffer_size,
                  int run = 1, int opt_level = -1)
    {
     ...
     * since the maximum value may change with new LLVM versions)
     */
    dsp_optimizer(const std::string& input, int argc, const char* argv[], const std::string& target, int buffer_size,
                  int opt_level = -1)
    {
