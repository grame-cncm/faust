...
class time_bench {
   protected:
    ...
    double rdtscpersec()
    {
        ...
        return double(fLastRDTSC - fFirstRDTSC) / (((double(fTv2.tv_sec) * 1000000 + double(fTv2.tv_usec)) -
                                                    (double(fTv1.tv_sec) * 1000000 + double(fTv1.tv_usec))) /
                                                   1000000);
    }
     ...
     */
    double rdtsc2sec(uint64 clk) { return double(clk) / rdtscpersec(); }

    double rdtsc2sec(double clk) { return clk / rdtscpersec(); }
   ...
   public:
    ...
    {
        return ((double(fTv2.tv_sec) * 1000000 + double(fTv2.tv_usec)) -
                (double(fTv1.tv_sec) * 1000000 + double(fTv1.tv_usec)));
    }
...
class measure_dsp : public decorator_dsp {
   ...
   public:
     ...
     *
     */
    measure_dsp(dsp* dsp, int buffer_size, int count) : decorator_dsp(dsp), fBufferSize(buffer_size), fCount(count)
    {
        init();
     ...
     *
     */
    measure_dsp(dsp* dsp, int buffer_size, double duration_in_sec) : decorator_dsp(dsp), fBufferSize(buffer_size)
    {
        init();
    ...
    double measureDurationUsec() { return fBench->measureDurationUsec(); }
     ...
     *  Returns best estimation
     */
    double getStats() { return fBench->getStats(fBufferSize, fDSP->getNumInputs(), fDSP->getNumOutputs()); }
