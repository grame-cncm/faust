...
/*!
 \addtogroup llvmcpp C++ interface for compiling Faust code. Note that the API is not thread safe : use
 'startMTDSPFactories/stopMTDSPFactories' to use it in a multi-thread context.
 @{
 */
 ...
 * have to explicitly use deleteDSPFactory to properly decrement the reference counter when the factory is no more
 * needed.
 *
 ...
 * return the same (reference counted) factory pointer. You will have to explicitly use deleteDSPFactory to properly
 ...
 * @return the DSP factory on success, otherwise a null pointer.
 */
llvm_dsp_factory* readDSPFactoryFromBitcodeFile(const std::string& bit_code_path, const std::string& target,
                                                int opt_level = -1);
 ...
 * @return the DSP factory on success, otherwise a null pointer.
 */
llvm_dsp_factory* readDSPFactoryFromIRFile(const std::string& ir_code_path, const std::string& target,
                                           int opt_level = -1);
