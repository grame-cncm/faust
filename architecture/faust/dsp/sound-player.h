...
class sound_base_player : public dsp {
   ...
   public:
    ...
    virtual void buildUserInterface(UI* ui_interface)
    {
        ...
        ui_interface->addHorizontalSlider("Set position", &fSetFrames, FAUSTFLOAT(0), FAUSTFLOAT(0),
                                          FAUSTFLOAT(fInfo.frames), FAUSTFLOAT(100));
    ...
    virtual void instanceConstants(int samplingRate) { fSamplingRate = samplingRate; }
    ...
    virtual void instanceClear() { fCurFrames = FAUSTFLOAT(0); }

    virtual sound_base_player* clone() { return new sound_base_player(fFileName); }

    virtual void metadata(Meta* m) { m->declare("name", fFileName.c_str()); }
    ...
    static void setFrame(FAUSTFLOAT val, void* arg) { static_cast<sound_base_player*>(arg)->setFrame(int(val)); }

    FAUSTFLOAT* getCurFramesZone() { return &fCurFrames; }

    FAUSTFLOAT* getSetFramesZone() { return &fSetFrames; }
};
...
class sound_memory_player : public sound_base_player {
   private:
    ...
    {
        // If position change
        if (fSetFrames != fLastSetFrames && std::abs(fSetFrames - fLastSetFrames) > BUFFER_SIZE &&
...
class PositionManager : public GUI {
   ...
   public:
    ...
    {
        fFileReader[dsp] =
            std::make_pair(new uiCallbackItem(this, dsp->getCurFramesZone(), sound_base_player::setFrame, dsp),
    ...
    void removeDSP(sound_base_player* dsp) { fFileReader.erase(dsp); }
};
