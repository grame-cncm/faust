...
inline double GetCurrentTimeInUsec()
{
    return double(AudioConvertHostTimeToNanos(AudioGetCurrentHostTime())) / 1000.;
}
#endif
#endif
...
struct ZoneUI : public GenericUI {
    ...
    // -- active widgets
    void addButton(const char* label, FAUSTFLOAT* zone) { insertZone(zone); }
    void addCheckButton(const char* label, FAUSTFLOAT* zone) { insertZone(zone); }
...
class timed_dsp : public decorator_dsp {
   protected:
    ...
    ztimedmap::iterator getNextControl(DatedControl& res, bool convert_ts)
    {
        ...
        for (it3 = fZoneUI.fZoneSet.begin(); it3 != fZoneUI.fZoneSet.end(); it3++) {
            ...
            if (it1 != GUI::gTimedZoneMap.end()) {  // Check if zone still in global GUI::gTimedZoneMap (since MidiUI
                                                    // may have been desallocated)
                DatedControl date2;
   ...
   public:
    ...
    virtual timed_dsp* clone() { return new timed_dsp(fDSP->clone()); }
