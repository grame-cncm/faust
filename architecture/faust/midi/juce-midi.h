...
class juce_midi_handler : public midi_handler {
   ...
   public:
    ...
    void progChange(int channel, int pgm) { fOutputBuffer.addEvent(MidiMessage::programChange(channel + 1, pgm), 0); }
    ...
    void start_sync(double date) { fOutputBuffer.addEvent(MidiMessage::midiStart(), 0); }

    void stop_sync(double date) { fOutputBuffer.addEvent(MidiMessage::midiStop(), 0); }

    void clock(double date) { fOutputBuffer.addEvent(MidiMessage::midiClock(), 0); }
};
...
class juce_midi : public juce_midi_handler, public MidiInputCallback {
   private:
    ...
    void handleIncomingMidiMessage(MidiInput*, const MidiMessage& message) { decodeMessage(message); }

   public:
    virtual ~juce_midi() { stop_midi(); }
    ...
    void progChange(int channel, int pgm) { fMidiOut->sendMessageNow(MidiMessage::programChange(channel + 1, pgm)); }
    ...
    void start_sync(double date) { fMidiOut->sendMessageNow(MidiMessage::midiStart()); }

    void stop_sync(double date) { fMidiOut->sendMessageNow(MidiMessage::midiStop()); }

    void clock(double date) { fMidiOut->sendMessageNow(MidiMessage::midiClock()); }
};
