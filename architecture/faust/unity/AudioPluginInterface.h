...
enum UnityAudioEffectDefinitionFlags {
    ...
    UnityAudioEffectDefinitionFlags_IsSpatializer =
        2 << 0,  // Should this plugin be inserted at sources and take over panning?
};
...
struct UnityAudioSpatializerData {
    ...
    UnityAudioEffect_DistanceAttenuationCallback distanceattenuationcallback;  // The spatializer plugin may override
                                                                               // the distance attenuation in order to
                                                                               // influence the voice prioritization
                                                                               // (leave this callback as NULL to use
                                                                               // the built-in audio source attenuation
                                                                               // curve)
};
...
struct UnityAudioParameterDefinition {
    ...
    float displayscale;     // Scale factor used only for the display of parameters (i.e. 100 for a percentage value
                            // ranging from 0 to 1)
...
struct UnityAudioEffectDefinition {
    ...
    UInt32 channels;         // Number of channels. Effects should set this to 0 and process any number of input/output
                      // channels they get in the process callback. Generator elements should specify a >0 value here.
    ...
    UnityAudioEffect_ReleaseCallback release;  // The release callback is called just before the plugin is freed and
                                               // should free any data associated with this specific instance of the
                                               // plugin. No further callbacks related to the instance will happen after
    ...
    UnityAudioParameterDefinition* paramdefs;  // A pointer to the definitions of the parameters exposed by this plugin.
                                               // This data pointed to must remain valid for the whole lifetime of the
                                               // dynamic library (ideally it's static).
