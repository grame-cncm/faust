...
class vstUIObject { /* superclass of all VST UI widgets */
   ...
   protected:
    ...
    float clip(float min, float max, float val) { return (val < min) ? min : (val > max) ? max : val; }
   ...
   public:
    vstUIObject(const char* label, float* zone) : fLabel(label), fZone(zone), metadata() {}
    ...
    virtual float GetValueNoNormalization() { return *fZone; }
    ...
    const float* getZone() const { return fZone; }
...
class vstSlider : public vstUIObject {
   ...
   public:
    ...
    {
        for (fPrecision = 0; fPrecision < MAX_PARAM_DISPLAY_PRECISION && step != floor(step);
             ++fPrecision, step *= 10.0) {
            ;
        }
    }
    virtual ~vstSlider() {}
    ...
    int getMinValue() const { return fMin; }

    int getMaxValue() const { return fMax; }
...
class vstUI : public UI {
   ...
   public:
    ...
    // Constructor
    vstUI() : fStopped(false), freqIndex(-1), gainIndex(-1), gateIndex(-1) {}

    // Destructor
    virtual ~vstUI()
    {
        for (vector<vstUIObject*>::iterator iter = fUITable.begin(); iter != fUITable.end(); iter++) {
            delete *iter;
        }

        for (map<const float*, Meta*>::iterator iter = m_controlMetadataMap.begin(); iter != m_controlMetadataMap.end();
             ++iter) {
            delete iter->second;
        }
    }  // end of destructor
    ...
    void setFreq(float val) { setAny(freqIndex, val, "freq"); }

    float getFreq(void) { return getAny(freqIndex, "freq"); }

    void setGate(float val) { setAny(gateIndex, val, "gate"); }

    void setGain(float val) { setAny(gainIndex, val, "gain"); }

    void setPrevFreq(float val) { setAny(prevFreqIndex, val, "prevfreq"); }

    void setPitchBend(float val) { setAny(pitchbendIndex, val, "pitchbend"); }
    ...
    void addVerticalSlider(const char* label, float* zone, float init, float min, float max, float step)
    {
        ...
        TRACE(fprintf(stderr,
                      "=== Faust vsti: Adding VSlider (HSlider) "
                      "with label '%s'\n",
                      label));
        ckAllMatches(label);
    }
    ...
    void addNumEntry(const char* label, float* zone, float init, float min, float max, float step)
    {
        ...
        fUITable.push_back(theSlider);
        TRACE(fprintf(stderr,
                      "=== Faust vsti: Adding NumEntry (HSlider) with "
                      "label '%s'\n",
                      label));
        ckAllMatches(label);
    }
