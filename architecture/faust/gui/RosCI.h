   ...
   public:
    ...
    void callbacksWriter(int num, std::vector<RosCI::CallbackParams> param_vector, std::string name)
    {
        ...
        for (int i = 0; i < num; i++) {
            ...
            if (include) {
                file << "#include \"" << parameters.msg_type << "/" << parameters.msg_name << ".h\"" << std::endl;
            }
        }
        ...
        for (int i = 0; i < num; i++) {
            RosCI::CallbackParams parameters = param_vector[i];
            file << "\tvoid callback" << i << "(const " << parameters.msg_type << "::" << parameters.msg_name
                 << "ConstPtr& msg, FAUSTFLOAT* zone)" << std::endl
                 << "\t{" << std::endl
        ...
        for (int i = 0; i < num; i++) {
            ...
            file << "\t\tros::Subscriber* my_sub" << i << " = new ros::Subscriber();" << std::endl
                 ...
                 << ">(\"" << parameters.topic_name << "\", 1," << std::endl
                 << "\t\t\tboost::bind(&RosCallbacks::callback" << i << ", this, _1, zones[" << i << "]));\n"
                 << std::endl;
        }
        ...
        file << "\tprivate :\n" << std::endl << "\tros::NodeHandle nh_;" << std::endl;
    ...
    void declare(FAUSTFLOAT* zone, const char* key, const char* val)
    {
        if (key == "ros")  // We do not care if key is not "ros" here
        {
            ...
            if (topic_params_.size() == 4 || topic_params_.size() == 6) {
    ...
    std::vector<CallbackParams> getCallbacksParameters() { return callbacks_parameters_; }
