...
class CMDUI : public UI {
   ...
   public:
    ...
    virtual void addButton(const char* label, FAUSTFLOAT* zone) { addOption(label, zone, 0, 0, 1); }

    virtual void addCheckButton(const char* label, FAUSTFLOAT* zone) { addOption(label, zone, 0, 0, 1); }
    ...
    char* input_file()
    {
        std::cout << "input file " << fFiles[0] << "\n";
        return fFiles[0];
    }
    char* output_file()
    {
        std::cout << "output file " << fFiles[1] << "\n";
        return fFiles[1];
    }
