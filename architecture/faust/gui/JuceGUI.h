...
struct CustomLookAndFeel : public LookAndFeel_V3 {
    void drawRoundThumb(Graphics& g, const float x, const float y, const float diameter, const Colour& colour,
                              ...
                              bool isButtonDown) override
    {
        ...
        if (width > 0 && height > 0) {
            ...
            Path outline;
            outline.addRoundedRectangle(0.5f + halfThickness, 0.5f + halfThickness, width - lineThickness,
                                        height - lineThickness, cornerSize, cornerSize, !(flatOnLeft || flatOnTop),
                               ...
                               float maxSliderPos, const Slider::SliderStyle style, Slider& slider) override
    {
        ...
        if (style == Slider::LinearHorizontal || style == Slider::LinearVertical) {
            ...
            LookAndFeel_V2::drawLinearSliderThumb(g, x, y, width, height, sliderPos, minSliderPos, maxSliderPos, style,
                                                  slider);
        }
    }
...
class uiBase {
   protected:
    ...
    int fDisplayRectWidth,
        fDisplayRectHeight;  // Size without margin, just the child dimensions, sum on one dimension, max on the other
   ...
   public:
    ...
    uiBase(int totWidth = 0, int totHeight = 0)
        : fTotalWidth(totWidth),
          fTotalHeight(totHeight),
          fDisplayRectWidth(0),
          fDisplayRectHeight(0),
          fHRatio(1),
          fVRatio(1)
    {
    }
    ...
    /** Return the size. */
    juce::Rectangle<int> getSize() { return juce::Rectangle<int>(0, 0, fTotalWidth, fTotalHeight); }

    /** Return the total height in pixels. */
    int getTotalHeight() { return fTotalHeight; }

    /** Return the total width in pixels. */
    int getTotalWidth() { return fTotalWidth; }
    ...
    {
        comp->setBounds(r.getX() - comp->getParentComponent()->getX(), r.getY() - comp->getParentComponent()->getY(),
...
class uiComponent : public uiBase, public Component, public uiItem {
   public:
     ...
     */
    uiComponent(GUI* gui, FAUSTFLOAT* zone, int w, int h, String name)
        : uiBase(w, h), Component(name), uiItem(gui, zone)
    {
    }
};
...
class uiSlider : public uiComponent, private juce::Slider::Listener {
   ...
   public:
    ...
    uiSlider(GUI* gui, FAUSTFLOAT* zone, FAUSTFLOAT w, FAUSTFLOAT h, FAUSTFLOAT cur, FAUSTFLOAT min, FAUSTFLOAT max,
             FAUSTFLOAT step, String name, String unit, String tooltip, MetaDataUI::Scale scale, SliderType type)
        : uiComponent(gui, zone, w, h, name), fType(type)
    {
...
class uiButton : public uiComponent, private juce::Button::Listener {
   ...
   public:
     ...
     */
    uiButton(GUI* gui, FAUSTFLOAT* zone, FAUSTFLOAT w, FAUSTFLOAT h, String label, String tooltip)
        : uiComponent(gui, zone, w, h, label)
    {
        int x = 0;
...
class uiCheckButton : public uiComponent, private juce::Button::Listener {
   ...
   public:
     ...
     */
    uiCheckButton(GUI* gui, FAUSTFLOAT* zone, FAUSTFLOAT w, FAUSTFLOAT h, String label, String tooltip)
        : uiComponent(gui, zone, w, h, label)
    {
        int x = 0;
        int y = (getHeight() - h) / 2;
    ...
    virtual void resized() override { fCheckButton.setBounds(getLocalBounds()); }
};
...
class uiMenu : public uiComponent, private juce::ComboBox::Listener {
   ...
   public:
     ...
     * \param   mdescr                              Menu description. Contains the names of the items associated with
     * their "value".
     */
    uiMenu(GUI* gui, FAUSTFLOAT* zone, String label, FAUSTFLOAT w, FAUSTFLOAT h, FAUSTFLOAT cur, FAUSTFLOAT lo,
           FAUSTFLOAT hi, String tooltip, const char* mdescr)
        : uiComponent(gui, zone, w, h, label)
    {
    ...
    virtual void resized() override { fComboBox.setBounds(0, 0 + kMenuHeight / 2, getWidth(), kMenuHeight / 2); }
...
class uiRadioButton : public uiComponent, private juce::Button::Listener {
   ...
   public:
     ...
     */
    uiRadioButton(GUI* gui, FAUSTFLOAT* zone, String label, FAUSTFLOAT w, FAUSTFLOAT h, FAUSTFLOAT cur, FAUSTFLOAT lo,
                  FAUSTFLOAT hi, bool vert, vector<string>& names, vector<double>& values, String tooltip,
                  int radioGroupID)
        : uiComponent(gui, zone, w, h, label), fIsVertical(vert)
    {
...
class uiVUMeter : public uiComponent, public SettableTooltipClient, public Timer {
   private:
    ...
    bool isNameDisplayed() { return (!(getName().startsWith("0x")) && getName().isNotEmpty()); }
    ...
    void setLabelPos()
    {
        if (fStyle == VVUMeter) {
            ...
            fLabel.setBounds((getWidth() - kNumDisplayWidth) / 2, (getHeight() - kNumDisplayHeight / 2) / 2,
                             kNumDisplayWidth, kNumDisplayHeight / 2);
        }
    }
    ...
    void drawHBargraphLin(Graphics& g, int x, int y, int width, int height)
    {
        ...
        if (fLevel > 0.2f) {
            g.setColour(c);
            g.fillRect(x + 1.0f + 0.2f * (width - 2), y + 1.0f,
                       jmin<float>((fLevel - 0.2f) * (width - 2), (0.9f - 0.2f) * (width - 2)), (float)height - 2);
        }
        // Drawing from 90% of the VU-meter to the current level, or the maximal range of the VU-meter
        if (fLevel > 0.9f) {
            g.setColour(c.darker());
            g.fillRect(x + 1.0f + 0.9f * (width - 2), y + 1.0f,
                       jmin<float>((fLevel - 0.9f) * (width - 2), (1.0f - 0.9f) * (width - 2)), (float)height - 2);
        }
    }
    /**
    ...
    void drawVBargraphDB(Graphics& g, int x, int width)
    {
        ...
        // Drawing from the minimal range to the current level, or -10dB.
        g.setColour(Colour((uint8)40, (uint8)160, (uint8)40, (uint8)alpha));
        g.fillRect(x + 1.0f, jmax(dB2y(fLevel), dB2y(-10)), (float)width - 2,
                   dB2y(fMin) - jmax(dB2y(fLevel), dB2y(-10)));

        // Drawing from -10dB to the current level, or -6dB.
        if (dblevel > dB2Scale(-10)) {
            g.setColour(Colour((uint8)160, (uint8)220, (uint8)20, (uint8)alpha));
            g.fillRect(x + 1.0f, jmax(dB2y(fLevel), dB2y(-6)), (float)width - 2,
                       dB2y(-10) - jmax(dB2y(fLevel), dB2y(-6)));
        }
        // Drawing from -6dB to the current level, or -3dB.
        if (dblevel > dB2Scale(-6)) {
            g.setColour(Colour((uint8)220, (uint8)220, (uint8)20, (uint8)alpha));
            g.fillRect(x + 1.0f, jmax(dB2y(fLevel), dB2y(-3)), (float)width - 2,
                       dB2y(-6) - jmax(dB2y(fLevel), dB2y(-3)));
        }
        // Drawing from -3dB to the current level, or 0dB.
        if (dblevel > dB2Scale(-3)) {
            g.setColour(Colour((uint8)240, (uint8)160, (uint8)20, (uint8)alpha));
        ...
        if (dblevel > dB2Scale(0)) {
            g.setColour(Colour((uint8)240, (uint8)0, (uint8)20, (uint8)alpha));
            g.fillRect(x + 1.0f, jmax(dB2y(fLevel), dB2y(fMax)), (float)width - 2,
                       dB2y(0) - jmax(dB2y(fLevel), dB2y(fMax)));
        }
    }
    ...
    void drawVBargraphLin(Graphics& g, int x, int width)
    {
        ...
        // Drawing from the minimal range to the current level, or 20% of the VU-meter.
        g.setColour(c.brighter());
        g.fillRect(x + 1.0f, jmax(lin2y(fLevel), lin2y(0.2)), (float)width - 2,
                   lin2y(fMin) - jmax(lin2y(fLevel), lin2y(0.2)));

        // Drawing from 20% of the VU-meter to the current level, or 90% of the VU-meter.
        if (fLevel > 0.2f) {
            g.setColour(c);
            g.fillRect(x + 1.0f, jmax(lin2y(fLevel), lin2y(0.9)), (float)width - 2,
                       lin2y(0.2) - jmax(lin2y(fLevel), lin2y(0.9)));
        }

        // Drawing from 90% of the VU-meter to the current level, or the maximum range.
        if (fLevel > 0.9f) {
            g.setColour(c.darker());
            g.fillRect(x + 1.0f, jmax(lin2y(fLevel), lin2y(fMax)), (float)width - 2,
                       lin2y(0.9) - jmax(lin2y(fLevel), lin2y(fMax)));
        }
    }
   ...
   public:
     ...
     * \param   vert                                True if vertical, false if horizontal.
     */
    uiVUMeter(GUI* gui, FAUSTFLOAT* zone, FAUSTFLOAT w, FAUSTFLOAT h, String label, FAUSTFLOAT mini, FAUSTFLOAT maxi,
              String unit, String tooltip, VUMeterType style, bool vert)
    ...
    void timerCallback() override
    {
        if (isShowing()) {
            ...
            if (fDB) {
                switch (fStyle) {
                    case VVUMeter:
                        if (((int)dB2y(lastLevel) != (int)dB2y(fLevel) && fLevel >= fMin && fLevel <= fMax) ||
                            forceRepaint) {
                            repaint();
                        }
                        break;
                    case HVUMeter:
                        if (((int)dB2x(lastLevel) != (int)dB2x(fLevel) && fLevel >= fMin && fLevel <= fMax) ||
                            forceRepaint) {
                            repaint();
                        }
                        break;
                    case NumDisplay:
                        ...
                        break;
                    case Led:
                        if ((dB2Scale(lastLevel) != dB2Scale(fLevel) && fLevel >= fMin && fLevel <= fMax) ||
                            forceRepaint) {
                            repaint();
                        }
                        break;
                ...
                switch (fStyle) {
                    case VVUMeter:
                        if (((int)lin2y(lastLevel) != (int)lin2y(fLevel) && fLevel >= fMin && fLevel <= fMax) ||
                            forceRepaint) {
                            repaint();
                        }
                        break;
    ...
    void resized() override { setLabelPos(); }
...
class uiBox : public uiBase, public Component {
   ...
   public:
    ...
    void add(Component* comp) override { addAndMakeVisible(comp); }
};
...
class JuceGUI : public GUI, public MetaDataUI, public Component {
   private:
    ...
    FAUSTFLOAT defaultVal(FAUSTFLOAT* zone, FAUSTFLOAT def) { return (fDefault) ? def : *zone; }
    ...
    void addSlider(const char* label, FAUSTFLOAT* zone, FAUSTFLOAT init, FAUSTFLOAT min, FAUSTFLOAT max,
                   FAUSTFLOAT step, int kWidth, int kHeight, SliderType type)
    {
        if (isKnob(zone)) {
            addKnob(label, zone, defaultVal(zone, init), min, max, step);
        } else if (isRadio(zone)) {
            addRadioButtons(label, zone, defaultVal(zone, init), min, max, step, fRadioDescription[zone].c_str(),
                            false);
        } else if (isMenu(zone)) {
            addMenu(label, zone, defaultVal(zone, init), min, max, step, fMenuDescription[zone].c_str());
        } else {
            fCurrentBox->add(new uiSlider(this, zone, kWidth, kHeight, defaultVal(zone, init), min, max, step,
                                          String(label), String(fUnit[zone]), String(fTooltip[zone]), getScale(zone),
                                          type));
        }
    }

    /** Add a radio buttons to the user interface. */
    void addRadioButtons(const char* label, FAUSTFLOAT* zone, FAUSTFLOAT init, FAUSTFLOAT min, FAUSTFLOAT max,
                         FAUSTFLOAT step, const char* mdescr, bool vert)
    {
        vector<string> names;
        vector<double> values;
        ...
        if (vert) {
            fCurrentBox->add(new uiRadioButton(this, zone, String(label), kCheckButtonWidth,
                                               names.size() * (kRadioButtonHeight - 25) + 25, defaultVal(zone, init),
                                               min, max, true, names, values, String(fTooltip[zone]), fRadioGroupID++));
        } else {
            fCurrentBox->add(new uiRadioButton(this, zone, String(label), kCheckButtonWidth, kRadioButtonHeight,
                                               defaultVal(zone, init), min, max, false, names, values,
                                               String(fTooltip[zone]), fRadioGroupID++));
        }
    }

    /** Add a menu to the user interface. */
    void addMenu(const char* label, FAUSTFLOAT* zone, FAUSTFLOAT init, FAUSTFLOAT min, FAUSTFLOAT max, FAUSTFLOAT step,
                 const char* mdescr)
    {
        fCurrentBox->add(new uiMenu(this, zone, String(label), kMenuWidth, kMenuHeight, defaultVal(zone, init), min,
                                    max, String(fTooltip[zone]), mdescr));
    }
    ...
    {
        fCurrentBox->add(new uiSlider(this, zone, kKnobWidth, kKnobHeight, defaultVal(zone, init), min, max, step,
                                      String(label), String(fUnit[zone]), String(fTooltip[zone]), getScale(zone),
                                      Knob));
    }

    /** Add a bargraph to the user interface. */
    void addBargraph(const char* label, FAUSTFLOAT* zone, FAUSTFLOAT min, FAUSTFLOAT max, int kWidth, int kHeight,
                     VUMeterType type)
    {
        if (isLed(zone)) {
            ...
            fCurrentBox->add(new uiVUMeter(this, zone, kWidth, kHeight, String(label), min, max, String(fUnit[zone]),
                                           String(fTooltip[zone]), type, false));
        }
    }
   ...
   public:
     ...
     */
    virtual ~JuceGUI() { delete fCurrentBox; }
    ...
    virtual void openTabBox(const char* label) override { openBox(new uiTabBox()); }

    /** Add a new vertical box to the user interface. */
    virtual void openVerticalBox(const char* label) override { openBox(new uiBox(true, String(label))); }

    /** Add a new horizontal box to the user interface. */
    virtual void openHorizontalBox(const char* label) override { openBox(new uiBox(false, String(label))); }
    ...
    virtual void addCheckButton(const char* label, FAUSTFLOAT* zone) override
    {
        ...
        fCurrentBox->add(
            new uiCheckButton(this, zone, newWidth, kCheckButtonHeight, String(label), String(fTooltip[zone])));
    }
                             ...
                             FAUSTFLOAT step) override
    {
        ...
        fCurrentBox->add(new uiSlider(this, zone, newWidth, kNumEntryHeight, defaultVal(zone, init), min, max, step,
                                      String(label), String(fUnit[zone]), String(fTooltip[zone]), getScale(zone),
                                      NumEntry));
    }
    ...
    {
        fCurrentBox->add(new uiVUMeter(this, zone, kLedWidth, kLedHeight, label, min, max, String(fUnit[zone]),
                                       String(fTooltip[zone]), Led, false));
    }
    ...
    {
        fCurrentBox->add(new uiVUMeter(this, zone, kNumDisplayWidth, kNumDisplayHeight, label, min, max,
                                       String(fUnit[zone]), String(fTooltip[zone]), NumDisplay, false));
    }
