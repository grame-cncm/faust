...
class OCVUI : public UI {
   public:
    ...
    void drawCircle(object my_object, cv::Mat my_image)
    {
        ...
        // draw circle
        cv::circle(my_image, cv::Point(my_object.centerX, my_object.centerY), my_object.radius, bgr_color, 2, 8, 0);
    }
    ...
    void imageProcessing(cv::Mat BGRImage)
    {
        ...
        // Objects contours
        std::vector<std::vector<cv::Point> > r_contours, y_contours, g_contours, c_contours, b_contours, m_contours;
        ...
        for (int i = 0; i < objects_storage_.size(); i++) {
            for (int j = 0; j < parameters_storage_.size(); j++) {
                if (objects_storage_[i].color == parameters_storage_[j].color && !parameters_storage_[j].used) {
     ...
     */
    void exitThread() { pthread_exit(NULL); }
