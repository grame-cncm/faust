...
class qsynthDialVokiStyle : public QCommonStyle {
   public:
                                    ...
                                    const QWidget* widget = NULL) const
    {
        ...
        double angle =
            DIAL_MIN  // offset
            + (DIAL_RANGE * (double(dial->sliderValue - dial->minimum) / (double(dial->maximum - dial->minimum))));
        ...
        QColor   meterColor  = (dial->state & State_Enabled) ? QColor("orange") : pal.mid().color();
        ...
        p->drawPie(xcenter - meterWidth / 2, ycenter - meterWidth / 2, meterWidth, meterWidth, (180 + 45) * 16,
                   -(degrees - 45) * 16);

        // Knob projected shadow
        QRadialGradient projectionGradient(xcenter + shineCenter, ycenter + shineCenter, shineExtension,
        ...
        p->drawEllipse(xcenter - shadowShift, ycenter - shadowShift, knobWidth, knobWidth);
        ...
        QRadialGradient gradient(xcenter - shineCenter, ycenter - shineCenter, shineExtension, xcenter - shineFocus,
                                 ycenter - shineFocus);
        ...
        p->drawEllipse(xcenter - knobWidth / 2, ycenter - knobWidth / 2, knobWidth, knobWidth);
        ...
        if (dial->subControls & QStyle::SC_DialTickmarks) {
            ...
            for (int i = 0; i < numTicks; ++i) {
                ...
                double angle    = DIAL_MIN + (DIAL_MAX - DIAL_MIN) * i / div;
        ...
        if (knobBorderWidth > 0) {
            ...
            p->drawEllipse(xcenter - side / 2 + indent, ycenter - side / 2 + indent, side - 2 * indent,
                           side - 2 * indent);
        }
        ...
        p->drawArc(xcenter - side / 2 + scaleShadowWidth / 2, ycenter - side / 2 + scaleShadowWidth / 2,
   ...
   protected:
    ...
    virtual void paintEvent(QPaintEvent*)
    {
        ...
        if (fValue <= fLevel[0]) {
            ...
            size_t l = fLevel.size() - 1;
            while (fValue < fLevel[l] && l > 0) l--;
...
class linBargraph : public AbstractDisplay {
   protected:
     ...
     */
    void paintScale(QPainter* painter) const { painter->drawRect(0, 0, width(), height()); }
...
class linVerticalBargraph : public linBargraph {
   public:
    ...
    virtual QSize sizeHint() const { return QSize(16, 128); }
};
...
class linHorizontalBargraph : public linBargraph {
   public:
    ...
    virtual QSize sizeHint() const { return QSize(128, 16); }
};
...
class dbVerticalBargraph : public dbBargraph {
   ...
   public:
    ...
    virtual QSize sizeHint() const { return QSize(18, 256); }
};
...
class dbHorizontalBargraph : public dbBargraph {
   ...
   public:
    ...
    virtual QSize sizeHint() const { return QSize(256, 18); }
};
...
class uiSlider : public QObject, public uiItem {
   ...
   public:
    uiSlider(GUI* ui, FAUSTFLOAT* zone, QAbstractSlider* slider, FAUSTFLOAT cur, FAUSTFLOAT lo, FAUSTFLOAT hi,
             FAUSTFLOAT step, MetaDataUI::Scale scale)
        : uiItem(ui, zone), fSlider(slider), fCur(cur), fMin(lo), fMax(hi), fStep(step)
    {
        // select appropriate converter according to scale mode
        if (scale == MetaDataUI::kLog) {
            fConverter = new LogValueConverter(0, 10000, fMin, fMax);
        } else if (scale == MetaDataUI::kExp) {
            fConverter = new ExpValueConverter(0, 10000, fMin, fMax);
        } else {
            fConverter = new LinearValueConverter(0, 10000, fMin, fMax);
        }
    ...
    virtual ~uiSlider() { delete fConverter; }
...
class ZoneSetter : public QObject {
   ...
   public:
    explicit ZoneSetter(FAUSTFLOAT v, FAUSTFLOAT* z, QObject* parent = NULL) : QObject(parent), fValue(v), fZone(z) {}
...
class uiRadioButtons : public QGroupBox, public uiItem {
   ...
   public:
    uiRadioButtons(GUI* ui, FAUSTFLOAT* z, const char* label, FAUSTFLOAT cur, FAUSTFLOAT lo, FAUSTFLOAT hi,
                   FAUSTFLOAT /*step*/, bool vertical, const char* mdescr, QWidget* parent)
    ...
    virtual void reflectZone()
    {
        ...
        if (defaultitem > -1) {
            fButtons[defaultitem]->setChecked(true);
        }
    }
};
...
class uiMenu : public QComboBox, public uiItem {
   ...
   public:
    uiMenu(GUI* ui, FAUSTFLOAT* z, const char* /*label*/, FAUSTFLOAT cur, FAUSTFLOAT lo, FAUSTFLOAT hi,
           FAUSTFLOAT /*step*/, const char* mdescr, QWidget* parent)
...
class uiNumEntry : public QObject, public uiItem {
   ...
   public:
    uiNumEntry(GUI* ui, FAUSTFLOAT* zone, QDoubleSpinBox* numEntry, FAUSTFLOAT cur, FAUSTFLOAT lo, FAUSTFLOAT hi,
               FAUSTFLOAT step)
   ...
   public slots:
    void setValue(double v) { modifyZone(FAUSTFLOAT(v)); }
};
...
static QImage getQRCode(const QString& url, int padding)
{
    ...
    if (!qrcodegen_encodeText(url.toStdString().c_str(), tempBuffer, qrcode, errCorLvl, qrcodegen_VERSION_MIN,
...
class QTGUI : public QWidget, public GUI, public MetaDataUI {
   ...
   protected:
    ...
    bool isTabContext()
    {
        // return fGroupStack.empty() || ((!fGroupStack.empty()) && (dynamic_cast<QTabWidget*>(fGroupStack.top()) !=
        // 0));
   ...
   public:
    ...
    virtual ~QTGUI() { delete fGeneralLayout; }
    ...
    void displayQRCode(const QString& url, QMainWindow* parent = NULL)
    {
        ...
        QString sheet =
            QString::fromLatin1("a{ text-decoration: underline; color: white; font: Menlo; font-size: 14px }");
    ...
    virtual void addNumDisplay(const char* label, FAUSTFLOAT* zone, FAUSTFLOAT init, FAUSTFLOAT min, FAUSTFLOAT max,
                               FAUSTFLOAT step)
    {
        // insert(label, new QDoubleSpinBox());
        if (label && label[0]) openVerticalBox(label);
    ...
    virtual void addVerticalKnob(const char* label, FAUSTFLOAT* zone, FAUSTFLOAT init, FAUSTFLOAT min, FAUSTFLOAT max,
                                 FAUSTFLOAT step)
    {
    ...
    virtual void addHorizontalKnob(const char* label, FAUSTFLOAT* zone, FAUSTFLOAT init, FAUSTFLOAT min, FAUSTFLOAT max,
                                   FAUSTFLOAT step)
    {
    ...
    virtual void addMenu(const char* label, FAUSTFLOAT* zone, FAUSTFLOAT init, FAUSTFLOAT min, FAUSTFLOAT max,
                         FAUSTFLOAT step, const char* mdescr)
    {
        if (label && label[0]) openVerticalBox(label);
