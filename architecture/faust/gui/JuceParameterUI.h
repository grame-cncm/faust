...
struct FaustPlugInAudioParameterBool : public AudioParameterBool, public uiOwnedItem {
    ...
    virtual void setValue(float newValue) override { modifyZone(FAUSTFLOAT(newValue)); }
};
...
struct FaustPlugInAudioParameterFloat : public AudioParameterFloat, public uiOwnedItem {
    FaustPlugInAudioParameterFloat(GUI* gui, FAUSTFLOAT* zone, const std::string& path, const std::string& label,
    ...
    virtual void setValue(float newValue) override { modifyZone(FAUSTFLOAT(range.convertFrom0to1(newValue))); }
};
...
class JuceParameterUI : public GUI, public PathBuilder {
   ...
   public:
    ...
    virtual void addVerticalSlider(const char* label, FAUSTFLOAT* zone, FAUSTFLOAT init, FAUSTFLOAT min, FAUSTFLOAT max,
                                   FAUSTFLOAT step)
    {
        fProcessor->addParameter(
            new FaustPlugInAudioParameterFloat(this, zone, buildPath(label), label, init, min, max, step));
    }
    ...
    {
        fProcessor->addParameter(
            new FaustPlugInAudioParameterFloat(this, zone, buildPath(label), label, 0, min, max, 0));
    }
