...
#define check_error(err)                                                                   \
    if (err) {                                                                             \
        printf("%s:%d, alsa error %d : %s\n", __FILE__, __LINE__, err, snd_strerror(err)); \
        exit(1);                                                                           \
    }
#define check_error_msg(err, msg)                                                                 \
    if (err) {                                                                                    \
        fprintf(stderr, "%s:%d, %s : %s(%d)\n", __FILE__, __LINE__, msg, snd_strerror(err), err); \
        exit(1);                                                                                  \
    }
#define display_error_msg(err, msg)                                                               \
    if (err) {                                                                                    \
        fprintf(stderr, "%s:%d, %s : %s(%d)\n", __FILE__, __LINE__, msg, snd_strerror(err), err); \
    }
...
struct AudioParam {
    ...
    AudioParam() : fCardName("hw:0"), fFrequency(44100), fBuffering(512), fPeriods(2), fSoftInputs(2), fSoftOutputs(2)
    {
    }
...
struct AudioInterface : public AudioParam {
    ...
    void open()
    {
        ...
        err = snd_pcm_open(&fOutputDevice, fCardName, SND_PCM_STREAM_PLAYBACK, 0);
        check_error(err)

            // setup output device parameters
            err = snd_pcm_hw_params_malloc(&fOutputParams);
        check_error(err) setAudioParams(fOutputDevice, fOutputParams);

        fCardOutputs = fSoftOutputs;
        snd_pcm_hw_params_set_channels_near(fOutputDevice, fOutputParams, &fCardOutputs);
        err = snd_pcm_hw_params(fOutputDevice, fOutputParams);
        check_error(err);
        ...
        if (fDuplexMode) {
            ...
            err = snd_pcm_hw_params_malloc(&fInputParams);
            check_error(err);
            ...
            err = snd_pcm_hw_params(fInputDevice, fInputParams);
            check_error(err);
    ...
    void shortinfo()
    {
        ...
        err = snd_ctl_open(&ctl_handle, fCardName, 0);
        check_error(err);
        snd_ctl_card_info_alloca(&card_info);
        err = snd_ctl_card_info(ctl_handle, card_info);
        check_error(err);
        printf("%s|%d|%d|%d|%d|%s\n", snd_ctl_card_info_get_driver(card_info), fCardInputs, fCardOutputs, fFrequency,
               fBuffering, snd_pcm_format_name((_snd_pcm_format)fSampleFormat));
    }
    ...
    void longinfo()
    {
        ...
        printf("Sampling Frequency : %d, Sample Format : %s, buffering : %d\n", fFrequency,
               snd_pcm_format_name((_snd_pcm_format)fSampleFormat), fBuffering);
...
long lopt(int argc, char* argv[], const char* longname, const char* shortname, long def)
{
    for (int i = 2; i < argc; i++)
        if (strcmp(argv[i - 1], shortname) == 0 || strcmp(argv[i - 1], longname) == 0) return atoi(argv[i]);
    return def;
}
...
{
    for (int i = 2; i < argc; i++)
        if (strcmp(argv[i - 1], shortname) == 0 || strcmp(argv[i - 1], longname) == 0) return argv[i];
    return def;
}
...
bool fopt(int argc, char* argv[], const char* longname, const char* shortname)
{
    for (int i = 1; i < argc; i++)
        if (strcmp(argv[i], shortname) == 0 || strcmp(argv[i], longname) == 0) return true;
    return false;
}
   ...
   public:
    alsaaudio(int argc, char* argv[], dsp* DSP) : fDSP(DSP), fRunning(false)
    {
        fAudio = new AudioInterface(
            AudioParam()
                .cardName(sopt(argc, argv, "--device", "-d", getDefaultEnv("FAUST2ALSA_DEVICE", "hw:0")))
    ...
    {
        fAudio = new AudioInterface(AudioParam().cardName("hw:0").frequency(srate).buffering(bsize).periods(2));
    }
