...
class netjackaudio : public audio {
   protected:
    ...
    static void netError(int error_code, void* arg) { return static_cast<netjackaudio*>(arg)->errorCb(error_code); }

    static int netProcess(jack_nframes_t buffer_size, int, float** audio_inputs, int, void** midi_inputs, int,
                          float** audio_outputs, int, void** midi_outputs, void* arg)
    {
    ...
    virtual int restartCb() { return 0; }

    virtual void shutdownCb() {}

    virtual void errorCb(int error_code) {}

    virtual int setSampleRate(jack_nframes_t nframes) { return 0; }

    virtual int setBufferSize(jack_nframes_t nframes) { return 0; }

    virtual void process(int count, float** audio_inputs, float** audio_outputs, void** midi_inputs,
                         void** midi_outputs)
    {
        AVOIDDENORMALS;
   ...
   public:
    netjackaudio(int net_format, const std::string& master_ip, int master_port, int mtu, int latency, int midi_in,
                 int midi_out)
        : fDSP(0),
          fNet(0),
    ...
    virtual bool isConnexionActive() { return jack_net_slave_is_active(fNet); }
...
class netjackaudio_control : public netjackaudio, public ControlUI {
   ...
   public:
    ...
    bool isConnexionActive() { return jack_net_slave_is_active(fNet); }

    virtual bool init(const char* name, dsp* dsp)
    {
        dsp->buildUserInterface(this);
        return initAux(name, dsp, dsp->getNumInputs() + 1, dsp->getNumOutputs() + 1, fMIDIInputs,
                       fMIDIOutputs);  // One more audio port for control
    }

    virtual int restartCb() { return -1; }
};
