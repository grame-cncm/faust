   ...
   public:
    ...
    bool initAux(const char* name)
    {
        ...
        char** physicalInPorts =
            (char**)jack_get_ports(fClient, NULL, JACK_DEFAULT_AUDIO_TYPE, JackPortIsPhysical | JackPortIsOutput);
        ...
        char** physicalOutPorts =
            (char**)jack_get_ports(fClient, NULL, JACK_DEFAULT_AUDIO_TYPE, JackPortIsPhysical | JackPortIsInput);
    ...
    virtual int getNumInputs() { return fPhysicalInputs.size(); }

    virtual int getNumOutputs() { return fPhysicalOutputs.size(); }
    ...
    jack_port_t* getInputPort(int port)
    {
        return (port >= 0 && port < (int)fInputPorts.size()) ? fInputPorts[port] : 0;
    }
    jack_port_t* getOutputPort(int port)
    {
        return (port >= 0 && port < (int)fOutputPorts.size()) ? fOutputPorts[port] : 0;
    }
};
...
class jackaudio_midi : public jackaudio, public jack_midi_handler {
   protected:
    ...
    virtual void processAudio(jack_nframes_t nframes)
    {
        ...
        // By convention timestamp of -1 means 'no timestamp conversion' : events already have a timestamp espressed in
        // frames
        fDSP->compute(-1, nframes, reinterpret_cast<FAUSTFLOAT**>(fInChannel),
   ...
   public:
    jackaudio_midi(const void* icon_data = 0, size_t icon_size = 0, bool auto_connect = true)
        : jackaudio(icon_data, icon_size, auto_connect),
          jack_midi_handler("JACKMidi"),
          fInputMidiPort(0),
          fOutputMidiPort(0)
    {
    }
    ...
    virtual bool start() { return jackaudio::start(); }

    virtual void stop() { jackaudio::stop(); }
};
