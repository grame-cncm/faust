...
struct LV2PluginUI {
  ...
  std::map<uint8_t,int> ctrlmap; // MIDI controller map
    ...
    LV2PluginUI(const int num_voices, LV2UI_Write_Function write_function, LV2UI_Controller controller)
        ...
        for (int i = 0, j = 0; i < ui->nelems; i++) {
            switch (ui->elems[i].type) {
                ...
                default:
                    ...
                    if (maxvoices == 0)
                        goto noinstr;
                    else if (freq == -1 && !strcmp(ui->elems[i].label, "freq"))
                        freq = i;
                    else if (gain == -1 && !strcmp(ui->elems[i].label, "gain"))
                        gain = i;
                    else if (gate == -1 && !strcmp(ui->elems[i].label, "gate"))
                        gate = i;
                    else {
                    noinstr:
#if 0  // FAUST_MIDICC
...
#endif
                write_function(controller, index, sizeof(float), 0, ui->elems[j].zone);
            }
    ...
    void getParameterDisplay(int index, char* text)
    {
        ...
        if (index < k) {
            ...
            sprintf(text, "%d %s", tuning_no, tuning_no > 0 ? mts->tuning[tuning_no - 1].name : "default");
#endif
        }
    }
...
QWidget* LV2QtGUI::open()
{
    ...
    QTGUIWrapper qtwrapper(qtinterface, plugui->maxvoices, plugui->n_tunings, &voices_zone, &tuning_zone);
    ...
    for (int i = 0; i < qtwrapper.nelems; i++) plugui->control_no[plugui->param_no[i]] = i;
#if 0
  ...
  for (int i = 0; i < plugui->n_params; i++)
...
#ifdef HTTPCTRL
    httpdinterface = new httpdUI(plugui->pluginName(), dsp->getNumInputs(), dsp->getNumOutputs(), argc, argv);
    ...
    for (QList<QObject*>::iterator i = allObjects.begin(); i != allObjects.end(); ++i) {
        ...
        if (slider) {
            ...
            connect(slider, SIGNAL(valueChanged(int)), this, SLOT(updateUI()), Qt::QueuedConnection);
        ...
        if (dial) {
            ...
            connect(dial, SIGNAL(valueChanged(int)), this, SLOT(updateUI()), Qt::QueuedConnection);
        ...
        if (button) {
            ...
            connect(button, SIGNAL(pressed()), this, SLOT(updateUI_buttonPressed()), Qt::QueuedConnection);
            connect(button, SIGNAL(released()), this, SLOT(updateUI_buttonReleased()), Qt::QueuedConnection);
        ...
        if (num) {
            ...
            if (num->buttonSymbols() != QAbstractSpinBox::NoButtons) {
                ...
                connect(num, SIGNAL(valueChanged(double)), this, SLOT(updateUI()), Qt::QueuedConnection);
                ...
                // iteration
            } else if (plugui->isPassiveControl(lv2ParamCount - 1) == 1 && !vBargraphChecked) {
#if FAUSTQT_DEBUG > 1
            ...
            } else if (lv2ParamCount < plugui->n_params && plugui->isPassiveControl(lv2ParamCount) == 2 &&
                       vBargraphChecked) {
        ...
        if (checkBox) {
            ...
            connect(checkBox, SIGNAL(stateChanged(int)), this, SLOT(updateUI_checkBox()), Qt::QueuedConnection);
        ...
        if (uiRadio) {
            ...
            for (QList<QRadioButton*>::iterator r = radiobuttons.begin(); r != radiobuttons.end(); ++r) {
                ...
                connect((*r), SIGNAL(clicked(bool)), this, SLOT(updateUI()), Qt::QueuedConnection);
        ...
        if (menu) {
            ...
            connect(menu, SIGNAL(activated(int)), this, SLOT(updateUI()), Qt::QueuedConnection);
...
void LV2QtGUI::updateQTGUI(QObject* object, float value)
{
    ...
    newValue = (minimum == maximum) ? minimum : minimum + plugui->quantize(value * (maximum - minimum), step);
...
static LV2UI_Handle instantiate(const struct _LV2UI_Descriptor* descriptor, const char* plugin_uri,
                                ...
                                LV2UI_Controller controller, LV2UI_Widget* widget, const LV2_Feature* const* features)
{
    if (strcmp(plugin_uri, PLUGIN_URI)) {
        fprintf(stderr, "%s: ERROR: this GUI does not support plugin with URI %s\n", PLUGIN_URI, plugin_uri);
        return NULL;
    }

    LV2PluginUI* plugui = new LV2PluginUI(LV2PluginUI::numVoices(), write_function, controller);
...
static void update_portval(LV2QtGUI* self, int i)
{
    ...
    if (i >= 0 && i < self->controls.size() && !self->controls[i].empty()) {
        ...
        if (plugui->isPassiveControl(i)) {
            for (QList<QObject*>::iterator it = self->controls[i].begin(); it != self->controls[i].end(); ++it)
...
static const LV2UI_Descriptor descriptor = {PLUGIN_URI "ui", instantiate, cleanup, port_event, extension_data};
