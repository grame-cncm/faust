...
JackArgParser::JackArgParser(const char* arg)
{
    ...
    do {
        ...
        if (pos == string::npos) pos = arg_len;
        // if double quote
        if (fArgString[pos] == '\"') {
...
{
    unsigned int i;
    for (i = 0; i < fArgv.size(); i++) free((void*)argv[i]);
    free((void*)argv);
}
...
void JackArgParser::ParseParams(jack_driver_desc_t* desc, JSList** param_list)
{
    ...
    for (i = 0; i < desc->nparams; i++) options_list += desc->params[i].character;
...
struct JackFaustInternal {
    ...
    JackFaustInternal(jack_client_t* client, const JSList* params) : fClient(client) {}

    ~JackFaustInternal() { delete fInterface; }
...
int jack_initialize(jack_client_t* client, const char* load_init)
{
    ...
    if (parser.GetArgc() > 0) parser.ParseParams(desc, &params);
