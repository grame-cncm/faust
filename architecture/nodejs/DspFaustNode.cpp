...
DspFaustNode::DspFaustNode(int sample_rate, int buffer_size) : DspFaust(sample_rate, buffer_size)
{
}
...
void DspFaustNode::Init(Handle<Object> exports)
{
    ...
    exports->Set(String::NewFromUtf8(isolate, "DspFaustNode"), tpl->GetFunction());
}
...
void DspFaustNode::New(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (args[0]->IsUndefined()) {  // if arg 0 is undefined then should be same for arg1
        ...
        dspFaustNode = new DspFaustNode((int)args[0]->NumberValue(), (int)args[1]->NumberValue());
    }
...
void DspFaustNode::keyOn(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    if (!args[0]->IsUndefined() && !args[1]->IsUndefined()) {
        ...
        args.GetReturnValue().Set(Number::New(isolate, fNode->DspFaust::keyOn(pitch, velocity)));
    } else {
...
void DspFaustNode::keyOff(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    if (!args[0]->IsUndefined()) {
        int pitch = (int)args[0]->NumberValue();
        args.GetReturnValue().Set(Integer::New(isolate, fNode->DspFaust::keyOff(pitch)));
    } else {
        printf("Missing argument\n");
    }
}
...
void DspFaustNode::deleteVoice(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    if (!args[0]->IsUndefined()) {
        ...
        args.GetReturnValue().Set(Integer::New(isolate, fNode->DspFaust::deleteVoice(voice)));
    } else {
        printf("Missing argument\n");
    }
}
...
void DspFaustNode::getParamValue(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    if (!args[0]->IsUndefined()) {
        std::string paramAddress(*v8::String::Utf8Value(args[0]->ToString()));
        if (isInteger(paramAddress)) {
            args.GetReturnValue().Set(Number::New(isolate, fNode->DspFaust::getParamValue(std::stoi(paramAddress))));
        } else {
            args.GetReturnValue().Set(Number::New(isolate, fNode->DspFaust::getParamValue(paramAddress.c_str())));
        }
    } else {
        printf("Missing argument\n");
    }
    ...
    if (!args[0]->IsUndefined() && !args[1]->IsUndefined()) {
        ...
        if (isInteger(paramAddress)) {
            args.GetReturnValue().Set(
                Number::New(isolate, fNode->DspFaust::getVoiceParamValue(std::stoi(paramAddress), voice)));
        } else {
            args.GetReturnValue().Set(
                Number::New(isolate, fNode->DspFaust::getVoiceParamValue(paramAddress.c_str(), voice)));
        }
    } else {
        printf("Missing argument\n");
    }
...
void DspFaustNode::getParamAddress(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    if (!args[0]->IsUndefined()) {
        ...
        args.GetReturnValue().Set(String::NewFromUtf8(isolate, fNode->DspFaust::getParamAddress(paramID)));
    } else {
        printf("Missing argument\n");
    }
}
...
void DspFaustNode::getParamMin(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    if (!args[0]->IsUndefined()) {
        std::string paramAddress(*v8::String::Utf8Value(args[0]->ToString()));
        if (isInteger(paramAddress)) {
            args.GetReturnValue().Set(Number::New(isolate, fNode->DspFaust::getParamMin(std::stoi(paramAddress))));
        } else {
            args.GetReturnValue().Set(Number::New(isolate, fNode->DspFaust::getParamMin(paramAddress.c_str())));
        }
    } else {
        printf("Missing argument\n");
    }
...
void DspFaustNode::getParamMax(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    if (!args[0]->IsUndefined()) {
        std::string paramAddress(*v8::String::Utf8Value(args[0]->ToString()));
        if (isInteger(paramAddress)) {
            args.GetReturnValue().Set(Number::New(isolate, fNode->DspFaust::getParamMax(std::stoi(paramAddress))));
        } else {
            args.GetReturnValue().Set(Number::New(isolate, fNode->DspFaust::getParamMax(paramAddress.c_str())));
        }
    } else {
        printf("Missing argument\n");
    }
...
void DspFaustNode::getParamInit(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    if (!args[0]->IsUndefined()) {
        std::string paramAddress(*v8::String::Utf8Value(args[0]->ToString()));
        if (isInteger(paramAddress)) {
            args.GetReturnValue().Set(Number::New(isolate, fNode->DspFaust::getParamInit(std::stoi(paramAddress))));
        } else {
            args.GetReturnValue().Set(Number::New(isolate, fNode->DspFaust::getParamInit(paramAddress.c_str())));
        }
    } else {
        printf("Missing argument\n");
    }
...
void DspFaustNode::getMetadata(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    if (!args[0]->IsUndefined() && !args[1]->IsUndefined()) {
        ...
        if (isInteger(paramAddress)) {
            args.GetReturnValue().Set(
                String::NewFromUtf8(isolate, fNode->DspFaust::getMetadata(std::stoi(paramAddress), key.c_str())));
        } else {
            args.GetReturnValue().Set(
                String::NewFromUtf8(isolate, fNode->DspFaust::getMetadata(paramAddress.c_str(), key.c_str())));
        }
    } else {
        printf("Missing argument\n");
    }
