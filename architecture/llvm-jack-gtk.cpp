...
class llvmdsp : public dsp {
   ...
   public:
    llvmdsp() { fDsp = newmydsp(); }

    virtual ~llvmdsp() { deletemydsp(fDsp); }

    virtual int getNumInputs() { return getNumInputsmydsp(fDsp); }
    virtual int getNumOutputs() { return getNumOutputsmydsp(fDsp); }

    static void classInit(int samplingFreq) { classInitmydsp(samplingFreq); }

    virtual void instanceInit(int samplingFreq) { instanceInitmydsp(fDsp, samplingFreq); }

    virtual void init(int samplingFreq) { initmydsp(fDsp, samplingFreq); }
