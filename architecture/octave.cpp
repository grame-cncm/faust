...
struct param {
    ...
    float  fMin;
    float  fMax;
    param(string name, float* z, float init, float a, float b) : fName(name), fVals(NULL), fZone(z), fMin(a), fMax(b)
    {
        *z = init;
    }
};
...
class FNUI : public UI {
   ...
   public:
    ...
    virtual void addButton(const char* label, float* zone) { addOption(label, zone, 0, 0, 1); }

    virtual int getNumOptions() { return numOptions; }

    virtual void addToggleButton(const char* label, float* zone) { addOption(label, zone, 0, 0, 1); }

    virtual void addCheckButton(const char* label, float* zone) { addOption(label, zone, 0, 0, 1); }
...
#define DEFAULT_BLOCKSIZE 64
    ...
    void interpolate_ctrlin(float* vals, NDArray in, int n)
{
    int    nin   = in.length();
...
DEFUN_DLD(FAUST_FUNC_NAME, args, nargout, "type " QUOTEME(FAUST_FUNC_NAME) "() to see the arguments.\n")
{
    ...
    if (nargin < numIn || nargin == 0) {
        if (numOut > 1) {
            octave_stdout << "[out1";
            for (i = 2; i <= numOut; i++) octave_stdout << ",out" << i;
        ...
        if (numIn == 0)
            ...
            octave_stdout << "in1";
        for (i = 2; i <= numIn; i++) octave_stdout << ", in" << i;
        for (i = 0; i < numOpts; i++) octave_stdout << ", " << opts[i].fName;
        octave_stdout << ")\n";
        delete interface;
        return retval;
    }
    ...
    if (numIn == 0) {
        ...
        for (i = 0; i < numIn; i++) {
            ...
            if (nr == 1) {
                if (nc > maxInputLength) maxInputLength = nc;
            } else if (nc == 1) {
                if (nr > maxInputLength) maxInputLength = nr;
            } else {
                maxInputLength = nc;
    ...
    if (allscalarctrls) {
        ...
        while (nleft > 0) {
            ...
            nleft -= n;
            k++;
            for (i = 0; i < numIn; i++) fins[i] += n;
            for (i = 0; i < numOut; i++) fouts[i] += n;
        }
        FREE(fins);
        FREE(fouts);
    }
    ...
    for (i = 0; i < numOut; i++) {
        ...
        for (int j = 0; j < maxInputLength; j++) output(0, j) = (double)foutputs[i][j];
        FREE(foutputs[i]);
        retval(i) = output;
    }
    for (i = 0; i < numOpts; i++) {
