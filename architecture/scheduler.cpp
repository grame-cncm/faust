...
struct AtomicCounter {
    ...
    INLINE AtomicCounter() { info.fValue = 0; }
...
static void get_affinity(pthread_t thread)
{
    ...
    kern_return_t          res         = thread_policy_get(pthread_mach_thread_np(thread), THREAD_AFFINITY_POLICY,
                                          (thread_policy_t)&theTCPolicy, &count, &get_default);
    if (res == KERN_SUCCESS) {
...
static void set_affinity(pthread_t thread, int tag)
{
    ...
    kern_return_t res        = thread_policy_set(pthread_mach_thread_np(thread), THREAD_AFFINITY_POLICY,
                                          (thread_policy_t)&theTCPolicy, THREAD_AFFINITY_POLICY_COUNT);
    if (res == KERN_SUCCESS) {
...
static int SetThreadToPriority(pthread_t thread, UInt32 inPriority, Boolean inIsFixed, UInt64 period,
                               UInt64 computation, UInt64 constraint)
{
    if (inPriority == 96) {
        ...
        thread_policy_set(pthread_mach_thread_np(thread), THREAD_EXTENDED_POLICY, (thread_policy_t)&theFixedPolicy,
        ...
        kern_return_t res              = thread_policy_set(pthread_mach_thread_np(thread), THREAD_PRECEDENCE_POLICY,
...
static UInt32 GetThreadPriority(pthread_t thread, int inWhichPriority)
{
    ...
    switch (threadInfo.policy) {
        case POLICY_TIMESHARE:
            ...
            thread_info(pthread_mach_thread_np(thread), THREAD_SCHED_TIMESHARE_INFO,
        ...
        case POLICY_FIFO:
            ...
            thread_info(pthread_mach_thread_np(thread), THREAD_SCHED_FIFO_INFO, (thread_info_t) & (thePolicyInfo.fifo),
                        &count);
        ...
        case POLICY_RR:
            ...
            thread_info(pthread_mach_thread_np(thread), THREAD_SCHED_RR_INFO, (thread_info_t) & (thePolicyInfo.rr),
                        &count);
...
static int GetParams(pthread_t thread, UInt64* period, UInt64* computation, UInt64* constraint)
{
    ...
    kern_return_t res = thread_policy_get(pthread_mach_thread_np(thread), THREAD_TIME_CONSTRAINT_POLICY,
                                          (thread_policy_t)&theTCPolicy, &count, &get_default);
    if (res == KERN_SUCCESS) {
...
#if defined(LLVM_50) || defined(LLVM_40) || defined(LLVM_39) || defined(LLVM_38) || defined(LLVM_37) || \
    defined(LLVM_36) || defined(LLVM_35) || defined(LLVM_34)
...
class TaskQueue {
   ...
   public:
    ...
    INLINE void Init(int task_queue_size)
    {
        ...
        int clock_per_microsec =
            (getenv("CLOCKSPERSEC") ? strtoll(getenv("CLOCKSPERSEC"), NULL, 10) : DEFAULT_CLOCKS_PER_SEC) / 1000000;

        fMaxStealing = getenv("OMP_STEALING_DUR") ? strtoll(getenv("OMP_STEALING_DUR"), NULL, 10) * clock_per_microsec
                                                  : MAX_STEAL_DUR * clock_per_microsec;
    }

    INLINE ~TaskQueue() { delete[] fTaskList; }
    ...
    INLINE void ResetStealingDur() { fStealingStart = 0; }
...
class TaskGraph {
   ...
   public:
    ...
    INLINE ~TaskGraph() { delete[] fTaskList; }

    INLINE void InitTask(int task, int val) { fTaskList[task] = val; }
...
class DSPThreadPool {
   ...
   public:
    ...
    bool IsFinished() { return (fCurThreadCount == 0); }
};
...
class WorkStealingScheduler {
   ...
   public:
    ...
    void AddReadyTask(int task_num) { fReadyTaskList[fReadyTaskListIndex++] = task_num; }

    void StartAll(void* dsp) { fThreadPool->StartAll(fStaticNumThreads - 1, true, dsp); }

    void StopAll() { fThreadPool->StopAll(); }
    ...
    void PushHead(int cur_thread, int task_num) { fTaskQueueList[cur_thread].PushHead(task_num); }

    int GetNextTask(int cur_thread) { return TaskQueue::GetNextTask(fTaskQueueList, cur_thread, fDynamicNumThreads); }

    void InitTask(int task_num, int count) { fTaskGraph->InitTask(task_num, count); }
    ...
    void GetReadyTask(int cur_thread, int* task_num) { fTaskGraph->GetReadyTask(fTaskQueueList[cur_thread], task_num); }
