...
#ifndef ${name}FXParams_H
#define ${name}FXParams_H
...
struct ${name}RTPCParams
{
    ...
    <<FOREACHPARAM:IF isRTPC==RTPC AND io_type==input: ${WwiseTypeCast} ${RTPCname}; >>
};
...
{
    ${name}FXParams();
    ${name}FXParams(const ${name}FXParams& in_rParams);
