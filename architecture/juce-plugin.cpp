...
class FaustVoice : public SynthesiserVoice, public dsp_voice {
   ...
   public:
    ...
    bool canPlaySound(SynthesiserSound* sound) override { return dynamic_cast<FaustSound*>(sound) != nullptr; }
    ...
    void renderNextBlock(AudioBuffer<FAUSTFLOAT>& outputBuffer, int startSample, int numSamples) override
    {
        // Play the voice
...
class FaustSynthesiser : public Synthesiser, public dsp_voice_group {
   ...
   public:
    ...
    void addSound(SynthesiserSound* sound) { fSynth.addSound(sound); }

    void allNotesOff(int midiChannel, bool allowTailOff) { fSynth.allNotesOff(midiChannel, allowTailOff); }

    void setCurrentPlaybackSampleRate(double newRate) { fSynth.setCurrentPlaybackSampleRate(newRate); }
...
FaustPlugInAudioProcessor::FaustPlugInAudioProcessor() : AudioProcessor(getBusesProperties()), fParameterUI(this)
{
    ...
    if (PluginHostType::getPluginLoadedAs() == wrapperType_Standalone) {
        if (gDSP.getNumInputs() == 0) {
            return BusesProperties().withOutput(
                "Output", AudioChannelSet::discreteChannels(std::min<int>(2, gDSP.getNumOutputs())), true);
        } else {
        ...
        if (gDSP.getNumInputs() == 0) {
            return BusesProperties().withOutput("Output", AudioChannelSet::discreteChannels(gDSP.getNumOutputs()),
                                                true);
        } else {
            return BusesProperties()
...
#else
    // Stereo is supported
    return ((layouts.getMainInputChannelSet().size() == 2) && (layouts.getMainOutputChannelSet().size() == 2)) ||
           ((layouts.getMainInputChannelSet().size() == fDSP->getNumInputs()) &&
            (layouts.getMainOutputChannelSet().size() == fDSP->getNumOutputs()));
#endif
...
void FaustPlugInAudioProcessor::process(AudioBuffer<FloatType>& buffer, MidiBuffer& midiMessages)
{
    ...
    fDSP->compute(-1, buffer.getNumSamples(), (FAUSTFLOAT**)buffer.getArrayOfReadPointers(),
