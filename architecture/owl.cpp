...
class OwlParameter {
   ...
   public:
    OwlParameter() : fPatch(0), fParameter(PARAMETER_A), fZone(0), fLabel(""), fMin(0), fSpan(1) {}
...
class OwlButton {
   protected:
    ...
    const char*   fLabel;   // Faust widget label
   public:
    OwlButton() : fPatch(0), fButton(PUSHBUTTON), fZone(0), fLabel("") {}
    OwlButton(const OwlButton& w) : fPatch(w.fPatch), fButton(w.fButton), fZone(w.fZone), fLabel(w.fLabel) {}
...
class OwlUI : public UI {
   ...
   public:
    ...
    virtual void addVerticalSlider(const char* label, FAUSTFLOAT* zone, FAUSTFLOAT init, FAUSTFLOAT lo, FAUSTFLOAT hi,
                                   FAUSTFLOAT step)
    {
        addOwlParameter(label, zone, lo, hi);
    }
    ...
    virtual void declare(FAUSTFLOAT* z, const char* k, const char* id)
    {
        if (strcasecmp(k, "OWL") == 0) {
            if (strncasecmp(id, "PARAMETER_", 10) == 0) id += 10;
            if (strcasecmp(id, "A") == 0)
                fParameter = PARAMETER_A;
            else if (strcasecmp(id, "B") == 0)
                fParameter = PARAMETER_B;
            else if (strcasecmp(id, "C") == 0)
                fParameter = PARAMETER_C;
            else if (strcasecmp(id, "D") == 0)
                fParameter = PARAMETER_D;
            else if (strcasecmp(id, "E") == 0)
                fParameter = PARAMETER_E;
            else if (strcasecmp(id, "F") == 0)
                fParameter = PARAMETER_F;
            else if (strcasecmp(id, "G") == 0)
                fParameter = PARAMETER_G;
            else if (strcasecmp(id, "H") == 0)
                fParameter = PARAMETER_H;
            else if (strcasecmp(id, "PUSH") == 0)
                fButton = PUSHBUTTON;
            else if (strcasecmp(id, "BYPASS") == 0)
                fButton = BYPASS_BUTTON;
        }
    }
};
...
struct OwlMemoryManager : public dsp_memory_manager {
    ...
    virtual void destroy(void* ptr) { delete (uint8_t*)ptr; }
};
...
int  owl_pushbutton(int value)
{
    ...
    if (++counter > getProgramVector()->audio_blocksize) counter = 0;
    return value;
}
}
