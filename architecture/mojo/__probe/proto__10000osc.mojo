# 10k oscillators spread from 20 to 10,020 Hz: even left, odd right.
...
comptime N_CHAN_OSCS = NUM_OSCS // 2
comptime BASE_FREQ = FaustFloat(20)
comptime FREQ_STEP = FaustFloat(10_000) / FaustFloat(NUM_OSCS - 1)
...
struct ProbeDsp(FaustDspGpu):
    ...
    def build_user_interface(mut dsp, mut ui: Some[FaustGui]) -> None:
        ui.open_vertical_box("10k spread")
        ui.open_vertical_box("Controls")
        ui.declare(dsp.shft, "unit", "Hz")
        ui.add_horizontal_slider(
            "shft", dsp.shft,
            FaustFloat(0), FaustFloat(-1000), FaustFloat(1000), FaustFloat(1)
        )
        ...
        ui.add_horizontal_slider(
            "gain", dsp.gain,
            FaustFloat(-24), FaustFloat(-60), FaustFloat(0), FaustFloat(0.5)
        )
        ui.close_box()
        ui.close_box()
    ...
    def gpu_compute(
        ...
        ctx.enqueue_function[ProbeDsp.mix_kernel](
            dsp_raw, work_buf, out_buf, count,
    ...
    def osc_kernel(
        ...
        var freq = BASE_FREQ + FaustFloat(osc) * FREQ_STEP + dsp[].shft
        if osc % 2 != 0:
            freq += dsp[].rshf
        freq = max(FaustFloat(1), freq)
        var phase = dsp[].phase[osc]
        var step = TWO_PI * freq / FaustFloat(dsp[].rate)
        var offset = osc * Int(count)

        for frame in range(Int(count)):
            samples[unsafe_offset=offset + frame] = sin(phase)
            phase += step
    ...
    def mix_kernel(
        ...
        for osc in range(0, NUM_OSCS, 2):
            left += samples[unsafe_offset=osc * frames + frame]
            right += samples[unsafe_offset=(osc + 1) * frames + frame]

        var norm = sqrt(FaustFloat(N_CHAN_OSCS))
        var amp = pow(FaustFloat(10), dsp[].gain / FaustFloat(20)) / norm
        output[unsafe_offset=frame] = amp * left
        output[unsafe_offset=frames + frame] = amp * right
