# 100 GPU oscillators: 50 at a fixed pitch and 50 with a GUI-controlled shift.
...
comptime NUM_FIXED_OSCS = NUM_OSCS // 2
comptime BASE_FREQ = FaustFloat(80)
comptime TWO_PI = FaustFloat(6.283185307179586)
comptime GAIN = FaustFloat(0.002)
comptime OSC_BLOCK_SIZE = 128
...
struct ProbeDsp(FaustDspGpu):
    ...
    def build_user_interface(mut dsp, mut ui: Some[FaustGui]) -> None:
        ...
        ui.declare(dsp.shift, "unit", "Hz")
        ui.add_horizontal_slider(
            "shift", dsp.shift,
            FaustFloat(0), FaustFloat(-10), FaustFloat(10), FaustFloat(0.1)
        )
        ui.close_box()
