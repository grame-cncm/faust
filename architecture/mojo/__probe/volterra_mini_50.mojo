...
struct mydsp(FaustDsp):
    ...
    def get_json(read dsp) -> String:
        return "{\"name\": \"volterra_mini_50\",\"filename\": \"volterra_mini_50.dsp\",\"version\": \"2.85.5\",\"compile_options\": \"-a inspect.mojo -lang mojo -fpga-mem-th 4 -ct 1 -es 1 -mcd 16 -mdd 1024 -mdy 33 -double -ftz 0\",\"library_list\": [\"/usr/local/share/faust/oscillator.lib\",\"/usr/local/share/faust/math.lib\",\"/usr/local/share/faust/music.lib\"],\"include_pathnames\": [\"/Users/manuelfarzini/Personal/dev/repo/faust/build/share/faust\",\"/usr/local/share/faust\",\"/usr/share/faust\",\"../_bench/src\",\"/Users/manuelfarzini/Personal/dev/repo/faust/architecture/mojo/../_bench/src\"],\"size\": 65776,\"inputs\": 0,\"outputs\": 1,\"meta\": [ { \"compile_options\": \"-a inspect.mojo -lang mojo -fpga-mem-th 4 -ct 1 -es 1 -mcd 16 -mdd 1024 -mdy 33 -double -ftz 0\" },{ \"filename\": \"volterra_mini_50.dsp\" },{ \"math.lib/author\": \"GRAME\" },{ \"math.lib/copyright\": \"GRAME\" },{ \"math.lib/deprecated\": \"This library is deprecated and is not maintained anymore. It will be removed in August 2017.\" },{ \"math.lib/license\": \"LGPL with exception\" },{ \"math.lib/name\": \"Math Library\" },{ \"math.lib/version\": \"1.0\" },{ \"music.lib/author\": \"GRAME\" },{ \"music.lib/copyright\": \"GRAME\" },{ \"music.lib/deprecated\": \"This library is deprecated and is not maintained anymore. It will be removed in August 2017.\" },{ \"music.lib/license\": \"LGPL with exception\" },{ \"music.lib/name\": \"Music Library\" },{ \"music.lib/version\": \"1.0\" },{ \"name\": \"volterra_mini_50\" },{ \"oscillator.lib/author\": \"Julius O. Smith (jos at ccrma.stanford.edu)\" },{ \"oscillator.lib/copyright\": \"Julius O. Smith III\" },{ \"oscillator.lib/deprecated\": \"This library is deprecated and is not maintained anymore. It will be removed in August 2017.\" },{ \"oscillator.lib/license\": \"STK-4.3\" },{ \"oscillator.lib/name\": \"Faust Oscillator Library\" },{ \"oscillator.lib/version\": \"1.11\" }],\"ui\": [ {\"type\": \"vgroup\",\"label\": \"volterra_mini_50\",\"items\": [ {\"type\": \"hslider\",\"label\": \"delta\",\"varname\": \"fHslider2\",\"shortname\": \"delta\",\"address\": \"/volterra_mini_50/delta\",\"init\": 2,\"min\": 0,\"max\": 6,\"step\": 0.1},{\"type\": \"hslider\",\"label\": \"freq carre\",\"varname\": \"fHslider1\",\"shortname\": \"freq_carre\",\"address\": \"/volterra_mini_50/freq_carre\",\"init\": 440,\"min\": 20,\"max\": 8000,\"step\": 1},{\"type\": \"hslider\",\"label\": \"freq\",\"varname\": \"fHslider0\",\"shortname\": \"freq\",\"address\": \"/volterra_mini_50/freq\",\"meta\": [{ \"unit\": \"Hz\" }],\"init\": 700,\"min\": 1,\"max\": 20000,\"step\": 1}]}]}"
    ...
    def metadata(read dsp, mut meta: Some[FaustMeta]) -> None:
        ...
        meta.declare("filename", "volterra_mini_50.dsp")
        ...
        meta.declare("name", "volterra_mini_50")
    ...
    def build_user_interface(mut dsp, mut ui: Some[FaustGui]) -> None:
        ui.open_vertical_box("volterra_mini_50")
        ui.add_horizontal_slider("delta", dsp.hslider2, FaustFloat(2.0), FaustFloat(0.0), FaustFloat(6.0), FaustFloat(0.1))
        ui.add_horizontal_slider("freq carre", dsp.hslider1, FaustFloat(440.0), FaustFloat(20.0), FaustFloat(8000.0), FaustFloat(1.0))
    ...
    def compute(
        ...
        var slow1 = (1.0) / ((slow0) + (1.0))
        var slow2 = F64(dsp.hslider1)
        var slow3 = max(slow2, 23.44894968246214)
        var slow4 = max(2e+01, abs(slow3))
        var slow5 = (1.0) / (slow4)
        var slow6 = (dsp.const3) * (slow4)
        var slow7 = max((slow2) + (F64(dsp.hslider2)), 23.44894968246214)
        var slow8 = max(2e+01, abs(slow7))
        var slow9 = (1.0) / (slow8)
        var slow10 = (dsp.const3) * (slow8)
        var slow11 = max(0.0, min(2047.0, (dsp.const4) / (slow7)))
        var i_slow12 = S32(slow11)
        var slow13 = F64(i_slow12)
        var slow14 = (slow11) - (slow13)
        var slow15 = (dsp.const2) / (slow8)
        var i_slow16 = (i_slow12) + (S32(1))
        var slow17 = max(0.0, min(2047.0, (dsp.const4) / (slow3)))
        var i_slow18 = S32(slow17)
        var slow19 = F64(i_slow18)
        var slow20 = (slow19) + ((1.0) - (slow17))
        var slow21 = (dsp.const2) / (slow4)
        var slow22 = (slow17) - (slow19)
        var i_slow23 = (i_slow18) + (S32(1))
        var slow24 = (slow13) + ((1.0) - (slow11))

        for var i0 in range(S32(0), count):
            dsp.i_vec0[S32(0)] = S32(1)
            var temp0 = F64(dsp.i_vec0[S32(1)])
            dsp.rec3[S32(0)] = (slow6) + ((dsp.rec3[S32(1)]) - (floor((slow6) + (dsp.rec3[S32(1)]))))
            var temp1 = pow_unrolled[2](((2.0) * (dsp.rec3[S32(0)])) + (-1.0))
            dsp.vec1[S32(0)] = temp1
            var temp2 = (temp1) - (dsp.vec1[S32(1)])
            dsp.rec4[S32(0)] = (slow10) + ((dsp.rec4[S32(1)]) - (floor((slow10) + (dsp.rec4[S32(1)]))))
            var temp3 = pow_unrolled[2](((2.0) * (dsp.rec4[S32(0)])) + (-1.0))
            dsp.vec2[S32(0)] = temp3
            var temp4 = (temp3) - (dsp.vec2[S32(1)])
            var temp5 = (slow15) * ((temp0) * (temp4))
            dsp.vec3[(dsp.iota0) & (S32(4095))] = temp5
            var temp6 = (slow21) * ((temp0) * (temp2))
            dsp.vec4[(dsp.iota0) & (S32(4095))] = temp6
            var temp7 = ((dsp.const2) * ((temp0) * (((slow5) * (temp2)) + ((slow9) * (temp4))))) - (((slow14) * (dsp.vec3[((dsp.iota0) - (i_slow16)) & (S32(4095))])) + ((((slow20) * (dsp.vec4[((dsp.iota0) - (i_slow18)) & (S32(4095))])) + ((slow22) * (dsp.vec4[((dsp.iota0) - (i_slow23)) & (S32(4095))]))) + ((slow24) * (dsp.vec3[((dsp.iota0) - (i_slow12)) & (S32(4095))]))))
            dsp.rec2[S32(0)] = (slow1) * ((dsp.rec2[S32(1)]) + ((slow0) * (temp7)))
            dsp.rec1[S32(0)] = (slow1) * ((dsp.rec1[S32(1)]) + ((slow0) * (dsp.rec2[S32(0)])))
            dsp.rec0[S32(0)] = (slow1) * ((dsp.rec0[S32(1)]) + ((slow0) * (dsp.rec1[S32(0)])))
            var temp8 = pow_unrolled[3](dsp.rec2[S32(0)])
            dsp.rec7[S32(0)] = (slow1) * ((dsp.rec7[S32(1)]) + ((slow0) * ((pow_unrolled[3](temp7)) - (temp8))))
            var temp9 = pow_unrolled[3](dsp.rec1[S32(0)])
            dsp.rec6[S32(0)] = (slow1) * ((dsp.rec6[S32(1)]) + ((slow0) * (((dsp.rec7[S32(0)]) + (temp8)) - (temp9))))
            dsp.rec5[S32(0)] = (slow1) * ((dsp.rec5[S32(1)]) + ((slow0) * (((dsp.rec6[S32(0)]) + (temp9)) - (pow_unrolled[3](dsp.rec0[S32(0)])))))
