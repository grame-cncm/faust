...
struct mydsp(FaustDsp):
    ...
    def compute(
        ...
        while (vindex_re0) <= ((count) - (S32(32))): 
            ...
            # Vectorizable loop 0 
            # Compute code 
            for var i_re0 in range(S32(S32(0)), vsize_re0): 
                zec0[i_re0] = (F64(input0[i_re0])) * ((1.0) - ((slow0) * (pow_unrolled[2](F64(input0[i_re0])))))
            # Vectorizable loop 1 
            # Compute code 
            for var i_re1 in range(S32(S32(0)), vsize_re0): 
                output0[i_re1] = FaustFloat(zec0[i_re1])
            # Vectorizable loop 2 
            # Compute code 
            for var i_re2 in range(S32(S32(0)), vsize_re0): 
        ...
        if ((vindex_re0) < (count) != 0):
            ...
            # Vectorizable loop 0 
            # Compute code 
            for var i_re3 in range(S32(S32(0)), vsize_re1): 
                zec0[i_re3] = (F64(input0[i_re3])) * ((1.0) - ((slow0) * (pow_unrolled[2](F64(input0[i_re3])))))
            # Vectorizable loop 1 
            # Compute code 
            for var i_re4 in range(S32(S32(0)), vsize_re1): 
                output0[i_re4] = FaustFloat(zec0[i_re4])
            # Vectorizable loop 2 
            # Compute code 
            for var i_re5 in range(S32(S32(0)), vsize_re1): 
