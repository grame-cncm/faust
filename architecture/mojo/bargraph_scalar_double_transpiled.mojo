...
struct mydsp(FaustDsp):
    ...
    def compute(
        ...
        for var i0 in range(S32(0), count):
            ...
            output0[i0] = FaustFloat(F64(dsp.hbargraph0))
            dsp.vbargraph0 = FaustFloat(((slow2) * (temp0)) - (slow0))
            output1[i0] = FaustFloat(F64(dsp.vbargraph0))
            dsp.hbargraph1 = FaustFloat((slow3) * (temp0))
            output2[i0] = FaustFloat(F64(dsp.hbargraph1))
            dsp.vbargraph1 = FaustFloat((slow6) * (temp0))
            output3[i0] = FaustFloat(F64(dsp.vbargraph1))
