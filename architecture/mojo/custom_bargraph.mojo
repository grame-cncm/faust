...
struct mydsp(FaustDsp):
    ...
    def compute(mut dsp, var count: S32, var inputs: ImmStreams, var outputs: MutStreams) -> None:
        ...
        while vindex <= end:
            ...
            dsp.hbargraph0 = output0[wfaust - 1]
            vstore(output1, (((slow2) * ((vload(input0)).cast[f64]())) - (slow0)).cast[dfaust]())
            dsp.vbargraph0 = output1[wfaust - 1]
            vstore(output2, ((slow3) * ((vload(input0)).cast[f64]())).cast[dfaust]())
            dsp.hbargraph1 = output2[wfaust - 1]
            vstore(output3, ((slow6) * ((vload(input0)).cast[f64]())).cast[dfaust]())
            dsp.vbargraph1 = output3[wfaust - 1]
            lo = ((slow7) * ((slow0) + ((vload[W](input0)).cast[f64]()))).cast[dfaust]()
            hi = ((slow7) * ((slow0) + ((vload[W](input0, wsize)).cast[f64]()))).cast[dfaust]()
            vstore(output4, lo.join(hi))
            lo = ((slow8) * (((vload[W](input0)).cast[f64]()) - (slow0))).cast[dfaust]()
            hi = ((slow8) * (((vload[W](input0, wsize)).cast[f64]()) - (slow0))).cast[dfaust]()
            vstore(output5, lo.join(hi))
            lo = ((slow9) * ((vload[W](input0)).cast[f64]())).cast[dfaust]()
            hi = ((slow9) * ((vload[W](input0, wsize)).cast[f64]())).cast[dfaust]()
            vstore(output6, lo.join(hi))
            lo = ((slow10) * ((vload[W](input0)).cast[f64]())).cast[dfaust]()
            hi = ((slow10) * ((vload[W](input0, wsize)).cast[f64]())).cast[dfaust]()
            vstore(output7, lo.join(hi))
            vindex += vsize
