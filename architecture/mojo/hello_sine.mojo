...
# name: "hello_sine"
# Compilation options: 
#   -a bench.mojo -lang mojo -fpga-mem-th 4 -ct 1 -cn HelloSine -es 1 -mcd 16 
...
========== dump2FIR 0xc12c351d0 statement begin ========== 
BlockInst 
	StoreVarInst(Address(fSampleRate, kStruct), LoadVarInst(Address(sample_rate, kFunArgs)))
	StoreVarInst(Address(fConst0, kStruct), BinopInst("/", Double(1.0), FunCallInst("min_", Double(1.92e+05), FunCallInst("max_", Double(1.0), CastInst("double", LoadVarInst(Address(fSampleRate, kStruct)))))))
EndBlockInst
========== dump2FIR statement end ==========
# ------------------------------------------------------------------------------
dsp.sample_rate = sample_rate
        dsp.const0 = (1.0) / (min(1.92e+05, max(1.0, F64(dsp.sample_rate))))
    ...
    def get_json(read dsp) -> String:
        return "{\"name\": \"hello_sine\",\"filename\": \"hello_sine.dsp\",\"version\": \"2.85.5\",\"compile_options\": \"-a bench.mojo -lang mojo -fpga-mem-th 4 -ct 1 -cn HelloSine -es 1 -mcd 16 -mdd 1024 -mdy 33 -double -ftz 0\",\"library_list\": [\"/usr/local/share/faust/stdfaust.lib\",\"/usr/local/share/faust/maths.lib\",\"/usr/local/share/faust/platform.lib\"],\"include_pathnames\": [\"/Users/manuelfarzini/Personal/dev/repo/faust/build/share/faust\",\"/usr/local/share/faust\",\"/usr/share/faust\",\"src\",\"/Users/manuelfarzini/Personal/dev/repo/faust/architecture/mojo/src\"],\"size\": 36,\"inputs\": 0,\"outputs\": 1,\"meta\": [ { \"compile_options\": \"-a bench.mojo -lang mojo -fpga-mem-th 4 -ct 1 -cn HelloSine -es 1 -mcd 16 -mdd 1024 -mdy 33 -double -ftz 0\" },{ \"filename\": \"hello_sine.dsp\" },{ \"maths.lib/author\": \"GRAME\" },{ \"maths.lib/copyright\": \"GRAME\" },{ \"maths.lib/license\": \"LGPL with exception\" },{ \"maths.lib/name\": \"Faust Math Library\" },{ \"maths.lib/version\": \"2.9.0\" },{ \"name\": \"hello_sine\" },{ \"platform.lib/name\": \"Generic Platform Library\" },{ \"platform.lib/version\": \"1.3.0\" }],\"ui\": [ {\"type\": \"vgroup\",\"label\": \"hello_sine\",\"items\": [ {\"type\": \"hslider\",\"label\": \"freq\",\"varname\": \"fHslider0\",\"shortname\": \"freq\",\"address\": \"/hello_sine/freq\",\"init\": 440,\"min\": 20,\"max\": 2000,\"step\": 1}]}]}"

    @always_inline
    def metadata(read dsp, mut meta: Some[FaustMeta]) -> None:
        meta.declare("compile_options", "-a bench.mojo -lang mojo -fpga-mem-th 4 -ct 1 -cn HelloSine -es 1 -mcd 16 -mdd 1024 -mdy 33 -double -ftz 0")
    ...
    def compute[dreal: DType](
        ...
        for var i0 in range(S32(0), count):
            dsp.rec0[S32(0)] = (slow0) + ((dsp.rec0[S32(1)]) - (F64(S32((slow0) + (dsp.rec0[S32(1)])))))
            output0[i0] = SIMD[dreal, 1](sin((6.283185307179586) * (dsp.rec0[S32(0)])))
