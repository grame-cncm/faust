...
struct BridgeGui(FaustGui):
    ...
    @always_inline
    def _add_zone[dreal: DType](
        mut ui, mut zone: SIMD[dreal, 1], passive: Bool = False
    ) -> None:
        comptime assert dreal == dfaust
        var host = Ptr(to=zone).unsafe_bitcast[FaustFloat]().unsafe_origin_cast[MUT_NOTRK]()
        for item in ui.zones:
            if item.host == host:
                # Aliases share one transfer; mixed directions are ambiguous.
                ui.valid = ui.valid and item.passive == passive
                return
        var offset = pointer_to_int(host) - pointer_to_int(ui.dsp.unsafe_value())
        ui.zones.append(BridgeZone(host, offset, host[], passive))
