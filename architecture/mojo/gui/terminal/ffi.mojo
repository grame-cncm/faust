...
def error_str(err: S32) -> String:
    return String(
            external_call["error_str", CStringSlice[IMM_NOTRK]](err)
        )
...
def gui_add_check_button(ui: Handle, var lbl: String, init: F64, mut id: S32) -> S32:
    return  external_call[
                "gui_add_check_button", S32
            ](ui, lbl.as_c_string_slice().ptr(), init, Ptr(to=id))

def gui_add_slider(
    ui: Handle, var lbl: String, init: F64, min: F64, max: F64, step: F64, mut id: S32
) -> S32:
    return  external_call[
                "gui_add_slider", S32
            ](ui, lbl.as_c_string_slice().ptr(), init, min, max, step, Ptr(to=id))

def gui_add_num_entry(
    ui: Handle, var lbl: String, init: F64, min: F64, max: F64, step: F64, mut id: S32
) -> S32:
    return  external_call[
                "gui_add_num_entry", S32
            ](ui, lbl.as_c_string_slice().ptr(), init, min, max, step, Ptr(to=id))

def gui_add_bargraph(
    ui: Handle, var lbl: String, min: F64, max: F64, mut id: S32
) -> S32:
    return  external_call[
                "gui_add_bargraph", S32
            ](ui, lbl.as_c_string_slice().ptr(), min, max, Ptr(to=id))
