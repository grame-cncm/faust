...
#ifndef mj_ticks_ms
#if MJ_SYSTEM_WIN
#define mj_ticks_ms() cast(u64, GetTickCount64())
#else
static u64 ticks_ms(void)
{
    struct timespec now;
    if (clock_gettime(CLOCK_MONOTONIC, &now) != 0) {
        mj_panic("monotonic clock unavailable");
    }
    return cast(u64, now.tv_sec) * 1000u + cast(u64, now.tv_nsec) / 1000000u;
}
#define mj_ticks_ms() ticks_ms()
#endif
#endif
...
// All Faust groups retain their hierarchy and always stack their children.
...
static ErrorCode gui_render(Gui* ui)
{
    ...
    ui->cols = min_int(ui->term.cols > 1 ? ui->term.cols - 1 : 1, min_int(GUI_WIDTH, MJ_VIEW_COLS));
    ui->margin = ui->term.cols > ui->cols ? (ui->term.cols - ui->cols) / 2 : 0;
    ...
    s32 pad = 2;
    ui->height = layout(ui, -1, root ? 0 : 3, pad + (root ? 0 : 2),
        root ? ui->cols : (ui->cols > 6 ? ui->cols - 6 : 0)) + pad + (root ? 0 : 4);
    s32 last = ui->height > ui->rows - 1 ? ui->height - (ui->rows - 1) : 0;
    ui->top = min_int(ui->top, last);
    mj_assert(ui->top >= 0);
    if (!root) {
        box_put(ui, (Rect){ 0, pad - ui->top, ui->cols, ui->height - pad }, "DSP");
    }
    for (s32 i = 0; i < ui->len; ++i) {
        Widget* wdg = &ui->wdgs[i];
        if (wdg->kind >= KIND_BOX) {
    ...
    buf_put(ui, "\033[H", 3);
    for (s32 y = 0; y < ui->rows; ++y) {
        // Clear both old margins after a resize, then position without padding
        // the frame buffer to the full width of a potentially huge terminal.
        char pos[32];
        s32 len = snprintf(pos, sizeof(pos), "\033[2K\033[%dG", cast(int, ui->margin + 1));
        mj_assert(len > 0 && cast(usize, len) < sizeof(pos));
        buf_put(ui, pos, len);
        for (s32 x = 0; x < ui->cols; ++x) {
...
static void gui_mouse(Gui* ui, Input in, Event* out)
{
    ...
    if (in.kind == INPUT_UP || in.kind == INPUT_CANCEL) {
        ui->click_ready = in.kind == INPUT_UP && ui->press >= 0 && ui->click_id == ui->press &&
    ...
    if (in.kind == INPUT_DOWN) {
        ...
        if (ui->press >= 0) {
            ...
            b32 twice = ui->click_ready && ui->click_id == ui->press &&
                ...
                abs(in.x - ui->click_x) <= CLICK_SLOP && in.y == ui->click_y;
            ui->click_ready = 0;
            ui->click_id = wdg->kind == WIDGET_BUTTON ? -1 : ui->press;
            ui->click_x = in.x;
            ui->click_y = in.y;
            ui->click_at = now;
            if (twice) {
    ...
    // Keep the restored default until the second click is released.
    if (ui->reset) {
        return;
    }
