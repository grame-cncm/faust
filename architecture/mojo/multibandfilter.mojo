...
struct mydsp(FaustDsp):
    ...
    def compute[dreal: DType](
        ...
        var input0 = inputs[S32(0)].as_noalias_ptr()
...
def main() -> None:
    ...
    if err:
        dsp.free()
        return
    var ptr = base.unsafe_value().as_noalias_ptr()
    var inputs = ptr.bitcast[Ptr[Real, READ_EXT]]().as_immutable().as_noalias_ptr()
    var outputs = (ptr + n_ins).bitcast[Ptr[Real, MUTA_EXT]]().as_noalias_ptr()
