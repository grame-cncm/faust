...
Compilation options: -lang cpp -fpga-mem-th 4 -ct 1 -es 1 -mcd 4 -mdd 1024 -mdy 33 -double -ftz 0 -vec -lv 2 -vs 4 -dfs
------------------------------------------------------------ */
...
class mydsp : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		m->declare("compile_options", "-lang cpp -fpga-mem-th 4 -ct 1 -es 1 -mcd 4 -mdd 1024 -mdy 33 -double -ftz 0 -vec -lv 2 -vs 4 -dfs");
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		/* Compute code */
		for (int i = 0; i < 4; i = faust_wrap_add(i, 1)) {
		...
		for (int j1 = 0; j1 < 4; j1 = faust_wrap_add(j1, 1)) {
			fRec0_perm[j1] = fRec0_tmp[faust_wrap_add(4, j1)];
		}
		/* Recursive loop 1 */
		/* Pre code */
		...
		for (int j3 = 0; j3 < 4; j3 = faust_wrap_add(j3, 1)) {
			fRec1_perm[j3] = fRec1_tmp[faust_wrap_add(4, j3)];
		}
		/* Vectorizable loop 2 */
		/* Pre code */
