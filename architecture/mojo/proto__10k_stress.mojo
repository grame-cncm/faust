...
struct ProbeDsp(FaustDspGpu):
    ...
    def build_user_interface(mut dsp, mut ui: Some[FaustGui]) -> None:
        ...
        ui.add_horizontal_slider(
            ...
            BASE_FREQ, FaustFloat(60.0), FaustFloat(1000.0), FaustFloat(1.0)
        )
        ui.close_box()
    ...
    def _compute_osc(
        ...
        var step = TWO_PI * dsp.frequency / FaustFloat(dsp.sample_rate)
        var offset = osc * Int(buf_size)

        for frame in range(Int(buf_size)):
            var sample = FaustFloat(0.0)
            for partial in range(1, NUM_PARTIALS + 1):
                var harmonic = FaustFloat(partial) * phase
                sample += sin(harmonic.cast[f32]()).cast[dfaust]() / FaustFloat(partial)
...
comptime BASE_FREQ = FaustFloat(220.0)
