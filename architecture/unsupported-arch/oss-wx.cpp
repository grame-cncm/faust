...
struct AudioParam {
    ...
    AudioParam& device(const char* n)
    {
        fDeviceName = n;
        return *this;
    }
    ...
    {
        fSamplingFrequency = f;
        return *this;
    }
    AudioParam& mode(int m)
    {
        fRWMode = m;
        return *this;
    }
    AudioParam& format(int f)
    {
        fSampleFormat = f;
        return *this;
    }
...
class AudioInterface {
   ...
   public:
    ...
    {
        int bytes = frames * 2 * fNumOfInputChannels;
        assert(bytes <= fInputBufferSize);
    ...
    {
        int bytes = frames * 2 * fNumOfOutputChannels;
        assert(bytes <= fOutputBufferSize);
...
class faustButton : public wxButton {
   ...
   public:
    ...
    void clickdown(wxCommandEvent& ev)
    {
        *fZone = 1.0; /*printf("click down : zone (at %p) = %f\n", fZone, *fZone);*/
        ev.Skip();
    }
    void clickup(wxCommandEvent& ev)
    {
        *fZone = 0.0; /*printf("click up : zone (at %p) = %f\n", fZone, *fZone);*/
        ev.Skip();
    }
...
class faustHorizontalSlider : public wxSlider {
   ...
   public:
    faustHorizontalSlider(wxWindow* parent, float* zone, float init, float min, float max, float step)
        : wxSlider(parent, -1, int(init / step), int(min / step), int(max / step), wxDefaultPosition, wxSize(120, 30),
                   wxSL_HORIZONTAL),
          fStep(step),
          fZone(zone)
    {
...
class faustVerticalSlider : public wxSlider {
   ...
   public:
    faustVerticalSlider(wxWindow* parent, float* zone, float init, float min, float max, float step)
        : wxSlider(parent, -1, int((max + min - init) / step), int(min / step), int(max / step), wxDefaultPosition,
...
class faustSpinCtrl : public wxSpinCtrl {
   ...
   public:
    ...
    {
        *fZone = GetValue();
        printf("spin ctrl update : zone (at %p) = %f\n", fZone, *fZone);
    }
...
class MyFrame : public wxFrame {
   public:
    MyFrame(const wxString& title, const wxPoint& pos, const wxSize& size) : wxFrame(0, -1, title, pos, size)
    {
        wxMenu* m = new wxMenu;
    ...
    void OnAbout(wxCommandEvent& event) { wxMessageBox("message 1", "message 2", wxOK | wxICON_INFORMATION); }
...
bool MyApp::OnInit()
{
    ...
    AudioInterface* audio = new AudioInterface(
        AudioParam().frequency(lopt(argv, "--frequency", 44100)).buffering(lopt(argv, "--buffer", 128))
