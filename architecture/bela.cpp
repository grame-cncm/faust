...
const char* const pinNamesStrings[] = {"ANALOG_0",   "ANALOG_1",   "ANALOG_2",   "ANALOG_3",   "ANALOG_4",
                                       "ANALOG_5",   "ANALOG_6",   "ANALOG_7",   "ANALOG_8",   "DIGITAL_0",
                                       "DIGITAL_1",  "DIGITAL_2",  "DIGITAL_3",  "DIGITAL_4",  "DIGITAL_5",
                                       "DIGITAL_6",  "DIGITAL_7",  "DIGITAL_8",  "DIGITAL_9",  "DIGITAL_10",
                                       "DIGITAL_11", "DIGITAL_12", "DIGITAL_13", "DIGITAL_14", "DIGITAL_15"};
...
class BelaWidget {
   ...
   public:
    BelaWidget() : fBelaPin(kNoPin), fZone(0), fLabel(""), fMin(0), fRange(1) {}

    BelaWidget(const BelaWidget& w)
        : fBelaPin(w.fBelaPin), fZone(w.fZone), fLabel(w.fLabel), fMin(w.fMin), fRange(w.fRange)
    {
    }

    BelaWidget(EInputPin pin, FAUSTFLOAT* z, const char* l, FAUSTFLOAT lo, FAUSTFLOAT hi)
        : fBelaPin(pin), fZone(z), fLabel(l), fMin(lo), fRange(hi - lo)
    {
    }
    ...
    void update(BelaContext* context)
    {
        switch (fBelaPin) {
            ...
            case kDIGITAL_14:
            case kDIGITAL_15:
                *fZone = digitalRead(context, 0 /* TODO: average frame?*/, ((int)fBelaPin - kDIGITAL_0)) > 0
                             ? fMin
                             : fMin + fRange;
                break;
            default:
...
class BelaUI : public UI {
    ...
    void skip() { fBelaPin = kNoPin; }

   public:
    BelaUI() : fIndex(0), fBelaPin(kNoPin) {}
    ...
    void update(BelaContext* context)
    {
        for (int i = 0; i < fIndex; i++) fTable[i].update(context);
    }
    ...
    virtual void addVerticalSlider(const char* label, FAUSTFLOAT* zone, FAUSTFLOAT init, FAUSTFLOAT lo, FAUSTFLOAT hi,
                                   FAUSTFLOAT step)
    {
        addBelaWidget(label, zone, lo, hi);
    }
    virtual void addHorizontalSlider(const char* label, FAUSTFLOAT* zone, FAUSTFLOAT init, FAUSTFLOAT lo, FAUSTFLOAT hi,
                                     FAUSTFLOAT step)
    {
        addBelaWidget(label, zone, lo, hi);
    }
    virtual void addNumEntry(const char* label, FAUSTFLOAT* zone, FAUSTFLOAT init, FAUSTFLOAT lo, FAUSTFLOAT hi,
                             FAUSTFLOAT step)
    {
        addBelaWidget(label, zone, lo, hi);
    }
...
FAUSTFLOAT * gInputBuffers = NULL,
           *gOutputBuffers = NULL;  // de-interleaved input/output buffers for the audio and analog inputs
...
{
    gNumBuffers =
        context->audioInChannels + context->audioOutChannels + context->analogInChannels + context->analogOutChannels;
    ...
    for (int ch = 0; ch < fDSP.getNumInputs(); ++ch) gFaustIns[ch] = gInputBuffers + (ch * context->audioFrames);

    // create the table of output channels
    for (int ch = 0; ch < fDSP.getNumOutputs(); ++ch) gFaustOuts[ch] = gOutputBuffers + (ch * context->audioFrames);
...
void render(BelaContext* context, void* userData)
{
    ...
    for (unsigned int frame = 0; frame < context->audioFrames; frame++) {
        for (unsigned int ch = 0; ch < gNumBuffers; ch++) {
            if (ch >= context->audioInChannels + context->analogInChannels) break;
            if (ch >= context->audioInChannels)  // handle analogChannels
            {
                unsigned int m = frame / 2;
                FAUSTFLOAT   mIn =
                    (FAUSTFLOAT)context->analogIn[m * context->analogInChannels + (ch - context->audioInChannels)];
                gInputBuffers[ch * context->audioFrames + frame] = mIn;
            } else  // handle audioInChannels
            {
                gInputBuffers[ch * context->audioFrames + frame] =
                    context->audioIn[frame * context->audioInChannels + ch];
            }
        }
    }
    ...
    for (unsigned int frame = 0; frame < context->audioFrames; frame++) {
        for (unsigned int ch = 0; ch < gNumBuffers; ch++) {
            if (ch >= context->audioOutChannels + context->analogOutChannels)
                ...
                {
                    unsigned int m = frame / 2;
                    context->analogOut[m * context->analogFrames + (ch - context->audioOutChannels)] =
                        gOutputBuffers[ch * context->audioFrames + frame];
                } else  // handle audioChannels
                {
                    context->audioOut[frame * context->audioOutChannels + ch] =
                        gOutputBuffers[ch * context->audioFrames + frame];
                }
            }
        }
...
{
    if (gInputBuffers != NULL) free(gInputBuffers);
    if (gOutputBuffers != NULL) free(gOutputBuffers);
