...
{
    for (int i = 0; i < argc - 1; i++) {
        if (option == argv[i]) return argv[i + 1];
    }
    return defaultValue;
}
...
//--------------------------------------------------------------------------
float OSCControler::version()
{
    return kVersion;
}
const char* OSCControler::versionstr()
{
    return kVersionStr;
}
...
void OSCControler::run()
{
    ...
    if (rootnode) {
        ...
        oscout << OSCStart("Faust OSC version") << versionstr() << "-" << quote(rootnode->getName()).c_str()
               << "is running on UDP ports " << fUDPPort << fUDPOut << fUPDErr;

        // and also on the standard output
        cout << "Faust OSC version " << versionstr() << " application " << quote(rootnode->getName()).c_str()
             << " is running on UDP ports " << fUDPPort << ", " << fUDPOut << ", " << fUPDErr << endl;
...
//--------------------------------------------------------------------------
const char* OSCControler::getRootName() const
{
    return fFactory->root()->getName();
}
...
bool OSCControler::isPathFiltered(std::string path)
{
    for (size_t i = 0; i < fFilteredPaths.size(); i++) {
        if (fFilteredPaths[i]->match(path.c_str())) return true;
    }
    return false;
}
