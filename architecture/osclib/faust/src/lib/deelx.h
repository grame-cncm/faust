...
CBufferRefT<ELT>::CBufferRefT(const ELT *pcsz)
{
    ...
    if (pcsz != 0)
        while (m_pRef[m_nSize] != 0) m_nSize++;
}

template <class ELT>
int CBufferRefT<ELT>::nCompare(const ELT *pcsz) const
{
    for (int i = 0; i < m_nSize; i++) {
        if (m_pRef[i] != pcsz[i]) return m_pRef[i] - pcsz[i];
    }
...
template <class ELT>
int CBufferRefT<ELT>::nCompareNoCase(const ELT *pcsz) const
{
    for (int i = 0; i < m_nSize; i++) {
        if (m_pRef[i] != pcsz[i]) {
            if (toupper((int)m_pRef[i]) != toupper((int)pcsz[i])) return m_pRef[i] - pcsz[i];
        }
    }
...
template <class ELT>
inline int CBufferRefT<ELT>::Compare(const ELT *pcsz) const
{
    return nCompare(pcsz) ? 1 : (int)pcsz[m_nSize];
}

template <class ELT>
inline int CBufferRefT<ELT>::CompareNoCase(const ELT *pcsz) const
{
    return nCompareNoCase(pcsz) ? 1 : (int)pcsz[m_nSize];
}

template <class ELT>
inline int CBufferRefT<ELT>::Compare(const CBufferRefT<ELT> &cref) const
{
    return m_nSize == cref.m_nSize ? nCompare(cref.GetBuffer()) : 1;
}

template <class ELT>
inline int CBufferRefT<ELT>::CompareNoCase(const CBufferRefT<ELT> &cref) const
{
...
//
template <class ELT>
CBufferT<ELT>::CBufferT(const ELT *pcsz, int length) : CBufferRefT<ELT>(0, length)
{
...
template <class ELT>
CBufferT<ELT>::CBufferT(const ELT *pcsz) : CBufferRefT<ELT>(pcsz)
{
    m_nMaxLength = CBufferRefT<ELT>::m_nSize + 1;
...
template <class ELT>
CBufferT<ELT>::CBufferT() : CBufferRefT<ELT>(0, 0)
{
    m_nMaxLength = 0;
    m_pBuffer    = 0;
}
...
template <class ELT>
inline int CBufferT<ELT>::Pop(ELT &el)
{
...
template <class ELT>
inline int CBufferT<ELT>::Peek(ELT &el) const
{
    if (CBufferRefT<ELT>::m_nSize > 0) {
...
{
    static const ELT _def[] = {0};
    return m_pBuffer ? m_pBuffer : (ELT *)_def;
}
...
template <class CHART>
int CBackrefElxT<CHART>::Match(CContext *pContext) const
{
...
template <class CHART>
int CBackrefElxT<CHART>::MatchNext(CContext *pContext) const
{
    int npos = 0;
...
template <class CHART>
int CBoundaryElxT<CHART>::Match(CContext *pContext) const
{
    const CHART *pcsz = (const CHART *)pContext->m_pMatchString;
    int          npos = pContext->m_nCurrentPos;
    int          tlen = pContext->m_pMatchStringLength;
...
template <class CHART>
int CBoundaryElxT<CHART>::MatchNext(CContext *) const
{
    return 0;
}

template <class CHART>
inline int CBoundaryElxT<CHART>::IsWordChar(CHART ch)
{
...
template <class CHART>
int CBracketElxT<CHART>::Match(CContext *pContext) const
{
    // check, for named
...
template <class CHART>
int CBracketElxT<CHART>::MatchNext(CContext *pContext) const
{
    int index = pContext->m_captureindex[m_nnumber];
...
template <class CHART>
int CDelegateElxT<CHART>::Match(CContext *pContext) const
{
    if (m_pelx != 0)
...
template <class CHART>
int CDelegateElxT<CHART>::MatchNext(CContext *pContext) const
{
    if (m_pelx != 0)
...
template <class CHART>
int CPosixElxT<CHART>::misblank(int c)
{
    return c == 0x20 || c == '\t';
}

template <class CHART>
int CPosixElxT<CHART>::Match(CContext *pContext) const
{
...
template <class CHART>
int CPosixElxT<CHART>::MatchNext(CContext *pContext) const
{
...
template <class CHART>
int CRangeElxT<CHART>::Match(CContext *pContext) const
{
    int tlen = pContext->m_pMatchStringLength;
    int npos = pContext->m_nCurrentPos;
...
template <class CHART>
int CRangeElxT<CHART>::MatchNext(CContext *pContext) const
{
    pContext->m_nCurrentPos -= m_brightleft ? -1 : 1;
    return 0;
}
...
template <class CHART>
int CStringElxT<CHART>::Match(CContext *pContext) const
{
    const CHART *pcsz = (const CHART *)pContext->m_pMatchString;
    int          npos = pContext->m_nCurrentPos;
    int          tlen = pContext->m_pMatchStringLength;
...
template <class CHART>
int CStringElxT<CHART>::MatchNext(CContext *pContext) const
{
    int slen = m_szPattern.GetSize();
...
template <class CHART>
int CConditionElxT<CHART>::Match(CContext *pContext) const
{
    // status
...
template <class CHART>
int CConditionElxT<CHART>::MatchNext(CContext *pContext) const
{
    // pop
...
//
template <class CHART>
CBuilderT<CHART>::CBuilderT() : m_pattern(0, 0), prev(0, 0), curr(0, 0), next(0, 0), nex2(0, 0)
{
    Clear();
}
            ...
            switch (ch1) {
                ...
                case RCHART('u'):
                    if (m_pattern[m_nNextPos + 2] != '{') {
                        ...
                        while (m_nNextPos + delta < m_pattern.GetSize() &&
                               m_pattern.At(m_nNextPos + delta) != RCHART('}'))
                            delta++;
...
ElxInterface *CBuilderT<CHART>::BuildSimple(int &flags)
{
    ...
    while (curr != CHART_INFO(0, 1)) {
        if (curr.type == 0) {
            ...
            if (vch == RCHART('^') || vch == RCHART('$') || vch == RCHART('A') || vch == RCHART('Z') ||
                vch == RCHART('b') || vch == RCHART('B') ||
                vch == RCHART('G')  // vch == RCHART('<') || vch == RCHART('>')
            ) {
    ...
    if (fixed.GetSize() > 0)
        return Keep(
            new CStringElxT<CHART>(fixed.GetBuffer(), fixed.GetSize(), flags & RIGHTTOLEFT, flags & IGNORECASE));
    else
        return GetStockElx(STOCKELX_EMPTY);
}
...
ElxInterface *CBuilderT<CHART>::BuildCharset(int &flags)
{
    ...
    switch (ch) {
        case RCHART('.'):
            return GetStockElx(flags & RIGHTTOLEFT ? ((flags & SINGLELINE) ? STOCKELX_DOT_ALL_RIGHTLEFT
                                                                           : STOCKELX_DOT_NOT_ALL_RIGHTLEFT)
        ...
        case RCHART('['): {
            ...
            while (curr != CHART_INFO(0, 1) && curr != CHART_INFO(RCHART(']'), 1)) {
                ...
                if (curr.type == 1 &&
                    (ch == RCHART('.') || ch == RCHART('w') || ch == RCHART('W') || ch == RCHART('s') ||
                     ch == RCHART('S') || ch == RCHART('d') || ch == RCHART('D') ||
...
ElxInterface *CBuilderT<CHART>::BuildBackref(int &flags)
{
    ...
    if (curr.ch == RCHART('<') || curr.ch == RCHART('\'')) {
        ...
        CBackrefElxT<CHART> *pbackref =
            (CBackrefElxT<CHART> *)Keep(new CBackrefElxT<CHART>(-1, flags & RIGHTTOLEFT, flags & IGNORECASE));
...
class CRegexpT {
   ...
   public:
    ...
    CHART *Replace(const CHART *tstring, int string_length, const CHART *replaceto, int to_length, int &result_length,
                   int start = -1, int ntimes = -1, MatchResult *result = 0, CContext *pContext = 0) const;
...
template <class CHART>
CHART *CRegexpT<CHART>::Replace(const CHART *tstring, int string_length, const CHART *replaceto, int to_length,
                                int &result_length, int start, int ntimes, MatchResult *remote_result,
                                CContext *oContext) const
{
    ...
    static const CHART     rtoptn[]  = {RCHART('\\'), RCHART('$'),  RCHART('('),  RCHART('?'), RCHART(':'),
                                   RCHART('['),  RCHART('$'),  RCHART('&'),  RCHART('`'), RCHART('\''),
                                   RCHART('+'),  RCHART('_'),  RCHART('\\'), RCHART('d'), RCHART(']'),
                                   RCHART('|'),  RCHART('\\'), RCHART('{'),  RCHART('.'), RCHART('*'),
                                   RCHART('?'),  RCHART('\\'), RCHART('}'),  RCHART(')'), RCHART('\0')};
    ...
    for (ntime = 0; ntimes < 0 || ntime < ntimes; ntime++) {
        ...
        while (1) {
            ...
            switch (replaceto[res.GetStart() + 1]) {
                ...
                case RCHART('+'):
                    ...
                    buf.Push(new StringRef(tstring + result->GetGroupStart(nmatch),
                                           result->GetGroupEnd(nmatch) - result->GetGroupStart(nmatch)));
                    break;
        ...
        // after
        if (lastI < to_length) buf.Push(new StringRef(replaceto + lastI, to_length - lastI));
...
template <int x>
int CAlternativeElxT<x>::Match(CContext *pContext) const
{
...
template <int x>
int CAlternativeElxT<x>::MatchNext(CContext *pContext) const
{
...
template <int x>
int CAssertElxT<x>::Match(CContext *pContext) const
{
    int nbegin = pContext->m_nCurrentPos;
    int nsize  = pContext->m_stack.GetSize();
    int ncsize = pContext->m_capturestack.GetSize();
...
template <int x>
int CAssertElxT<x>::MatchNext(CContext *pContext) const
{
    int ncsize = 0;
...
template <int x>
int CEmptyElxT<x>::Match(CContext *) const
{
    return 1;
}

template <int x>
int CEmptyElxT<x>::MatchNext(CContext *) const
{
    return 0;
}
...
template <int x>
int CGlobalElxT<x>::Match(CContext *pContext) const
{
    return pContext->m_nCurrentPos == pContext->m_nBeginPos;
}

template <int x>
int CGlobalElxT<x>::MatchNext(CContext *) const
{
    return 0;
}
...
//
template <int x>
CGreedyElxT<x>::CGreedyElxT(ElxInterface *pelx, int nmin, int nmax) : CRepeatElxT<x>(pelx, nmin)
{
    m_nvart = nmax - nmin;
}

template <int x>
int CGreedyElxT<x>::Match(CContext *pContext) const
{
...
template <int x>
int CGreedyElxT<x>::MatchNext(CContext *pContext) const
{
...
template <int x>
int CGreedyElxT<x>::MatchVart(CContext *pContext) const
{
    int n      = 0;
    int nbegin = pContext->m_nCurrentPos;
...
template <int x>
int CGreedyElxT<x>::MatchNextVart(CContext *pContext) const
{
    int n = 0;
    pContext->m_stack.Pop(n);
...
template <int x>
int CIndependentElxT<x>::Match(CContext *pContext) const
{
    int nbegin = pContext->m_nCurrentPos;
    int nsize  = pContext->m_stack.GetSize();
    int ncsize = pContext->m_capturestack.GetSize();
...
template <int x>
int CIndependentElxT<x>::MatchNext(CContext *pContext) const
{
...
template <int x>
int CListElxT<x>::Match(CContext *pContext) const
{
    if (m_elxlist.GetSize() == 0) return 1;
...
template <int x>
int CListElxT<x>::MatchNext(CContext *pContext) const
{
    if (m_elxlist.GetSize() == 0) return 0;
...
//
template <int x>
CPossessiveElxT<x>::CPossessiveElxT(ElxInterface *pelx, int nmin, int nmax) : CGreedyElxT<x>(pelx, nmin, nmax)
{
}

template <int x>
int CPossessiveElxT<x>::Match(CContext *pContext) const
{
    int nbegin = pContext->m_nCurrentPos;
    int nsize  = pContext->m_stack.GetSize();
    int ncsize = pContext->m_capturestack.GetSize();
...
template <int x>
int CPossessiveElxT<x>::MatchNext(CContext *pContext) const
{
    int nbegin = 0, ncsize = 0;
...
//
template <int x>
CReluctantElxT<x>::CReluctantElxT(ElxInterface *pelx, int nmin, int nmax) : CRepeatElxT<x>(pelx, nmin)
{
    m_nvart = nmax - nmin;
}

template <int x>
int CReluctantElxT<x>::Match(CContext *pContext) const
{
    if (!CRepeatElxT<x>::MatchFixed(pContext)) return 0;
...
template <int x>
int CReluctantElxT<x>::MatchNext(CContext *pContext) const
{
    if (MatchNextVart(pContext)) return 1;
...
template <int x>
int CReluctantElxT<x>::MatchVart(CContext *pContext) const
{
...
template <int x>
int CReluctantElxT<x>::MatchNextVart(CContext *pContext) const
{
...
template <int x>
int CRepeatElxT<x>::Match(CContext *pContext) const
{
    return MatchFixed(pContext);
}

template <int x>
int CRepeatElxT<x>::MatchNext(CContext *pContext) const
{
    return MatchNextFixed(pContext);
}

template <int x>
int CRepeatElxT<x>::MatchFixed(CContext *pContext) const
{
...
template <int x>
int CRepeatElxT<x>::MatchNextFixed(CContext *pContext) const
{
