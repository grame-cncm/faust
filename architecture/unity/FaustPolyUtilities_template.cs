...
namespace FaustUtilities_MODEL {
	...
	*/
	public class Faust_Context {
        ...
        #if UNITY_STANDALONE_OSX || UNITY_STANDALONE_WIN || UNITY_EDITOR  || UNITY_WSA || UNITY_WSA_10_0 || UNITY_IOS
		...
		[DllImport(_dllName)]
		private static extern IntPtr Faust_contextNew(int buffersize, int nvoices);
		...
		[DllImport(_dllName)]
		private static extern void Faust_keyOn(IntPtr ctx, int channel, int pitch, int velocity);
   		
		[DllImport(_dllName)]
		private static extern void Faust_keyOff(IntPtr ctx, int channel, int pitch, int velocity);
   
		[DllImport(_dllName)]
		private static extern void Faust_ctrlChange(IntPtr ctx, int channel, int ctrl, int value); 
   
		[DllImport(_dllName)]
		private static extern void Faust_allNotesOff(IntPtr ctx);

		public Faust_Context(int buffersize) {
			_context = Faust_contextNew(buffersize, 16);
		}
