...
<< includeIntrinsic >>
   ...
   public:
    ...
    void ComputeUnityParameters(UnityAudioEffectDefinition* definition)
    {
        ...
        UnityAudioParameterDefinition* pdef =
            (UnityAudioParameterDefinition*)malloc(n * sizeof(UnityAudioParameterDefinition));
        for (int i = 0; i < n; i++) {
...
UNITY_AUDIODSP_RESULT UNITY_AUDIODSP_CALLBACK ProcessCallback(UnityAudioEffectState* state, float* inbuffer,
                                                              float* outbuffer, unsigned int length, int inchannels,
                                                              int outchannels)
{
...
UNITY_AUDIODSP_RESULT UNITY_AUDIODSP_CALLBACK SetFloatParameterCallback(UnityAudioEffectState* state, int pnum,
                                                                        float pval)
{
    unitydsp* p = (unitydsp*)state->effectdata;
...
UNITY_AUDIODSP_RESULT UNITY_AUDIODSP_CALLBACK GetFloatParameterCallback(UnityAudioEffectState* state, int pnum,
                                                                        float* pval, char*)
{
    unitydsp* p = (unitydsp*)state->effectdata;
    if (p) {
