...
def _widget_addresses(json_metadata: Mapping) -> Dict[str, str]:
	"""Map each widget varname to its UI address path.

	The map serves as a lightweight identity for a compiled DSP: two different
	DSPs can share generic zone names like ``fHslider0``, but they will rarely
	also share the full UI address paths.

	Args:
		json_metadata: The parsed Faust JSON of a generated module (a class
			attribute, so no module construction is needed).

	Returns:
		Dict mapping varnames (e.g. ``fHslider0``) to addresses
	...
	def walk(items: List[Dict[str, Any]]) -> None:
		for item in items:
			if "items" in item:
				walk(item["items"])
			elif "varname" in item:
				addresses[item["varname"]] = item["address"]

	walk(json_metadata["ui"])
	return addresses


def save_params(variables: Mapping, path: str | Path, model: Optional[nn.Module] = None) -> None:
	...
	Args:
		variables: The variables pytree returned by ``model.init`` (e.g.
			``{"params": {...}}``).
		path: Destination ``.safetensors`` file path.
		model: Optional module the variables belong to. When given, the file's
			metadata header records the DSP identity (class name and widget
			address map from ``json_metadata``) so that :func:`load_params`
			can verify the file was saved from the same compiled DSP.
	"""
	extra_metadata = None
	if model is not None:
		extra_metadata = {
			"__dsp_class__": type(model).__name__,
			"__labels__": json.dumps(_widget_addresses(type(model).json_metadata)),
		}
	_save_state_safetensors(variables, path, extra_metadata)
...
def load_params(
	...
	Args:
		...
		expected: Optional reference ``variables`` pytree (e.g. the result of
			``model.init``) whose structure the file must match exactly. This
			catches shape/name mismatches, but different DSPs reuse generic
			parameter names (most DSPs have an ``fHslider0``), so it cannot
			detect a file saved from an unrelated DSP with the same key set —
			pass ``model`` for that.
		model: Optional module to verify DSP identity against. When the file
			records identity metadata (written by ``save_params(..., model=...)``)
			whose widget addresses differ from ``model``'s ``json_metadata``,
			the file comes from a different DSP and loading raises.
	...
	Raises:
		UnknownParameterError: If ``expected`` is given and the file's
			parameter set does not match its structure, or if ``model`` is
			given and the file's recorded DSP identity does not match.
	"""
	...
	if model is not None and "__labels__" in metadata:
		file_labels = json.loads(metadata["__labels__"])
		model_labels = _widget_addresses(type(model).json_metadata)
		if file_labels != model_labels:
			...
			raise UnknownParameterError(
				f"'{path}' (saved from DSP class '{saved_class}') has different "
				f"widget addresses than {type(model).__name__}, so it comes "
				f"from a different DSP: {details}."
			)
	return tree
...
<<includeclass>>
	...
	def load_soundfile(self, filepath: str) -> Tuple[np.ndarray, int]:
		...
		# soundfile_dirs should always include at least "". The generated Linen
		# module declares the field as Optional[List[str]] = None (a mutable
		# default is not possible on a frozen dataclass field), so treat None
		# as "no extra directories".
		soundfile_dirs = [""] + list(self.soundfile_dirs or [])
	...
	def add_bargraph(self, zone: str, ui_path: List[str], label: str, a_min: float, a_max: float, unnorm_funcs: Dict[str, Tuple[str, Callable]]) -> None:
		...
		# Register the zone as a plain (non-parameter) attribute so that
		# initialize_carry's dir(self) scan seeds the carry entry the generated
		# tick writes into (jax.lax.scan requires a stable carry pytree).
	...
	def _finalize_interface(self) -> None:
		...
		Called by the generated ``setup`` after ``build_interface``. Walks the
		...
		Uses ``object.__setattr__`` because Flax Linen modules are frozen
		dataclasses outside of dataclass field assignment.
		"""
		...
		object.__setattr__(self, "_key_to_zone", key_to_zone)
	...
	def _extract_rng_key(
		...
		# call via apply(..., rngs=...) or passed explicitly). This is only called
		# for stochastic DSPs (is_stochastic), which need an explicit RNG.
		if rngs is None:
			raise InvalidRNGError(
				"This DSP is stochastic (it calls random_* foreign functions), "
				"so an RNG is required: pass `rngs` to the call"
			)
	...
	def unnormalize(self, gumbel_key: Optional[Array] = None) -> Dict[str, Array]:
		"""
	...
	def unnormalize_params(self, normalized_params: Dict[str, ArrayLike], gumbel_key: Optional[Array] = None) -> Dict[str, Array]:
		"""
	...
	def get_continuous_params(self, batch_size: Optional[int] = None, normalized: bool = True) -> Dict[str, Array]:
		"""
		...
		for zone, metadata in self._parameter_metadata.items():
			...
			if param_type in ["slider", "hslider", "vslider", "button", "checkbox"]:
				if normalized:
					...
					continuous[zone] = unnorm_func(jnp.asarray(getattr(self, zone)))
	...
	def get_categorical_params(self, batch_size: Optional[int] = None) -> Dict[str, Dict[str, Array]]:
		"""
	...
	def params_from_labels(self, label_params: Dict[str, Any]) -> Dict[str, Any]:
		...
		use it when you need explicit zone keys.
	...
	def with_defaults(self, partial_params: Dict[str, Any]) -> Dict[str, Array]:
		"""
		...
		Example:
			...
			>>> audio = model.apply(variables, inputs, params=full_params)
		"""
		return self._prepare_params(partial_params, None, None)

	def initialize_carry(self) -> Dict[str, Array]:
		"""
	...
	def process_block(
		...
		*,
		...
		# Merge unnormalized params into a copy of the carry state (the caller's
		# dict must stay usable, e.g. for checkpointing or retrying a block)
		carry = dict(carry)
...
def test(args: argparse.Namespace) -> None:
	...
	if args.tabulate is not False:
		...
		for zone, metadata in model.get_parameter_metadata().items():
			...
			if param_type in ["slider", "hslider", "vslider"]:
				logger.info(f"  {full_label} [{zone}]: {metadata['min']} .. {metadata['max']} (default: {metadata['default']}, scale: {metadata['scale_mode']})")
			elif param_type == "nentry":
				# A step<=0 nentry registers as a continuous parameter and has
				# no discrete option count.
				options = metadata["num_options"] if "num_options" in metadata else "continuous"
				logger.info(f"  {full_label} [{zone}]: nentry {metadata['min']} .. {metadata['max']} (step: {metadata['step']}, options: {options})")
			elif param_type in ["button", "checkbox"]:
				logger.info(f"  {full_label} [{zone}]: {param_type}")
			elif param_type == "soundfile":
				logger.info(f"  {full_label} [{zone}]: soundfile ({metadata['num_files']} files)")
			else:
				logger.info(f"  {full_label} [{zone}]: {param_type}")
	...
	if args.jit:
		# Run the forward pass via apply(). The RNG key is bound inside a method
		# closure (apply() reserves its own `rngs=` kwarg for make_rng), so we
		# never mutate an RNG counter inside the jit trace.
		@jax.jit
		def forward(x: ArrayLike):
			return _model.apply(
				variables, x,
				method=lambda mdl, xx: mdl(xx, unroll=args.unroll, rngs=key),
			)
	else:
		def forward(x: ArrayLike):
...
def realtime_audio_example(
	...
	# variables; the RNG key is bound in a method closure to avoid apply()'s
	# `rngs=` kwarg).
	@partial(jax.jit, donate_argnums=(0,))
	def process_block_jit(carry, inputs: ArrayLike, rng_key: Array):
		new_carry, outputs = _model.apply(
			variables, carry, inputs,
			method=lambda mdl, c, i: mdl.process_block(c, i, unroll=unroll, rngs=rng_key),
		)
		return new_carry, outputs
