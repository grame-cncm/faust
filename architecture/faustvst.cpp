...
class VSTUI : public UI {
   ...
   protected:
    ...
    void add_elem(ui_elem_type_t type, const char *label, float *zone, float init, float min, float max, float step);
    void add_elem(ui_elem_type_t type, const char *label, float *zone, float min, float max);
...
inline void VSTUI::add_elem(ui_elem_type_t type, const char *label, float *zone, float init, float min, float max,
                            float step)
{
    ui_elem_t *elems1 = (ui_elem_t *)realloc(elems, (nelems + 1) * sizeof(ui_elem_t));
    if (elems1)
...
struct MTSTuning {
    ...
    MTSTuning(const MTSTuning &t) : name(0), len(0), data(0) { *this = t; }
...
MTSTuning::MTSTuning(const char *filename)
{
    ...
    if (data[0] != 0xf0 || data[len - 1] != 0xf7 ||                       // not a sysex message
        ...
        !((len == 21 && data[4] == 8) || (len == 33 && data[4] == 9))) {  // no 1- or 2-byte tuning
        free(data);
        len  = 0;
        data = 0;
        return;
...
struct VSTPlugin {
    ...
    static const char *pluginName() { return meta_get("name", "mydsp"); }

    static const char *pluginAuthor() { return meta_get("author", ""); }

    static const char *pluginDescription() { return meta_get("description", ""); }

    static const char *pluginVersion() { return meta_get("version", "0.0"); }
    ...
    static int numControls()
    {
        ...
        if (num_voices > 0 && load_sysex_data()) num_extra += (mts->tuning.size() > 0);
#endif
    ...
    VSTPlugin(const int num_voices, const int sr)
        : maxvoices(num_voices),
          ndsps(num_voices <= 0 ? 1 : num_voices),
          vd(num_voices > 0 ? new VoiceData(num_voices) : 0)
    {
        ...
        if (vd) {
            ...
            for (int i = 0; i < 16; i++) {
                ...
                for (int j = 0; j < 12; j++) vd->tuning[i][j] = 0.0f;
            }
            vd->n_used = 0;
        ...
        assert(k == 0 || (ctrls && inctrls && outctrls && ports && portvals && units));
        ...
        for (int i = 0, j = 0; i < ui[0]->nelems; i++) {
            ...
            switch (ui[0]->elems[i].type) {
                ...
                case UI_V_BARGRAPH:
                    ...
                    outctrls[q++] = i;
                    {
                        std::map<int, list<strpair> >::iterator it = ui[0]->metadata.find(i);
                        if (it != ui[0]->metadata.end()) {
                            for (std::list<strpair>::iterator jt = it->second.begin(); jt != it->second.end(); jt++) {
                                const char *key = jt->first, *val = jt->second;
#if DEBUG_META
                                fprintf(stderr, "ctrl '%s' meta: '%s' -> '%s'\n", ui[0]->elems[i].label, key, val);
#endif
                                if (strcmp(key, "unit") == 0) unit = val;
                            }
                        }
                ...
                default:
                    ...
                    if (maxvoices == 0)
                        goto noinstr;
                    else if (freq == -1 && !strcmp(ui[0]->elems[i].label, "freq"))
                        freq = i;
                    else if (gain == -1 && !strcmp(ui[0]->elems[i].label, "gain"))
                        gain = i;
                    else if (gate == -1 && !strcmp(ui[0]->elems[i].label, "gate"))
                        gate = i;
                    else {
                    noinstr:
                        std::map<int, list<strpair> >::iterator it = ui[0]->metadata.find(i);
		...
		fprintf(stderr, "%s: cc %d -> %s\n", dsp_name, num,
			ui[0]->elems[i].label);
                        ...
                        units[p]               = unit;
                        for (int ch = 0; ch < 16; ch++) midivals[ch][p] = val;
                    }
                    break;
            }
        }
    ...
    ~VSTPlugin()
    {
        ...
        free(portvals);
        free(units);
        for (int ch = 0; ch < 16; ch++) free(midivals[ch]);
        if (inbuf) {
            for (int i = 0; i < n; i++) free(inbuf[i]);
            free(inbuf);
        }
        if (outbuf) {
            for (int i = 0; i < m; i++) free(outbuf[i]);
            free(outbuf);
        }
        free(dsp);
        free(ui);
    ...
    void print_voices(const char *msg)
    {
        ...
        for (uint8_t ch = 0; ch < 16; ch++)
            for (int note = 0; note < 128; note++)
                if (vd->notes[ch][note] >= 0) fprintf(stderr, " [%d] %d(#%d)", ch, note, vd->notes[ch][note]);
        fprintf(stderr, "\nqueued (%d):", vd->queued.size());
        for (int i = 0; i < nvoices; i++)
            if (vd->queued.find(i) != vd->queued.end()) fprintf(stderr, " #%d", i);
        fprintf(stderr, "\nused (%d):", vd->n_used);
        for (boost::circular_buffer<int>::iterator it = vd->used_voices.begin(); it != vd->used_voices.end(); it++)
            fprintf(stderr, " #%d->%d", *it, vd->note_info[*it].note);
        fprintf(stderr, "\nfree (%d):", vd->n_free);
        for (boost::circular_buffer<int>::iterator it = vd->free_voices.begin(); it != vd->free_voices.end(); it++)
            fprintf(stderr, " #%d", *it);
        fprintf(stderr, "\n");
    }
#endif
    ...
    int alloc_voice(uint8_t ch, int8_t note, int8_t vel)
    {
        ...
        if (i >= 0) {
            ...
            for (boost::circular_buffer<int>::iterator it = vd->used_voices.begin(); it != vd->used_voices.end();
                 it++) {
                if (*it == i) {
    ...
    {
        float pitch = note + vd->tune[chan] + vd->tuning[chan][note % 12] + vd->bend[chan];
...
#if DEBUG_VOICES
        fprintf(stderr, "voice on: %d %d (%g Hz) %d (%g)\n", i, note, midicps(note, ch), vel, vel / 127.0);
#endif
        if (freq >= 0) *ui[i]->elems[freq].zone = midicps(note, ch);
        if (gate >= 0) *ui[i]->elems[gate].zone = 1.0f;
        if (gain >= 0) *ui[i]->elems[gain].zone = vel / 127.0;
...
#endif
        if (gate >= 0) *ui[i]->elems[gate].zone = 0.0f;
    }
    ...
    void all_notes_off()
    {
        for (int i = 0; i < nvoices; i++) voice_off(i);
        for (int i = 0; i < 16; i++) vd->bend[i] = 0.0f;
        memset(vd->notes, 0xff, sizeof(vd->notes));
        vd->free_voices.clear();
        vd->n_free = nvoices;
        for (int i = 0; i < nvoices; i++) vd->free_voices.push_back(i);
        vd->queued.clear();
    ...
    void resume()
    {
        for (int i = 0; i < ndsps; i++) dsp[i]->init(rate);
        for (int i = 0, j = 0; i < ui[0]->nelems; i++) {
            int p = ui[0]->elems[i].port;
            if (p >= 0) {
                float val   = ui[0]->elems[i].init;
    ...
    void process_audio(int blocksz, float **inputs, float **outputs)
    {
        ...
        if (!active) {
            ...
            if (n == m) {
                ...
                for (int i = 0; i < m; i++)
                    for (unsigned j = 0; j < blocksz; j++) outputs[i][j] = inputs[i][j];
            } else {
                // silence
                for (int i = 0; i < m; i++)
                    for (unsigned j = 0; j < blocksz; j++) outputs[i][j] = 0.0f;
            }
            return;
        }
        ...
        for (int i = 0; i < n_in; i++) {
            ...
            if (newval != oldval) {
                ...
                for (int ch = 0; ch < 16; ch++) midivals[ch][k] = newval;
        ...
        if (n_samples < blocksz) {
            ...
            if (outbuf) {
                for (int i = 0; i < m; i++) {
                    outbuf[i] = (float *)realloc(outbuf[i], blocksz * sizeof(float));
                    assert(outbuf[i]);
                }
            }
            n_samples = blocksz;
        ...
        if (outbuf) {
            ...
            for (int l = 0; l < nvoices; l++) {
                ...
                for (int i = 0; i < m; i++)
                    for (unsigned j = 0; j < blocksz; j++) outputs[i][j] += outbuf[i][j];
            }
        } else {
    ...
    void process_midi(unsigned char *data, int sz)
    {
        ...
        for (int i = 0; i < sz; i++) fprintf(stderr, " 0x%0x", data[i]);
        fprintf(stderr, "\n");
#endif
        ...
        switch (status) {
            ...
            case 0xe0: {
                ...
                vd->bend[chan] = (val - 0x2000) / 8192.0f * vd->range[chan];
#if DEBUG_MIDICC
                fprintf(stderr, "pitch-bend (chan %d): %g cent\n", chan + 1, vd->bend[chan] * 100.0);
#endif
                update_voices(chan);
                break;
            }
                ...
                switch (data[1]) {
                        ...
                        if (rpn_msb[chan] == 0) {
                            switch (rpn_lsb[chan]) {
                                case 0:
                                    ...
                                    // in cents
                                    vd->range[chan] = data_msb[chan] + data_lsb[chan] / 100.0;
#if DEBUG_RPN
                                ...
                                case 1: {
                                    ...
                                    int value      = (data_msb[chan] << 7) | data_lsb[chan];
                                ...
                                master_tune:
                                    vd->tune[chan] = vd->coarse[chan] + vd->fine[chan];
#if DEBUG_RPN
                    ...
                    default: {
                        ...
                        if (it != ctrlmap.end()) {
                            ...
                            int   j = inctrls[it->second], k = ui[0]->elems[j].port;
                            ...
                            if (is_instr) {
                                ...
                                for (boost::circular_buffer<int>::iterator it = vd->used_voices.begin();
                                     it != vd->used_voices.end(); it++) {
                                    int i = *it;
                                    if (vd->note_info[i].ch == chan) *ui[i]->elems[j].zone = val;
                                }
                            } else {
    ...
    void process_sysex(uint8_t *data, int sz)
    {
        ...
        if ((data[0] == 0x7e || data[0] == 0x7f) && data[2] == 8) {
            ...
            if ((sz == 19 && data[3] == 8) || (sz == 31 && data[3] == 9)) {
                ...
                for (int i = 0; i < 12; i++) {
                    ...
                    for (uint8_t ch = 0; ch < 16; ch++)
                        if (chanmsk & (1 << ch)) vd->tuning[ch][i] = t;
                }
                if (realtime) {
                    for (uint8_t ch = 0; ch < 16; ch++)
                        if (chanmsk & (1 << ch)) {
...
#if DEBUG_MTS
                fprintf(stderr, "octave-tuning-%s (chan ", realtime ? "realtime" : "non-realtime");
                bool first = true;
                for (uint8_t i = 0; i < 16;)
                    if (chanmsk & (1 << i)) {
                        uint8_t j;
                        for (j = i + 1; j < 16 && (chanmsk & (1 << j));) j++;
                        if (first)
                            first = false;
                        else
                            fprintf(stderr, ",");
    ...
    void change_tuning(int num)
    {
        ...
        modified = true;
        if (num < 0) num = 0;
        if (num > mts->tuning.size()) num = mts->tuning.size();
        tuning = num;
        if (tuning > 0) {
            process_sysex(mts->tuning[tuning - 1].data, mts->tuning[tuning - 1].len);
        } else {
            memset(vd->tuning, 0, sizeof(vd->tuning));
#if DEBUG_MTS
            fprintf(stderr, "octave-tuning-default (chan 1-16): equal temperament\n");
#endif
#endif
        }
    }
...
class VSTWrapper : public AudioEffectX {
   public:
    ...
    int map_param(int i) { return (i < 0 || i >= n_params) ? i : param_no[i]; }

    int map_control(int i) { return (i < 0 || i >= n_params) ? i : control_no[i]; }
...
VSTWrapper::VSTWrapper(audioMasterCallback audioMaster) : AudioEffectX(audioMaster, 1, VSTPlugin::numControls())
{
...
bool VSTWrapper::getProgramNameIndexed(VstInt32 category, VstInt32 index, char *text)
{
    if (index < 1) {
...
void VSTWrapper::getParameterDisplay(VstInt32 index, char *text)
{
    ...
    if (index < k) {
        ...
        sprintf(text, "%d %s", plugin->tuning,
                plugin->tuning > 0 ? plugin->mts->tuning[plugin->tuning - 1].name : "default");
#endif
    }
}
...
void VSTWrapper::setParameter(VstInt32 index, float value)
{
    ...
    if (index >= 0 && index < k) {
        ...
        if (fabs(val) < fabs(step) || fabs(val) / fabs(max - min) < eps) val = 0.0;
...
bool VSTWrapper::getInputProperties(VstInt32 index, VstPinProperties *properties)
{
    const char *dsp_name = VSTPlugin::pluginName();
    const int   n        = plugin->dsp[0]->getNumInputs();
    if (index < 0 || index >= n) return false;
    snprintf(properties->label, kVstMaxLabelLen, "%s input #%d", dsp_name, index);
    sprintf(properties->shortLabel, "In%d", index);
    properties->flags = kVstPinIsActive;
    // XXXTODO: deal with multi-channel setups (>2) here
    if (n == 2) properties->flags |= kVstPinIsStereo;
    return true;
}

bool VSTWrapper::getOutputProperties(VstInt32 index, VstPinProperties *properties)
{
    const char *dsp_name = VSTPlugin::pluginName();
    const int   n        = plugin->dsp[0]->getNumOutputs();
    if (index < 0 || index >= n) return false;
    snprintf(properties->label, kVstMaxLabelLen, "%s output #%d", dsp_name, index);
...
void VSTWrapper::processReplacing(float **inputs, float **outputs, VstInt32 n_samples)
{
 ...
 * @param effect
 */
VSTQtGUI::VSTQtGUI(VSTWrapper *effect)
    : effect(effect),
      widget(NULL),
      uidsp(NULL),
#ifdef OSCCTRL
...
struct QTGUIElem {
    ...
    QList<int>  p;
    const char *label;
    QTGUIElem(int _i, QList<int> _p, const char *_label) : i(_i), p(_p), label(_label) {}
};
...
class QTGUIWrapper : public UI {
   ...
   public:
    int *elem_no;
    int  nelems;
    QTGUIWrapper(QTGUI *_ui, int _maxvoices, int _numtunings, float *_voices_zone, float *_tuning_zone)
        : is_instr(_maxvoices > 0),
          ui(_ui),
          level(0),
          maxvoices(_maxvoices),
          numtunings(_numtunings),
          voices_zone(_voices_zone),
          tuning_zone(_tuning_zone),
          have_freq(false),
          have_gain(false),
          have_gate(false),
          elem_no(0),
    ...
    virtual void closeBox()
    {
        if (--level == 0) {
            ...
            if (is_instr) {
                ...
                ui->addHorizontalSlider("polyphony", voices_zone, maxvoices / 2, 0, maxvoices, 1);
                addElem("Polyphony");
#if FAUST_MTS
                if (numtunings > 0) ui->addHorizontalSlider("tuning", tuning_zone, 0, 0, numtunings, 1);
                addElem("Tuning");
#endif
            }
#endif
            ...
            nelems  = 0;
            for (QList<QTGUIElem>::iterator x = elems.begin(); x != elems.end(); ++x, ++nelems) {
                elem_no[nelems] = x->i;
#if 0
...
bool VSTQtGUI::open(void *ptr)
{
    ...
    QTGUIWrapper qtwrapper(qtinterface, effect->getMaxVoices(), effect->getNumTunings(), &voices_zone, &tuning_zone);
    ...
    for (int i = 0; i < qtwrapper.nelems; i++) effect->control_no[effect->param_no[i]] = i;
#if 0
...
#ifdef HTTPCTRL
    httpdinterface = new httpdUI(VSTPlugin::pluginName(), dsp->getNumInputs(), dsp->getNumOutputs(), argc, argv);
    ...
    for (QList<QObject *>::iterator i = allObjects.begin(); i != allObjects.end(); ++i) {
        ...
        if (num) {
            ...
            if (num->buttonSymbols() != QAbstractSpinBox::NoButtons) {
                ...
                // iteration
            } else if (effect->isPassiveControl(vstParamCount - 1) == 1 && !vBargraphChecked) {
#if FAUSTQT_DEBUG > 1
            ...
            } else if (vstParamCount < effect->n_params && effect->isPassiveControl(vstParamCount) == 2 &&
                       vBargraphChecked) {
        ...
        for (int i = 0; i < controls.size(); ++i) {
            ...
            if (!controls[i].empty() && control_values[i] != val) {
                if (effect->isPassiveControl(i)) {
                    for (QList<QObject *>::iterator it = controls[i].begin(); it != controls[i].end(); ++it)
 ...
 * @return
 */
float VSTQtGUI::valueToVST(double value, double minimum, double maximum)
{
    float newFloat = 0.f;
    if (minimum == maximum)
        return newFloat;
...
void VSTQtGUI::updateQTGUI(QObject *object, float value, bool init)
{
    ...
    if (radioBut) {
        ...
        if (fabs(val - value) / (1 + fabs(maximum - minimum)) < eps) radioBut->click();
        return;
    }
    ...
    if (fabs(newValue) < fabs(step) || fabs(newValue) / fabs(maximum - minimum) < eps) newValue = 0.0;
...
void VSTQtGUI::updateVST()
{
    ...
    if (vstParam >= effect->getNumControls()) {
        ...
        if (vstParam > effect->getNumControls()) effect->updateDisplay();
    }
#endif
}
