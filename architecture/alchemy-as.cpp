...
void BuildUIArray(AS3_Val& array)
{
    for (int i = 0; i < uielems_size; ++i) {
        AS3_Val result = AS3_Object(
            "type:AS3ValType,id:AS3ValType,label:AS3ValType,min:AS3ValType,max:AS3ValType,init:AS3ValType,step:"
            "AS3ValType",
            AS3_Int(uielems[i].type), AS3_Int(uielems[i].id), AS3_String(uielems[i].label), AS3_Number(uielems[i].min),
            AS3_Number(uielems[i].max), AS3_Number(uielems[i].init), AS3_Number(uielems[i].step));
...
class ActionScriptUI : public UI {
   public:
    ...
    void updateControl(int id, float value) { *(uidToZone[id]) = value; }
    ...
    virtual void addCheckButton(char* label, float* zone) { return addToggleButton(label, zone); }
...
int main()
{
    ...
    // e.g. faust.api_tick().
    AS3_Val result =
        AS3_Object("api_init:AS3ValType, api_shutdown:AS3ValType, api_onControlChange:AS3ValType, api_tick:AS3ValType",
