...
function createTree(div, parent){
	var myNode = document.createElement("div");
	myNode.patchID = div.patchID;
	myNode.course = [];
...
// 	Stop Recursion in Tree		
		...
		myNode.sourceCode = div.Source;

		myNode.course[myNode.course.length] = myNode; 

		if(div.inputConnections){
			for(var j=0; j<div.inputConnections.length; j++)
				myNode.nodeInputs[j] = createTree(div.inputConnections[j].source, myNode);
		}
	}
...
// 		var patchName = "MyBeautifulPatch";
...
function createFaustEquivalent(parent){
...
// Save concatenated params in new DIV
		...
		if(DSP){
			...
			var faustDiv = createFaustModule(document.body.scrollWidth/3, document.body.scrollHeight/3, patchName, parent);
