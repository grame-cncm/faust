...
Using developments done for the Web (WebAssembly backends and **libfaust** library compiled in WebAssembly with Emscripten), statically and dynamically Faust generated WebAudio nodes can be easily produced and deployed on the Web.
...
Assuming that the compiled Faust DSP file is [polyphonic ready](/news/2016/01/13/polyphonic-instruments.html), a polyphonic ready WebAudio node (with 16 voices by default) can be created with the *-poly* parameter, and will generate the following class for the node, to be used like: 
...
function startosc()
{
    ...
    faust.createDSPFactory(dsp_code,
                            argv,
                            function (factory) {
                            faust.createDSPWorkletInstance(factory, audio_context, 1024
                                                            function (node) {
                                                                osc = node;
