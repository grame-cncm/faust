...
readarray -t CANDIDATES < <(find "$ROOT" -type f \( -iname '*.c' -o -iname '*.h' -o -iname '*.cpp' -o -iname '*.hpp' \) -print)
...
for f in "${files[@]}"; do
  ...
  # Common typedef adjustments (conservative)
  ...
  # Simple tick expression replacements
  ...
  # Some macro definitions e.g. DEFAULT_ESP_PERIPH_WAIT_TICK
...
echo
echo "Next steps:"
echo "  1) Inspect the remaining files listed above (they may contain complex/macro cases)."
echo "  2) Manually edit remaining tricky cases or ask me to craft targeted fixes."
